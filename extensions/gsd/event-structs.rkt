...
;;
;; v0.99.52 W5: All GSD events now carry an explicit schema-version (1) so
;; verifiers can rely on a stable, registered per-type serialization contract.

(require "../../util/event/event-macro.rkt")

(define-typed-event gsd-mode-changed-event
                    "gsd.mode.changed"
                    (mode)
                    #:optional ([reason #f] [error #f])
                    #:schema-version 1)

(define-typed-event gsd-transition-attempted-event
                    "gsd.transition.attempted"
                    (from to)
                    #:schema-version 1)

(define-typed-event gsd-transition-succeeded-event
                    "gsd.transition.succeeded"
                    (from to)
                    #:schema-version 1)

(define-typed-event gsd-wave-completed-event "gsd.wave.completed" (wave) #:schema-version 1)

(define-typed-event gsd-wave-started-event "gsd.wave.started" (wave) #:schema-version 1)

(define-typed-event gsd-plan-parsed-event "gsd.plan.parsed" (wave-count) #:schema-version 1)

(define-typed-event gsd-command-received-event
                    "gsd.command.received"
                    (command args)
                    #:schema-version 1)

(define-typed-event gsd-archive-failed-event "gsd.archive.failed" (error) #:schema-version 1)

(define-typed-event gsd-plan-validated-event
                    "gsd.plan.validated"
                    (wave-count)
                    #:optional ([valid? #t] [error-count 0] [warning-count 0])
                    #:schema-version 1)

(define-typed-event gsd-plan-normalized-event "gsd.plan.normalized" (wave-count) #:schema-version 1)

(define-typed-event gsd-command-completed-event
                    "gsd.command.completed"
                    (command success)
                    #:schema-version 1)

;; CF1-1: gsd.plan.archived
(define-typed-event gsd-plan-archived-event "gsd.plan.archived" (path) #:schema-version 1)

;; CF1-4: gsd.transition.failed
(define-typed-event gsd-transition-failed-event
                    "gsd.transition.failed"
                    (from to reason)
                    #:schema-version 1)

;; Future-ready: no emission sites yet
(define-typed-event gsd-wave-failed-event "gsd.wave.failed" (wave error) #:schema-version 1)
(define-typed-event gsd-wave-skipped-event "gsd.wave.skipped" (wave reason) #:schema-version 1)
(define-typed-event gsd-guard-blocked-event "gsd.guard.blocked" (tool reason) #:schema-version 1)
(define-typed-event gsd-guard-allowed-event "gsd.guard.allowed" (tool) #:schema-version 1)
