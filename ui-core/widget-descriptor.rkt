...
(provide
 ...
 (contract-out
  ...
  [filter-widgets-by-zone (-> (listof widget-descriptor?) symbol? (listof widget-descriptor?))]
  [filter-widgets-by-trust (-> (listof widget-descriptor?) symbol? (listof widget-descriptor?))]))
...
(define (make-widget-descriptor id extension-id
                                #:zone [zone #f]
                                #:kind [kind 'text]
                                #:content [content ""]
  ...
  (widget-descriptor id extension-id zone kind content priority
                     capabilities trust-level (gensym 'widget-lifecycle)))
