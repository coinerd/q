...
(define (ui-action->deltas action-type payload)
  (cond
    ...
    [(equal? action-type "ui.header.clear") (list (ui-delta DELTA-CLEAR-HEADER #f))]
    [(equal? action-type "ui.footer.set")
     (list (ui-delta DELTA-SET-FOOTER (hash-ref payload 'lines '())))]
    [(equal? action-type "ui.footer.clear") (list (ui-delta DELTA-CLEAR-FOOTER #f))]
    ...
    [(equal? action-type "ui.overlay.show")
     (list (ui-delta DELTA-SHOW-OVERLAY (hash-ref payload 'content (hash))))]
    [(equal? action-type "ui.overlay.dismiss") (list (ui-delta DELTA-DISMISS-OVERLAY #f))]
    [else '()]))
