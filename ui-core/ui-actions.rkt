...
(define (valid-ui-action-name? name)
  (and (string? name)
       (member name all-ui-action-names)
       #t))
...
(define ui-action-schema
  (hasheq
   UI-ACTION-HEADER-SET       '(lines)
   UI-ACTION-HEADER-CLEAR     '()
   UI-ACTION-FOOTER-SET       '(lines)
   UI-ACTION-FOOTER-CLEAR     '()
   UI-ACTION-STATUS-SET       '(status)
   UI-ACTION-WIDGET-REGISTER  '(ext-name key descriptor)
   UI-ACTION-WIDGET-UNREGISTER '(ext-name key)
   UI-ACTION-WIDGET-UNREGISTER-ALL '(ext-name)
   UI-ACTION-OVERLAY-SHOW     '(content)
   UI-ACTION-OVERLAY-DISMISS  '()
   UI-ACTION-THEME-CHANGE     '(theme)
   UI-ACTION-LAYOUT-BREAKPOINT '(breakpoint)
   UI-ACTION-FOCUS-REQUEST    '(component)))
