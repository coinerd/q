...
(provide composer-home
         ...
         (contract-out
          ...
          [composer-next-grapheme-start
           (-> string? exact-nonnegative-integer? exact-nonnegative-integer?)]
          [composer-prev-grapheme-start
           (-> string? exact-nonnegative-integer? exact-nonnegative-integer?)]
          [composer-grapheme-span-at
           (-> string? exact-nonnegative-integer? exact-nonnegative-integer?)]
          ;; Selection helpers
          [composer-selection-range
           (-> composer-state? (values exact-nonnegative-integer? exact-nonnegative-integer?))]
          ...
          [composer-insert-string (->* (composer-state? string?) ((or/c #f symbol?)) composer-state?)]
...
(define (make-composer-state #:buffer [buffer ""]
                             #:cursor [cursor 0]
...
(define (variation-selector? c)
  (or (char<=? #\UFE00 c #\UFE0F) (char<=? #\UE0100 c #\UE01EF)))
      ...
      (let ([ci (string-ref s i)])
        (not
         (or (mark? ci)
             ...
             (and (regional-indicator? ci) (> i 0) (regional-indicator? (string-ref s (sub1 i)))))))))
...
(define (cluster-end s start)
  ...
  (let loop ([i (add1 start)]
    ...
    (cond
      [(>= i n) i]
      [(zwj? (string-ref s i)) (loop (add1 i) #t)]
      [after-zwj? (loop (add1 i) #f)]
      [(continue-char? (string-ref s i)) (loop (add1 i) #f)]
...
(define (composer-grapheme-start? s pos)
  (and (<= 0 pos (string-length s)) (= pos (cluster-start s pos))))
...
(define (composer-next-grapheme-start s pos)
  ...
  (if (>= p n)
      n
      (cluster-end s (cluster-start s p))))
...
(define (composer-grapheme-span-at s pos)
  (if (>= pos (string-length s))
      ...
      (let ([start (cluster-start s pos)]) (max 1 (- (cluster-end s start) start)))))
...
(define (composer-select-all st)
  (struct-copy composer-state st [cursor (string-length (composer-state-buffer st))] [sel-anchor 0]))
  ...
  (if (composer-has-selection? st)
      (let-values ([(a b) (composer-selection-range st)])
        (struct-copy composer-state
                     st
                     [buffer
                      (string-append (substring (composer-state-buffer st) 0 a)
...
(define (push-undo st tag)
  (define entry
    (list (composer-state-buffer st) (composer-state-cursor st) (composer-state-sel-anchor st) tag))
...
(define (commit st tag edited)
  (struct-copy composer-state edited [undo-stack (push-undo st tag)] [redo-stack '()]))
...
(define (insert-at st str tag)
  ...
  (commit st
          ...
          (struct-copy composer-state
                       st
                       [buffer (string-append (substring buf 0 pos) str (substring buf pos))]
                       [cursor (+ pos (string-length str))]
                       [sel-anchor #f]
                       [preferred-col #f])))

(define (composer-insert-char st c)
  (insert-at st (string c) 'type))
(define (composer-insert-string st s [tag #f])
  (if (= (string-length s) 0)
      st
      (insert-at st s tag)))
(define (composer-insert-newline st)
  (insert-at st "\n" 'type))
  ...
  (cond
    ...
    [else
     ...
     (commit st
             ...
             (struct-copy composer-state
                          st
                          [buffer (string-append (substring buf 0 prev) (substring buf cur))]
                          [cursor prev]
                          [preferred-col #f]))]))
         ...
         (let* ([start (cluster-start buf cur)]
           ...
           (commit st
                   ...
                   (struct-copy composer-state
                                st
                                [buffer
                                 (string-append (substring buf 0 start) (substring buf end))]))))]))
  ...
  (if (null? stack)
      ...
      (let* ([e (car stack)]
        ...
        (struct-copy composer-state
                     ...
                     [redo-stack (cons redo-entry (composer-state-redo-stack st))]))))
  ...
  (if (null? stack)
      ...
      (let* ([e (car stack)]
        ...
        (struct-copy composer-state
                     ...
                     [undo-stack (cons undo-entry (composer-state-undo-stack st))]))))
  ...
  (if (= (string-length killed) 0)
      ...
      (commit st
              ...
              (struct-copy composer-state
                           ...
                           [kill-ring (cons killed (composer-state-kill-ring st))]))))

(define (composer-yank st)
  (define ring (composer-state-kill-ring st))
  (if (null? ring)
      st
      (insert-at st (car ring) 'paste)))
...
(define (composer-home st)
  (composer-cursor-home st))
(define (composer-end st)
  (composer-cursor-end st))

(define (composer-at-beginning? st)
  (= (composer-state-cursor st) 0))

(define (composer-at-end? st)
  (= (composer-state-cursor st) (string-length (composer-state-buffer st))))
...
(define (composer-set-selection st anchor)
  ...
  (struct-copy composer-state st [sel-anchor (max 0 (min anchor b))]))
...
(define (composer-replace-buffer st text)
  ...
  (commit st
          ...
          (struct-copy composer-state
                       st
                       [buffer b]
                       [cursor (min (composer-state-cursor st) (string-length b))]
                       [sel-anchor #f]
                       [preferred-col #f])))
...
(module+ test
  ...
  (check-equal? (composer-state-buffer (composer-backspace (make-composer-state #:buffer family
                                                                                #:cursor fam-len)))
                "")
  (check-equal?
   (composer-state-buffer (composer-delete (make-composer-state #:buffer (string-append "a" family)
                                                                #:cursor 1)))
   "a")
  ...
  ;; Movement
  (check-equal? (composer-state-cursor
                 (composer-cursor-left (make-composer-state #:buffer (string-append "a" family)
                                                            #:cursor (add1 fam-len))))
                1)
