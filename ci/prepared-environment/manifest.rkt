...
(define known-safe-prefixes
  '("addon-store/" "q-compiled/" "manifest.json"))
...
(define (sha256-file p)
  ...
  (define (pad! bs)
    ...
    (bytes-append bs (bytes 128) (make-bytes k 0)
  ...
  (define (process-block in h0)
    ...
    (for ([i (in-range 64)])
      ...
      (set! d c) (set! c b) (set! b a) (set! a (word-add temp1 temp2)))
...
(define (path-in-allowlist? rel allowlist)
  ...
  (for/or ([prefix (in-list allowlist)])
    (or (string=? s prefix)
        (and (string-suffix? prefix "/")
             (string-prefix? s prefix)))))

(define (emit! out
               schema-revision
               repository git-sha os arch
               racket-version racket-variant racket-distribution
               lock-digest source-digest allowlist)
  (unless (= schema-revision schema-revision-supported)
    (die "unsupported schema revision ~a (supported: ~a)"
         schema-revision schema-revision-supported))
  ;; The producer may only allowlist known-safe prefixes (I10).
  (for ([p (in-list allowlist)])
    (unless (member p known-safe-prefixes)
      (die "refusing to allowlist non-safe path ~a (known-safe: ~a)"
           p known-safe-prefixes)))
  (define manifest
    (hasheq 'schema_revision schema-revision
            'repository repository
            'git_sha git-sha
            'os os
            'arch arch
            'racket_version racket-version
            'racket_variant racket-variant
            'racket_distribution racket-distribution
            'lock_digest lock-digest
            'source_digest source-digest
            'allowlisted_paths allowlist))
  (call-with-output-file out #:exists 'replace
    (lambda (p) (write-json manifest p) (newline p)))
  (printf "prepared-environment manifest written: ~a (git ~a, ~a/~a, Racket ~a, lock ~a)~n"
          out git-sha os arch racket-version lock-digest))

(define (verify! manifest-path
                 expect-repository expect-git-sha expect-os expect-arch
                 expect-racket-version expect-lock-digest root)
  (define manifest
  ...
  (unless (hash? manifest) (die "manifest is not a JSON object"))

  (define (field name)
    (hash-ref manifest name
              (lambda () (die "manifest is missing required field ~a" name))))
  ...
  (for ([c (in-list checks)])
    ...
    (unless (equal? actual expected)
      (die "manifest mismatch on ~a: artifact has ~s, consumer requires ~s — \
  ...
  (unless (null? offenders)
    (die "artifact contains ~a non-allowlisted path(s), e.g. ~a — refusing to restore"
         (length offenders) (take offenders (min 5 (length offenders)))))

  (printf "prepared-environment manifest VERIFIED: git ~a, ~a/~a, Racket ~a, lock ~a; \
           all extracted paths inside allowlist~n"
          expect-git-sha expect-os expect-arch expect-racket-version expect-lock-digest))
...
;; paths are stable on every hosted image.
(define (non-allowlisted-files root allowlist)
  ...
  (define files (find-files (lambda (_) #t) abs-root))
  (sort
   (for/list ([p (in-list files)]
              #:unless (path-in-allowlist?
                        (find-relative-path abs-root (simplify-path p))
                        allowlist))
  ...
  (match argv
    [(cons "emit" rest)
     ...
     (command-line
      ...
      #:argv rest
      #:once-each
      [("--out") v-out "manifest.json output path" (set! out v-out)]
      ...
      #:multi
      [("--allowlist") v-path "allowlisted artifact path prefix (repeatable)"
       (set! allowlist (cons v-path allowlist))]
      #:args ()
      (for ([required (in-list (list out repository git-sha os arch racket-version
                                     lock-digest source-digest))])
        (when (not required) (die "emit: missing required argument(s)")))
      (emit! (string->path out) schema-revision-supported repository git-sha os arch
             racket-version racket-variant racket-distribution
             lock-digest source-digest (reverse allowlist)))]
    [(cons "verify" rest)
     ...
     (command-line
      ...
      #:argv rest
      #:once-each
      [("--manifest") v-manifest "path to manifest.json" (set! manifest-path v-manifest)]
      ...
      #:args ()
      (for ([required (in-list (list manifest-path expect-repository expect-git-sha
                                     expect-os expect-arch expect-racket-version
                                     expect-lock-digest root))])
        (when (not required) (die "verify: missing required argument(s)")))
      (verify! (string->path manifest-path) expect-repository expect-git-sha
               expect-os expect-arch expect-racket-version expect-lock-digest
               (string->path root)))]
    [(cons "digest" rest)
     (define file #f)
     (command-line
      #:program "manifest.rkt digest"
      #:argv rest
      #:once-each
      [("--file") v-file "file to hash" (set! file v-file)]
      #:args ()
      (unless file (die "digest: --file is required"))
      (displayln (sha256-file (string->path file))))]
    [_ (eprintf "usage: manifest.rkt emit|verify|digest ...~n")
     (exit 2)]))
