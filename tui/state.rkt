  ...
  (case ev
    [("assistant.message.completed")
     ...
     (struct-copy ui-state state
                  [transcript (append (ui-state-transcript state)
    ...
    [("tool.call.started")
     ;; Show tool started, mark busy
     (define name (hash-ref payload 'name "?"))
     (define ts (event-time evt))
     (struct-copy ui-state state
                  [transcript (append (ui-state-transcript state)
                                      (list (transcript-entry 'tool-start
                                                              (format "[tool: ~a]" name)
                                                              ts
                                                              (hash 'name name))))]
                  [busy? #t]
    ...
    [("tool.call.completed")
     ...
     (struct-copy ui-state state
                  [transcript (append (ui-state-transcript state)
                                      (list (transcript-entry 'tool-end
                                                              (format "[tool done: ~a]" name)
                                                              ts
                                                              (hash 'name name))))]
                  [pending-tool-name #f])]
     ...
     (struct-copy ui-state state
                  [transcript (append (ui-state-transcript state)
                                      (list (transcript-entry 'tool-fail
                                                              (format "[tool failed: ~a — ~a]" name err)
                                                              ts
    ...
    [("turn.completed")
     ...
     (struct-copy ui-state state [busy? #f])]
...
(define (scroll-to-top state)
  ...
  (struct-copy ui-state state [scroll-offset (max 0 (- n 1))]))

;; Set mouse selection anchor (start of drag)
(define (set-selection-anchor state col row)
  (struct-copy ui-state state
               [sel-anchor (cons col row)]
               [sel-end (cons col row)]))
...
(define (has-selection? state)
  (and (ui-state-sel-anchor state)
       (ui-state-sel-end state)))
