...
;; Install the cleanup extent before channel creation.  Keeping this helper
;; outside either entry point makes the setup-failure path directly testable.
(define (call-with-tui-approval-channel thunk)
  (dynamic-wind void
                (lambda ()
...
(define (run-tui-with-runtime rt-config cli-cfg)
  ;; This covers failures in set/create/load/init as well as the loop, while
