...
(require racket/port
         ...
         (only-in "../../util/protocol-types.rkt"
                  make-message message-role message-content))
...
(define (run-subagent args role-prompt max-turns allowed-tools exec-ctx)
  ...
  (with-handlers ([exn:fail? (lambda (e)
    ...
    ;; Get the provider from exec-ctx or use mock
    (define provider
      (if exec-ctx
          (let ([rt (exec-context-runtime-settings exec-ctx)])
            (hash-ref rt 'provider (build-mock-provider-for-subagent)))
    ...
    (define settings (hasheq 'model (hash-ref args 'model "mock-model")))

    ;; Create execution context for child
    (define child-ctx
      (make-exec-context
       #:working-directory (if exec-ctx (exec-context-working-directory exec-ctx) (current-directory))
    ...
    (define system-msg (make-message (generate-id) #f 'system 'message (list role-prompt) (current-seconds) (hasheq)))
    (define user-msg (make-message (generate-id) #f 'user 'message (list task) (current-seconds) (hasheq)))
    ...
    (define result-text
      (string-join
       (for/list ([m (in-list result-messages)]
                  #:when (eq? (message-role m) 'assistant))
         (define content (message-content m))
         (if (string? content) content (format "~a" content)))
       "\n"))

    (make-success-result
     (list (hasheq 'type "text" 'text result-text))
     (hasheq 'turns-used max-turns
             'status (symbol->string final-status)
             'session-id session-id))))

;; Run a simple agent loop for the subagent
(define (run-subagent-loop provider registry messages ctx max-turns)
  (let loop ([msgs messages] [turns-remaining max-turns] [all-results '()])
    (cond
      [(<= turns-remaining 0)
       (values all-results 'max-turns-reached)]
      [else
       ...
       (define assistant-msg
         (make-message (generate-id) #f 'assistant 'message
       ...
       (cond
         [(eq? (model-response-stop-reason resp) 'tool_calls)
          ...
          (values new-all 'stopped)]
         [else
          (values new-all 'complete)])])))

;; Helper: create a mock provider for subagent testing
(define (build-mock-provider-for-subagent)
  (define mock-response
    (make-model-response
     (list (hasheq 'type "text" 'text "Subagent task completed."))
...
(define (return-error msg)
  (make-tool-result msg (hasheq 'error #t) #t))
