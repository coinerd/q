...
;;; ("warn in v1.00.20, remove after v1.00.21") is executed here.
...
(define (make-tmp-planning plan-text wave-docs)
  ...
  (with-output-to-file (build-path tmp ".planning" "PLAN.md")
    (lambda () (display plan-text)))
  (for ([wd wave-docs])
...
(define suite
  (test-suite
   "BUG-0023 residual (v1.00.22 W1): inline `## Wave N:` plan rejected at /go"
...
(define seam-suite
  (test-suite
   "BUG-0023 residual (v1.00.22 W1): /go seam enforcement"
      ...
      (lambda ()
        ...
        (match result
          [(list 'error msg)
           ...
           (check-true (regexp-match? #rx"- \\[Inbox\\] W0: Title → waves/W0-slug\\.md" msg)
   ...
   (test-case "canonical index plan: /go loads silently (no deprecation warnings)"
     (define tmp (make-tmp-planning CANONICAL-INDEX-PLAN
   ...
   (test-case "mixed plan (index rows + inline sections): index wins, /go accepts"
     ...
     (define tmp (make-tmp-planning MIXED-PLAN
                                    (list (list "W0-index-wins.md" CANONICAL-WAVE-DOC))))
     (dynamic-wind
      void
      (lambda ()
        (define result (validate-plan-for-go tmp))
...
")
     ...
     (define tmp (make-tmp-planning RELAXED-PLAN
                                    (list (list "W0-index-wins.md" CANONICAL-WAVE-DOC))))
     (dynamic-wind
      void
      (lambda ()
...
(module+ main
  (void (run-tests (test-suite "inline-format-deprecation" suite seam-suite))))
