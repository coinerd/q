...
(define (evt-turn type payload turn-id)
  (event 1 type 1000.0 "S" turn-id payload))
...
(test-case "goal.status appends transcript entry and sets status-message"
  ...
  (define st1
    (apply-event-to-state st (evt "goal.status" (hasheq 'message "Goal turn 1/8: working..."))))
...
(test-case "canonical prompt turn.completed preserves active-goal evaluating state"
  (define st (initial-ui-state #:session-id "S" #:model-name "glm-5.1"))
  (define st1
    (apply-event-to-state st (evt "goal.started" (hasheq 'goal-text "build website" 'max-turns 8))))
  (define st2 (apply-event-to-state st1 (evt "goal.turn.started" (hasheq 'turn-number 1))))
  ...
  (define st4
    (apply-event-to-state
     st3
     (evt-turn "turn.completed" (hasheq 'scope "prompt" 'reason "completed") "prompt-1")))
...
(test-case "canonical prompt terminal with stale turn-id keeps active-goal working state"
  (define st (initial-ui-state #:session-id "S" #:model-name "glm-5.1"))
  (define st1
    (apply-event-to-state st (evt "goal.started" (hasheq 'goal-text "build website" 'max-turns 8))))
  (define st2 (apply-event-to-state st1 (evt "goal.turn.started" (hasheq 'turn-number 1))))
...
(test-case "model.stream.completed with missing or malformed usage does not throw"
  ...
  (check-not-exn (lambda ()
                   (apply-event-to-state st (evt "model.stream.completed" (hasheq 'usage #f))))))
