...
(define-test-suite classification-suite
  ...
  (test-case "read-only tools are auto-approved"
    (for ([name '("read" "ls" "find" "grep" "date")])
      (check-eq? (classify-tool-by-name name) 'auto-approved
  ...
  (test-case "destructive tools need approval"
    (for ([name '("write" "edit" "bash" "delete-lines" "firecrawl"
                  "spawn-subagent" "spawn-subagents"
                  "browser_open" "browser_click" "browser_type"
                  "delete-memory" "clear-memory")])
      (check-eq? (classify-tool-by-name name) 'needs-approval
  ...
  (test-case "auto-approved and needs-approval sets are disjoint"
    (check-equal? (set-intersect auto-approved-tool-names
                                 needs-approval-tool-names)
                  (set)))

  (test-case "union of sets equals all classified names"
    (define all-classified (list->set (all-classified-tool-names)))
    (define union (set-union auto-approved-tool-names needs-approval-tool-names))
