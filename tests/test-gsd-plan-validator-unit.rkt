...
(define suite
  (test-suite
   ...
   (test-case "wave with no files → error (all waves lack files)"
     (define plan (make-plan #:waves (list (make-wave #:files '() #:verify ""))))
     (define result (validate-plan-strict plan))
     ;; Single wave with no files triggers both per-wave warning and global error
     (check-not-false (member "Plan has no file references in any wave — nothing to execute"
                              (validation-result-errors result))))

   (test-case "wave with no title → warnings"
     (define plan (make-plan #:waves (list (make-wave #:title ""))))
     (define result (validate-plan-strict plan))
     (check-not-false (member "Wave 0: no explicit title"
                              (validation-result-warnings result))))

   (test-case "all waves with no files → error"
     (define plan (make-plan #:waves (list (make-wave #:files '()) (make-wave #:files '() #:index 1))))
     (define result (validate-plan-strict plan))
     (check-not-false (member "Plan has no file references in any wave — nothing to execute"
                              (validation-result-errors result))))
   ...
   (test-case "normalized wave with no files → warning"
     (define plan (make-norm-plan #:waves (list (make-norm-wave #:files '()))))
     (define result (validate-normalized-plan plan))
     (check-true (validation-result? result))
     (check-not-false (member "Wave 0: no file references — wave may not produce changes"
                              (validation-result-warnings result))))
   ...
   (test-case "normalized wave with no verify → valid (warnings don't block)"
     ...
     (check-true (or (gsd-validated-plan? result)
                     (and (validation-result? result)
                          (null? (validation-result-errors result))))))

   (test-case "all normalized waves with no files → error"
     (define plan (make-norm-plan #:waves (list (make-norm-wave #:files '())
