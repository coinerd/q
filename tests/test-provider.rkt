...
(test-case "model-request predicate and fields"
  (define req-1
...
(test-case "model-request with tools"
  (define tool-def
    (hash 'type
          "function"
          'function
          (hash 'name "read" 'description "Read a file" 'parameters (hash))))
  (define req-2
...
(test-case "model-response predicate and fields"
  (define resp-1
    (make-model-response (list (hash 'type "text" 'text "Hi there"))
...
(test-case "stream-chunk basic fields"
...
(test-case "stream-chunk done with usage"
...
(test-case "model-request JSON roundtrip"
  (define tool-def
    (hash 'type
          "function"
          'function
...
(test-case "model-response JSON roundtrip"
  (define resp-1
    (make-model-response (list (hash 'type "text" 'text "Hi there"))
                         (hash 'prompt_tokens 10 'completion_tokens 5 'total_tokens 15)
                         "gpt-4"
...
(test-case "mock provider: predicate, name, capabilities"
  (define mock-resp
    (make-model-response (list (hash 'type "text" 'text "Mock response"))
...
(test-case "mock provider: provider-send returns correct response"
  (define mock-resp
    (make-model-response (list (hash 'type "text" 'text "Mock response"))
                         (hash 'prompt_tokens 5 'completion_tokens 3 'total_tokens 8)
                         "mock-model"
...
(test-case "mock provider: provider-stream returns generator of chunks, last is done"
  (define mock-resp
    (make-model-response (list (hash 'type "text" 'text "Mock response"))
                         (hash 'prompt_tokens 5 'completion_tokens 3 'total_tokens 8)
                         "mock-model"
...
(test-case "mock provider with tool-call response"
  (define mock-tool-resp
    (make-model-response
     (list (hash 'type "text" 'text "Reading file...")
           (hash 'type "tool-call" 'id "tc-1" 'name "read" 'arguments (hash 'path "foo.rkt")))
...
(test-case "openai-compatible provider: predicate, name, capabilities"
  (define oai-config
    (hash 'base-url "https://api.openai.com/v1" 'api-key "test-key-123" 'model "gpt-4"))
...
(test-case "openai-build-request-body: non-streaming request"
  (define tool-def
    (hash 'type
          "function"
          'function
  ...
  (for ([tool-entry (in-list oai-tools)])
    ...
    (check-equal? (hash-ref tool-entry 'type)
                  "function"
    ...
    (check-true (string? (hash-ref fn 'name #f)) "function hash should have string 'name'")
...
(test-case "openai-build-request-body: streaming request"
  (define req-1
    (make-model-request (list (hash 'role "user" 'content "Hello"))
                        #f
                        (hash 'model "gpt-4" 'temperature 0.7 'max-tokens 1024)))
...
(test-case "openai-parse-response: text response"
  (define fake-openai-response
    (hash 'id
          ...
          (list (hash 'index
                      0
                      'message
                      (hash 'role "assistant" 'content "Hello from GPT")
                      'finish_reason
                      "stop"))
          'usage
          (hash 'prompt_tokens 10 'completion_tokens 5 'total_tokens 15)))
...
(test-case "openai-parse-response: tool-call response"
          ...
          (list (hash 'index
                      ...
                      (hash 'role
                            ...
                            (list (hash 'id
                                        ...
                                        "function"
                                        'function
                                        (hash 'name "read" 'arguments "{\"path\":\"foo.rkt\"}"))))
                      'finish_reason
                      "tool_calls"))
          'usage
          (hash 'prompt_tokens 20 'completion_tokens 15 'total_tokens 35)))
  ...
  (define tc-parts
    (filter (lambda (c) (equal? (hash-ref c 'type #f) "tool-call"))
...
(test-case "make-model-request with no tools"
  (define req-simple
    (make-model-request (list (hash 'role "user" 'content "Hi")) #f (hash 'model "test")))
...
(test-case "stream-chunk with all fields populated"
  (define sc-full
    (make-stream-chunk "delta"
...
(test-case "BUG-16: HTTP 200 passes without error"
  (check-not-exn
   (λ () ((lambda (sl rb) (check-provider-status! "Provider" sl rb)) #"HTTP/1.1 200 OK" #"{}"))))

(test-case "BUG-16: HTTP 201 passes without error"
  (check-not-exn
   (λ () ((lambda (sl rb) (check-provider-status! "Provider" sl rb)) #"HTTP/1.1 201 Created" #"{}"))))

(test-case "BUG-16: HTTP 401 raises error"
  (check-exn exn:fail?
             (λ ()
               ((lambda (sl rb) (check-provider-status! "Provider" sl rb))
...
(test-case "BUG-16: HTTP 403 raises error"
  (check-exn exn:fail?
             (λ ()
               ((lambda (sl rb) (check-provider-status! "Provider" sl rb)) #"HTTP/1.1 403 Forbidden"
                                                                           #"forbidden"))))

(test-case "BUG-16: HTTP 404 raises error"
  (check-exn exn:fail?
             (λ ()
               ((lambda (sl rb) (check-provider-status! "Provider" sl rb)) #"HTTP/1.1 404 Not Found"
                                                                           #"not found"))))
...
(test-case "BUG-17: Provider default-model injected into empty request settings"
  (define prov
    (make-openai-compatible-provider
     (hash 'base-url "https://example.com/v1" 'api-key "test-key" 'model "glm-5.1")))
  ...
  (define model-in-settings (hash-ref (model-request-settings req) 'model #f))
