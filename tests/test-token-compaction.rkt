...
(test-case "make-token-compaction-config accepts keyword overrides"
  (define cfg
    (make-token-compaction-config #:keep-recent-tokens 5000
                                  #:reserve-tokens 2000
...
(test-case "estimate-messages-tokens sums across messages"
  (define msgs (list (make-msg-with-tokens 'user 100) (make-msg-with-tokens 'assistant 200)))
...
(test-case "estimate-messages-tokens handles mixed content"
  (define msgs (list (make-test-msg 'user "short") (make-msg-with-tokens 'assistant 50)))
  (define total (estimate-messages-tokens msgs))
  (check-true (> total 0)))
...
(test-case "backward-token-walk: all messages fit within budget"
  (define msgs (list (make-test-msg 'user "hello") (make-test-msg 'assistant "hi there")))
...
(test-case "build-token-summary-window: nothing to summarize when under budget"
  (define cfg
    (make-token-compaction-config #:keep-recent-tokens 50000
                                  #:reserve-tokens 1000
                                  #:max-context-tokens 100000))
  (define msgs (list (make-test-msg 'user "hello") (make-test-msg 'assistant "world")))
...
(test-case "build-token-summary-window: splits when over budget"
  (define cfg
    (make-token-compaction-config #:keep-recent-tokens 100
                                  #:reserve-tokens 50
...
(test-case "build-token-summary-window: reserve tokens reduce effective budget"
  (define cfg
    (make-token-compaction-config #:keep-recent-tokens 50
                                  #:reserve-tokens 450
                                  #:max-context-tokens 500))
...
(test-case "build-token-summary-window: integrates with estimate-messages-tokens"
  ...
  (define cfg
    (make-token-compaction-config #:keep-recent-tokens 200
                                  #:reserve-tokens 0
