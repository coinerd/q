...
(define-test-suite
 ...
 (test-case "register-provider! validates provider? — rejects non-provider"
   ...
   (check-exn exn:fail:contract? (lambda () (register-provider! reg "bad" 42)))
   (check-exn exn:fail:contract? (lambda () (register-provider! reg "bad" (hasheq)))))
...
(define-test-suite
 ...
 (test-case "ctx-session-messages returns messages when set"
   (define ctx
     (make-extension-ctx #:session-id "test-123"
                         #:session-dir "/tmp"
...
(run-tests (make-test-suite "Wave 4: SDK Surface Expansion Tests"
