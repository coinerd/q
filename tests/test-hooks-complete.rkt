...
(define (make-test-extension-registry . point-handler-pairs)
  ...
  (for ([pair (in-list point-handler-pairs)])
    ...
    (define ext (extension (format "test-ext-~a" point) "0.1.0" "1.0" (hasheq point handler)))
    (register-extension! reg ext))
  reg)

(define (make-hook-dispatcher reg)
  (lambda (hook-point payload) (dispatch-hooks hook-point payload reg)))
...
(define hooks-tests
  (test-suite "Hook Dispatch Points (R2-7)"
    ...
    (test-case "model-request-pre hook is dispatched before streaming"
      ...
      (define reg
        (make-test-extension-registry (list 'model-request-pre
                                            (lambda (payload)
      ...
      (check-equal? (hash-ref (unbox captured-payload) 'message-count 0)
                    1
    ...
    (test-case "model-response-post hook is dispatched after response"
      ...
      (define captured-payload (box #f))
      (define reg
        (make-test-extension-registry (list 'model-response-post
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
    ...
    (test-case "tool-call-pre hook is dispatched before tool execution"
      ...
      (define test-tool
        (make-tool "test-tool"
                   ...
                   (lambda (args exec-ctx) (make-success-result "tool output"))))
    ...
    (test-case "tool-result-post hook is dispatched after tool execution"
      ...
      (define test-tool
        (make-tool "result-tool"
                   "A test tool"
                   (hasheq 'type "object" 'properties (hasheq))
                   (lambda (args exec-ctx) (make-success-result "done"))))
    ...
    (test-case "hook-pass action allows normal execution"
      ...
      (define result
        (run-agent-turn (list (make-user-message "Hi"))
                        mock-prov
                        bus
                        #:session-id "test"
                        #:turn-id "t1"
    ...
    (test-case "hook-block action stops dispatch"
      (define reg
        (make-test-extension-registry (list 'model-request-pre
                                            (lambda (payload) (hook-block "blocked for testing")))))
      (define hook-dispatcher (make-hook-dispatcher reg))
      (define result (hook-dispatcher 'model-request-pre (hasheq)))
      (check-equal? (hook-result-action result) 'block "action is block")
      (check-equal? (hook-result-payload result) "blocked for testing" "block reason preserved"))
    ...
    (test-case "maybe-dispatch-hooks with #f registry returns payload unchanged"
      (define-values (payload result) (maybe-dispatch-hooks #f 'turn-start (hasheq 'x 1)))
    ...
    (test-case "session-before-compact hook is dispatched before compaction"
      ...
      (define captured-payload (box #f))
      (define ext-reg
        (make-test-extension-registry (list 'session-before-compact
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
    ...
    (test-case "before-agent-start hook is dispatched before first iteration"
      ...
      (define captured-payload (box #f))
      (define ext-reg
        (make-test-extension-registry (list 'before-agent-start
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
      ...
      (check-equal? (hash-ref (unbox captured-payload) 'session-id)
                    "test-session"
                    "payload should contain session-id")
      (check-equal? (hash-ref (unbox captured-payload) 'max-iterations)
                    10
                    "payload should contain max-iterations")
      (check-equal? (hash-ref (unbox captured-payload) 'context-message-count)
                    1
    ...
    (test-case "before-agent-start hook 'block prevents iteration loop"
      ...
      (define ext-reg
        (make-test-extension-registry (list 'before-agent-start
                                            (lambda (payload) (hook-block "agent start blocked")))))
      ...
      (check-equal? (hash-ref (loop-result-metadata result) 'reason)
                    "extension-block"
                    "metadata should contain extension-block reason")
      ;; agent.blocked event emitted
      (define names (map event-event (unbox evts)))
      (check-not-false (member "agent.blocked" names) "agent.blocked event should be emitted")
      (delete-directory/files tmpdir #:must-exist? #f))
    ...
    (test-case "message-start hook is dispatched before streaming"
      ...
      (define captured-payload (box #f))
      (define reg
        (make-test-extension-registry (list 'message-start
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
    ...
    (test-case "message-start hook block aborts turn"
      ...
      (define reg
        (make-test-extension-registry (list 'message-start
                                            (lambda (payload) (hook-block "blocked by test")))))
      (define hook-dispatcher (make-hook-dispatcher reg))
      (define content-parts (list (hasheq 'type "text" 'text "Response")))
      (define mock-prov (make-mock-provider (make-model-response content-parts (hash) "mock" #f)))
      (define result
      ...
      (check-equal? (hash-ref (loop-result-metadata result) 'hook #f)
                    'message-start
    ...
    (test-case "message-update hook receives each text delta"
         ...
         (list 'message-update
               (lambda (payload)
                 (when (hash-ref payload 'delta-text #f)
                   (set-box! deltas (append (unbox deltas) (list (hash-ref payload 'delta-text)))))
                 (hook-pass payload)))))
      (define hook-dispatcher (make-hook-dispatcher reg))
      ...
      (check-equal? (unbox deltas) '("Hello world") "message-update should receive text delta"))

    (test-case "message-update hook block stops streaming"
      (define bus (make-event-bus))
      (define call-count (box 0))
      (define reg
        (make-test-extension-registry (list 'message-update
                                            (lambda (payload)
    ...
    (test-case "message-end hook is dispatched after response"
      ...
      (define captured-payload (box #f))
      (define reg
        (make-test-extension-registry (list 'message-end
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
      ...
      (check-equal? (hash-ref (unbox captured-payload) 'content #f)
                    "Response text"
    ...
    (test-case "message-end hook amend modifies content"
      ...
      (define reg
        (make-test-extension-registry
         (list 'message-end
               (lambda (payload) (hook-amend (hash-set payload 'content "AMENDED CONTENT"))))))
      (define hook-dispatcher (make-hook-dispatcher reg))
    ...
    (test-case "message-end hook block returns hook-blocked"
      ...
      (define reg
        (make-test-extension-registry (list 'message-end
                                            (lambda (payload) (hook-block "suppressed by test")))))
      (define hook-dispatcher (make-hook-dispatcher reg))
      (define content-parts (list (hasheq 'type "text" 'text "Response text")))
      (define mock-prov (make-mock-provider (make-model-response content-parts (hash) "mock" #f)))
      (define result
    ...
    (test-case "resources-discover hook receives base-dir"
      ...
      (define captured-payload (box #f))
      (define reg
        (make-test-extension-registry (list 'resources-discover
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
    ...
    (test-case "resources-discover hook amend provides custom resources"
      ...
      (define reg
        (make-test-extension-registry (list 'resources-discover
                                            (lambda (payload) (hook-amend custom-resources)))))
      (define tmp-dir (make-temporary-file "q-test-resources-~a" 'directory))
      (define q-dir (build-path tmp-dir ".q"))
      (make-directory* q-dir)
      (define result (load-global-resources tmp-dir #:hook-dispatcher (make-hook-dispatcher reg)))
    ...
    (test-case "session-before-switch hook dispatched on resume"
      ...
      (define captured-payload (box #f))
      (define ext-reg
        (make-test-extension-registry (list 'session-before-switch
                                            (lambda (payload)
                                              (set-box! hook-called? #t)
                                              (set-box! captured-payload payload)
                                              (hook-pass payload)))))
      ...
      (let ([switch-hash (payload->hash (unbox captured-payload))])
        (check-equal? (hash-ref switch-hash 'session-id)
                      (session-id sess)
                      "payload should contain session-id")
        (check-equal? (hash-ref switch-hash 'operation)
                      'resume
    ...
    (test-case "handler returning void does not crash dispatch-hooks"
      (define reg (make-test-extension-registry (list 'model-request-pre (lambda (payload) (void)))))
    ...
    (test-case "handler returning string does not crash dispatch-hooks"
      (define reg (make-test-extension-registry (list 'model-request-pre (lambda (payload) "bad"))))
      (define result (dispatch-hooks 'model-request-pre (hasheq 'x 1) reg))
    ...
    (test-case "handler returning #f does not crash dispatch-hooks"
      (define reg (make-test-extension-registry (list 'model-request-pre (lambda (payload) #f))))
      (define result (dispatch-hooks 'model-request-pre (hasheq 'x 1) reg))
    ...
    (test-case "handler returning proper hook-result still works after fix"
      (define reg
        (make-test-extension-registry
         (list 'model-request-pre (lambda (payload) (hook-amend (hash-set payload 'ok #t))))))
      (define result (dispatch-hooks 'model-request-pre (hasheq 'x 1) reg))
      ...
      (check-equal? (hash-ref (hook-result-payload result) 'ok)
                    #t
                    "payload should contain amendment"))))
