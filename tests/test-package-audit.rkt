...
(define make-tmp-dir
  (with-handlers ([exn:fail? (lambda (_) (lambda (fmt) (make-temporary-file fmt 'directory)))])
...
(test-case "low-risk skill package"
  ...
  (dynamic-wind void
                (lambda ()
                  ...
                  (write-rkt-file! dir "helper.rkt" "#lang racket/base\n(define (hello) \"world\")\n")
                  (define result (audit-package dir))
                  (check-equal? (audit-result-risk-level result) 'low)
                  (check-pred null?
                              (filter (lambda (f) (eq? (finding-severity f) 'high))
...
(test-case "subprocess patterns produce high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     ...
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1) (format "expected >=1 high finding, got ~a" highs))
...
(test-case "network patterns produce high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (write-qpm-json! dir #:type "extension")
     (write-rkt-file! dir "net.rkt" "#lang racket/base\n(require net/http)\n")
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "network")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "file-write patterns produce medium finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     ...
     (define result (audit-package dir))
     (define meds
       (filter (lambda (f) (eq? (finding-severity f) 'medium)) (audit-result-findings result)))
     (check-true (>= (length meds) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "file write")) meds)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "format-audit-report returns non-empty string"
  (define result (audit-result 'high (list (finding 'high "subprocess execution")) #t))
...
(test-case "missing manifest handled gracefully"
  ...
  (dynamic-wind void
                (lambda ()
                  ...
                  (define result (audit-package dir))
                  (check-equal? (audit-result-risk-level result) 'medium)
                  (check-true (ormap (lambda (f) (string-contains? (finding-message f) "manifest"))
                                     (audit-result-findings result))))
                (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "entry point in manifest adds finding"
  ...
  (dynamic-wind void
                (lambda ()
                  ...
                  (write-rkt-file! dir "main.rkt" "#lang racket/base\n")
                  (define result (audit-package dir))
                  (check-true (ormap (lambda (f) (string-contains? (finding-message f) "entry"))
                                     (audit-result-findings result))))
                (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "eval pattern produces high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (write-qpm-json! dir #:type "extension")
     (write-rkt-file! dir "evl.rkt" "#lang racket/base\n(eval \"(+ 1 2)\")\n")
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "eval/compile")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "dynamic-require pattern produces high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (write-qpm-json! dir #:type "extension")
     (write-rkt-file! dir "dyn.rkt" "#lang racket/base\n(dynamic-require 'racket/list 'first)\n")
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "dynamic-require")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "ffi pattern produces high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (write-qpm-json! dir #:type "extension")
     (write-rkt-file! dir "ffi.rkt" "#lang racket/base\n(require ffi/unsafe)\n")
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "FFI")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
...
(test-case "env-modification pattern produces high finding"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (write-qpm-json! dir #:type "extension")
     (write-rkt-file! dir "env.rkt" "#lang racket/base\n(putenv \"FOO\" \"bar\")\n")
     (define result (audit-package dir))
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) "environment")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
  ...
  (dynamic-wind
   ...
   (lambda ()
     ...
     (define highs
       (filter (lambda (f) (eq? (finding-severity f) 'high)) (audit-result-findings result)))
     (check-true (>= (length highs) 1))
     (check-true (ormap (lambda (f) (string-contains? (finding-message f) ".zo")) highs)))
   (lambda () (delete-directory/files dir #:must-exist? #f))))
