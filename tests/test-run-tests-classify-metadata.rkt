...
(define (finding-kinds result)
  (map (lambda (f) (hash-ref f 'kind))
       (hash-ref result 'findings)))

(define (finding-codes result)
  (map (lambda (f) (hash-ref f 'code))
       (hash-ref result 'findings)))

(define (codes-of-kind result kind)
  (filter values
          (map (lambda (f)
                 (and (eq? (hash-ref f 'kind) kind)
                      (hash-ref f 'code)))
               (hash-ref result 'findings))))
...
(test-equal? "canonical-isolation: process unchanged"
             (meta:canonical-isolation "process")
             "process")

(test-not-false "deprecated-isolation-alias?: subprocess"
           (meta:deprecated-isolation-alias? "subprocess"))

(test-false "deprecated-isolation-alias?: process"
            (meta:deprecated-isolation-alias? "process"))
...
(test-equal? "valid file: explicit classification"
             (hash-ref valid-result 'classification)
             'explicit)

(test-equal? "valid file: no error findings"
             (filter (lambda (f) (eq? f 'error))
                     (finding-kinds valid-result))
             '())

(test-equal? "valid file: no warning findings"
             (filter (lambda (f) (eq? f 'warning))
                     (finding-kinds valid-result))
             '())
...
(define bad-suite-file
  (write-sample "bad-suite.rkt" (with-header "@suite nonsense" "@speed fast")))
(test-not-false "invalid suite value is an error"
           (member 'invalid-suite (codes-of-kind (meta:validate-file bad-suite-file) 'error)))

(define bad-speed-file
  (write-sample "bad-speed.rkt" (with-header "@suite unit" "@speed turbo")))
...
(define bad-boundary-file
  (write-sample "bad-boundary.rkt"
                (with-header "@suite unit" "@speed fast" "@boundary outer-space")))
(test-not-false "invalid boundary value is an error"
           (member 'invalid-boundary (codes-of-kind (meta:validate-file bad-boundary-file) 'error)))

(define bad-mutates-file
  (write-sample "bad-mutates.rkt"
                (with-header "@suite unit" "@speed fast" "@mutates everything")))
(test-not-false "invalid mutates value is an error"
           (member 'invalid-mutates (codes-of-kind (meta:validate-file bad-mutates-file) 'error)))

(define bad-isolation-file
  (write-sample "bad-isolation.rkt"
                (with-header "@suite unit" "@speed fast" "@isolation container")))
(test-not-false "invalid isolation value is an error"
           (member 'invalid-isolation (codes-of-kind (meta:validate-file bad-isolation-file) 'error)))
...
(define bad-timeout-file
  (write-sample "bad-timeout.rkt"
                (with-header "@suite unit" "@speed fast" "@timeout soon")))
(test-not-false "malformed timeout is an error"
           (member 'malformed-timeout (codes-of-kind (meta:validate-file bad-timeout-file) 'error)))
...
(define alias-file
  (write-sample "alias.rkt"
                (with-header "@suite unit" "@speed fast" "@isolation subprocess")))
...
(test-equal? "header file: explicit classification"
             (hash-ref valid-result 'classification)
             'explicit)
...
(define reserved-file
  (write-sample "reserved.rkt"
                (with-header "@suite unit" "@speed fast" "@covers issue-1234")))
...
(test-not-false "summary counts invalid findings"
           (> (hash-ref summary 'invalid_count) 0))
...
(test-not-false "summary tracks per-area breakdown"
           (> (hash-count (hash-ref summary 'per_area)) 0))
