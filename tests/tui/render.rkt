  ...
  (test-suite
   ...
   (test-case "render-branch-list returns header + 2 branches"
     (let ([branches (list (branch-info "b1" #f 'user #t #t)
       ...
       (check-true (string-contains? (styled-segment-text (first (styled-line-segments (first lines))))
                                     "Branches (2)")
                   "header shows count")))
   ...
   (test-case "render-branch-list active branch has arrow marker"
     (let ([branches (list (branch-info "active-branch" #f 'user #t #t))])
       ...
       (check-true (string-contains? line-text "→")
                   "active branch has arrow marker")
       (check-true (string-contains? line-text "active-branch")
                   "branch id is shown")))
   ...
   (test-case "render-children-list returns header + 2 children"
     (let ([children (list (branch-info "child1" "parent" 'user #t #f)
       ...
       (check-true (string-contains? header-text "Children of parent")
                   "header shows parent id")))
   ...
   (test-case "render-children-list empty shows no-children message"
     (let ([lines (render-children-list "parent" '() 80)])
       ...
       (check-true (string-contains? msg-text "no children")
                   "shows no children message")))
   ...
   (test-case "format-entry error message has bold red style"
     (let ([entry (transcript-entry 'error "Oops" 1000 (hash))])
       (define lines (format-entry entry 80))
       (check-equal? (styled-segment-style (first (styled-line-segments (first lines))))
                     '(bold red)
                     "error entry has bold red style")))
   ...
   (test-case "format-entry tool-start is cyan"
     (let ([entry (transcript-entry 'tool-start "[tool: read]" 1000 (hash))])
       (define lines (format-entry entry 80))
       (check-equal? (styled-segment-style (first (styled-line-segments (first lines))))
                     '(cyan))))

   (test-case "format-entry tool-end is green"
     (let ([entry (transcript-entry 'tool-end "[tool done: read]" 1000 (hash))])
       (define lines (format-entry entry 80))
       (check-equal? (styled-segment-style (first (styled-line-segments (first lines))))
                     '(green))))

   (test-case "format-entry tool-fail is red"
     (let ([entry (transcript-entry 'tool-fail "[tool failed: read]" 1000 (hash))])
       (define lines (format-entry entry 80))
       (check-equal? (styled-segment-style (first (styled-line-segments (first lines))))
                     '(red))))
   ...
   (test-case "render-transcript: more lines than height shows last N"
     (let* ([entries (list (transcript-entry 'system "line1" 0 (hash))
       ...
       ;; First visible should be "line2", second "line3"
       (check-equal? (styled-segment-text (first (styled-line-segments (first lines))))
                     "line2")
   ...
   (test-case "render-transcript: scroll-offset=1 shows lines offset 1 from bottom"
     (let* ([entries (list (transcript-entry 'system "line1" 0 (hash))
            ...
            [state (struct-copy ui-state (initial-ui-state)
                                [transcript entries]
                                [scroll-offset 1])])
       (define lines (render-transcript state 2 200))
       (check-equal? (length lines) 2 "render-transcript: scroll=1 shows 2 lines")
       ;; Should show line2 and line3 (offset 1 from bottom)
       (check-equal? (styled-segment-text (first (styled-line-segments (first lines))))
                     "line2")
       (check-equal? (styled-segment-text (first (styled-line-segments (second lines))))
                     "line3")))
   ...
   (test-case "render-transcript: scroll-offset=2 shows older lines"
     (let* ([entries (list (transcript-entry 'system "line1" 0 (hash))
       ...
       ;; Should show line1, line2, line3 (last 5 - 3 - 2 = start at 0)
       (check-equal? (styled-segment-text (first (styled-line-segments (first lines))))
                     "line1")))
...
(define selection-tests
  (test-suite
   ...
   (test-case "apply-selection-highlight single line selection"
     (define lines (list (styled-line (list (styled-segment "line0" '())))
     ...
     (check-equal? (styled-line-segments (first result))
                   (styled-line-segments (first lines)))
     (check-not-false (member 'inverse (styled-segment-style
                                    (first (styled-line-segments (second result))))))
     (check-equal? (styled-line-segments (third result))
                   (styled-line-segments (third lines))))

   (test-case "apply-selection-highlight multi-line selection"
     (define lines (list (styled-line (list (styled-segment "aa" '())))
     ...
     (for ([line result] [i (in-naturals)])
       (define segs (styled-line-segments line))
       (define has-inverse? (for/or ([s segs]) (member 'inverse (styled-segment-style s))))
       (check-not-false has-inverse?
                   (format "line ~a should have inverted segment" i))))
   ))
...
(define highlight-multi-seg-tests
  (test-suite
   "highlight-line-range multi-segment"
   ...
   (test-case "highlight-line-range multi-segment full-line highlight"
     ...
     (define sl (styled-line (list (styled-segment "hi" '())
                                   (styled-segment " there" '()))))
   ...
   (test-case "highlight-line-range single char in second segment"
     ...
     (define sl (styled-line (list (styled-segment "ab" '())
                                   (styled-segment "cd" '()))))
   ...
   (test-case "apply-selection-highlight with multi-segment line"
     ...
     (define lines (list (styled-line (list (styled-segment "> " '(bold cyan))
...
(define wrapping-tests
  (test-suite
   "word-wrapping for assistant markdown"
   ...
   (test-case "wrap-styled-line preserves multi-segment styles"
     ...
     (define sl (styled-line (list (styled-segment (make-string 50 #\b) '(bold))
   ...
   (test-case "md-format-assistant does NOT wrap code blocks"
     ...
     (for ([line result])
       ...
       (check-not-false
        (member 'green
                (apply append (map styled-segment-style (styled-line-segments line))))
   ...
   (test-case "md-format-assistant wraps markdown with bold and code inline"
     ...
     (define text "**bold text** normal text and more text that continues past forty columns of width")
