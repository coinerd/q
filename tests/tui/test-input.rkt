...
(define input-tests
  (test-suite "TUI Input"
    ...
    (test-case "submit returns text and resets state"
      (let ()
        (define-values (text st2)
          (input-submit
           (struct-copy input-state (initial-input-state) [buffer "hello world"] [cursor 11])))
    ...
    (test-case "submit on whitespace-only buffer returns #f"
      (let ()
        (define-values (text st2)
          (input-submit (struct-copy input-state (initial-input-state) [buffer "   "] [cursor 3])))
        (check-false text)))
    ...
    (test-case "submit trims whitespace from text"
      (let ()
        (define-values (text st2)
          (input-submit
           (struct-copy input-state (initial-input-state) [buffer "  hello  "] [cursor 9])))
    ...
    (test-case "input-current-text returns buffer contents"
      (check-equal?
       (input-current-text (struct-copy input-state (initial-input-state) [buffer "abc"]))
       "abc"))
    ...
    (test-case "input-at-end? checks cursor at buffer length"
      ...
      (check-true (input-at-end?
                   (struct-copy input-state (initial-input-state) [buffer "ab"] [cursor 2])))
      (check-false (input-at-end?
                    (struct-copy input-state (initial-input-state) [buffer "ab"] [cursor 1]))))
    ...
    (test-case "input-insert-newline: inserts newline at cursor position"
      (let* ([st (struct-copy input-state (initial-input-state) [buffer "hello"] [cursor 2])]
    ...
    (test-case "input-insert-newline: clears history-idx"
      (let* ([st0 (input-history-push (initial-input-state) "old")]
        ...
        (check-false (input-state-history-idx st2) "insert-newline clears history browsing")))
    ...
    (test-case "input-visible-window: CJK text scrolls"
      ...
      (check-true (and (>= cursor-col INPUT-PROMPT-WIDTH) (< cursor-col 12))
