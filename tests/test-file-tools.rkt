  ...
  (test-suite "read-tool"
    ...
    (test-case "read with offset and limit"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-read (hasheq 'path (path->string f) 'offset 2 'limit 3)))
                       (check-false (result-is-error? r))
  ...
  (test-suite "write-tool"
    ...
    (test-case "write creates parent directories"
      (with-temp-dir (λ (dir)
                       ...
                       (define r
                         (tool-write (hasheq 'path (path->string f) 'content "nested content")))
                       (check-false (result-is-error? r))
    ...
    (test-case "write overwrites existing file"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-write (hasheq 'path (path->string f) 'content "new content")))
                       (check-false (result-is-error? r))
                       (check-equal? (file->string f) "new content"))))

    (test-case "write to read-only location returns error"
      (define r (tool-write (hasheq 'path "/dev/null/impossible/path.txt" 'content "fail")))
      (check-true (result-is-error? r)))

    (test-case "write empty string creates empty file"
      (with-temp-dir (λ (dir)
                       (define f (temp-file-path dir "empty.txt"))
                       (define r (tool-write (hasheq 'path (path->string f) 'content "")))
                       (check-false (result-is-error? r))
    ...
    (test-case "write returns path in details"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-write (hasheq 'path (path->string f) 'content "x")))
                       (check-false (result-is-error? r))
                       (check-equal? (hash-ref (result-details r) 'path) (path->string f)))))))
  ...
  (test-suite "edit-tool"
    ...
    (test-case "edit replaces exact match"
      (with-temp-dir (λ (dir)
                       ...
                       (define r
                         (tool-edit (hasheq 'path (path->string f) 'old-text "bar" 'new-text "QUX")))
                       (check-false (result-is-error? r))
    ...
    (test-case "edit with old-text not found returns error"
      (with-temp-dir (λ (dir)
                       ...
                       (define r
                         (tool-edit (hasheq 'path (path->string f) 'old-text "xyz" 'new-text "abc")))
                       (check-true (result-is-error? r))
                       (define c (string-join (result-content r) ""))
                       (check-true (regexp-match? #rx"not found" c))
    ...
    (test-case "edit with ambiguous match returns error with count"
      (with-temp-dir (λ (dir)
                       ...
                       (define r
                         (tool-edit (hasheq 'path (path->string f) 'old-text "aaa" 'new-text "XXX")))
    ...
    (test-case "edit with multi-line old-text"
      (with-temp-dir
       (λ (dir)
         ...
         (define r
           (tool-edit (hasheq 'path (path->string f) 'old-text "line2\nline3" 'new-text "REPLACED")))
         (check-false (result-is-error? r))
         (check-equal? (file->string f) "line1\nREPLACED\nline4"))))

    (test-case "edit on non-existent file returns error"
      (define r
        (tool-edit (hasheq 'path "/tmp/q-no-such-file-edit-xyz.txt" 'old-text "x" 'new-text "y")))
      (check-true (result-is-error? r))
      (define c (string-join (result-content r) ""))
      (check-true (regexp-match? #rx"not found" c)))
    ...
    (test-case "edit returns diff info in details"
      (with-temp-dir
       (λ (dir)
         ...
         (define r
           (tool-edit (hasheq 'path (path->string f) 'old-text "quick brown" 'new-text "slow white")))
         (check-false (result-is-error? r))
    ...
    (test-case "edit with old-text equal to new-text is still valid"
      (with-temp-dir
       (λ (dir)
         ...
         (define r
           (tool-edit (hasheq 'path (path->string f) 'old-text "unchanged" 'new-text "unchanged")))
         (check-false (result-is-error? r))
    ...
    (test-case "edit empty file with empty old-text is ambiguous/error"
      (with-temp-dir (λ (dir)
                       ...
                       (define r
                         (tool-edit (hasheq 'path (path->string f) 'old-text "" 'new-text "x")))
                       (check-true (result-is-error? r)))))))
...
(run-tests (test-suite "file-tools"
             read-tests
             write-tests
             edit-tests))
