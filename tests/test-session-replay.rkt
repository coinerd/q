...
(define (make-simple-session)
  ...
  (list (make-message "m1" #f 'user 'message (list (make-text-part "list files")) 1000 (hasheq))
        (make-message "m2"
                      "m1"
                      'assistant
                      'message
                      (list (make-tool-call-part "tc1" "bash" "ls -la"))
                      1001
                      (hasheq))
        (make-message "m3"
                      "m2"
...
(define (make-unmatched-call-session)
  ...
  (list (make-message "m1" #f 'user 'message (list (make-text-part "do something")) 1000 (hasheq))
        (make-message "m2"
                      "m1"
                      'assistant
                      'message
                      (list (make-tool-call-part "tc-orphan" "bash" "rm -rf /"))
                      1001
                      (hasheq))))

(define (make-orphan-result-session)
  ;; Tool result referencing a non-existent tool-call-id → drift
  (list (make-message "m1" #f 'user 'message (list (make-text-part "hello")) 1000 (hasheq))
        (make-message "m2"
                      "m1"
                      'tool
                      'message
                      (list (make-tool-result-part "ghost-tc" "some output" #f))
                      1001
                      (hasheq))))
...
(define (make-mixed-session)
  ...
  (list (make-message "m1" #f 'user 'message (list (make-text-part "do stuff")) 1000 (hasheq))
        ;; Matched read call
        (make-message "m2"
                      "m1"
                      'assistant
                      'message
                      (list (make-tool-call-part "tc-read-1" "read" "/etc/hosts"))
                      1001
                      (hasheq))
        (make-message "m3"
                      "m2"
                      'tool
                      'message
                      (list (make-tool-result-part "tc-read-1" "127.0.0.1 localhost" #f))
                      1002
                      (hasheq))
        ;; Matched bash call
        (make-message "m4"
                      ...
                      'assistant
                      'message
                      (list (make-tool-call-part "tc-bash-1" "bash" "date"))
                      1003
                      (hasheq))
        (make-message "m5"
                      "m4"
                      'tool
                      'message
                      (list (make-tool-result-part "tc-bash-1" "Fri Apr 10 12:00:00 UTC 2026" #f))
                      1004
                      (hasheq))
        ...
        (make-message "m6"
                      ...
                      'assistant
                      'message
                      (list (make-tool-call-part "tc-unmatched" "bash" "ls /tmp"))
                      1005
                      (hasheq))
        ...
        (make-message "m7"
                      ...
                      'tool
                      'message
                      (list (make-tool-result-part "tc-ghost" "phantom data" #f))
                      1006
                      (hasheq))))

(define (make-multi-call-session)
  ;; 3 matched tool calls across multiple messages
  (list (make-message "m1" #f 'user 'message (list (make-text-part "go")) 1000 (hasheq))
        (make-message "m2"
                      "m1"
                      'assistant
                      'message
                      (list (make-tool-call-part "tc1" "read" "a.txt")
                            (make-tool-call-part "tc2" "bash" "ls"))
                      1001
                      (hasheq))
        (make-message "m3"
                      "m2"
                      'tool
                      'message
                      (list (make-tool-result-part "tc1" "content of a" #f))
                      1002
                      (hasheq))
        (make-message "m4"
                      "m3"
        ...
        (make-message "m5"
                      ...
                      'assistant
                      'message
                      (list (make-tool-call-part "tc3" "edit" "fix typo"))
                      1004
                      (hasheq))
        (make-message "m6"
                      "m5"
                      'tool
                      'message
                      (list (make-tool-result-part "tc3" "ok" #f))
                      1005
                      (hasheq))))
...
(define-test-suite
 ...
 (test-case "safe wrapper: missing file returns error hash"
   ...
   (check-not-false (hash-ref report 'error #f) "error key present for missing file")
   (check-equal? (hash-ref report 'total-entries) 0)
 ...
 (test-case "safe wrapper: corrupted file returns partial results"
   ...
   (call-with-output-file path
                          (lambda (out) (write-string "NOT VALID JSON\n" out))
                          #:mode 'text
                          #:exists 'replace)
     ...
     (list (make-message "m1" #f 'user 'message (list (make-text-part "go")) 1000 (hasheq))
           ...
           (make-message "m2"
                         ...
                         (list (make-tool-call-part "tc-a" "read" "file1")
                               (make-tool-call-part "tc-b" "read" "file2"))
                         1001
                         (hasheq))
           ...
           (make-message "m3"
                         ...
                         'tool
                         'message
                         (list (make-tool-result-part "tc-a" "content1" #f))
                         1002
                         (hasheq))
           (make-message "m4"
                         "m2"
                         'tool
                         'message
                         (list (make-tool-result-part "tc-b" "content2" #f))
                         1003
                         (hasheq))))
   (write-session! path msgs)
