...
(define cancellation-tests
  (test-suite "Agent Loop Cancellation Tests"
    ...
    (test-case "A1: cancellation after first text delta emits turn.cancelled"
      (define events (box '()))
    ...
    (test-case "A2: message-update hook block during tool-call delta stops stream"
      (define events (box '()))
      (define bus (make-event-bus))
      (subscribe! bus (lambda (e) (set-box! events (cons e (unbox events)))))
      ...
      (define (blocking-hook-dispatcher hook-type data)
        (if (and (eq? hook-type 'message-update) (hash-ref data 'delta-tool-call #f))
            (hook-block)
            (hook-pass)))
    ...
    (test-case "A3: cancellation at stream end triggers handle-cancellation"
      (define events (box '()))
      (define bus (make-event-bus))
      (subscribe! bus (lambda (e) (set-box! events (cons e (unbox events)))))
      (define cancel-tok (make-cancellation-token))
      ...
      (define prov
        (make-provider (lambda () "cancel-end-mock")
                       (lambda () (hash 'streaming #t 'token-counting #t))
                       (lambda (req) (make-model-response '() (hasheq) "mock" 'stop))
                       (lambda (req)
