...
(let* ([msg (hasheq 'role "user" 'content "test")]
  ...
  (check-equal? (model-request-messages req2)
                (list msg)
                "model-request roundtrip: messages preserved")
  (check-not-false (model-request-tools req2) "model-request roundtrip: tools preserved")
  (check-equal? (hash-ref (model-request-settings req2) 'model)
                "gpt-4"
...
(let* ([req (make-model-request '() #f (hasheq))]
       [js (model-request->jsexpr req)]
       [req2 (jsexpr->model-request js)])
  (check-false (model-request-tools req2) "model-request roundtrip: tools #f preserved"))
...
(let* ([content (list (hasheq 'type "text" 'text "world"))]
  ...
  (check-equal? (model-response-content resp2) content "model-response roundtrip: content preserved")
  (check-equal? (model-response-model resp2) "gpt-4" "model-response roundtrip: model preserved")
  (check-equal? (model-response-stop-reason resp2)
                'stop
...
(let ([chunk
  ...
  (check-equal? (stream-chunk-finish-reason chunk) 'stop "stream-chunk: finish-reason keyword"))
