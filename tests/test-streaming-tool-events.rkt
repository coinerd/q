...
(define streaming-tool-event-tests
  (test-suite "Agent Loop Streaming → Tool Event Ordering"
    ...
    (test-case "text+tool response: assistant.message.completed before tool.call.started"
      (define bus (make-event-bus))
      (define captured (box '()))

      (subscribe! bus
                  (lambda (evt) (set-box! captured (append (unbox captured) (list (event-ev evt))))))
      ...
      (define ctx
        (list
         (make-message "m-1" #f 'user 'message (list (make-text-part "read foo")) 1000 '#hash())))
      ...
      (define result
        (run-agent-turn
         ...
         #:turn-id "t1"
         #:tools
         (list (hasheq 'type "function" 'function (hasheq 'name "read" 'parameters (hasheq))))))
    ...
    (test-case "text-only response: no tool events emitted"
      (define bus (make-event-bus))
      (define captured (box '()))
      ...
      (define events (unbox captured))
      (check-false (member "tool.call.started" events) "no tool.call.started for text-only response")
    ...
    (test-case "multiple tool calls: all tool.call.started after assistant.message.completed"
      (define bus (make-event-bus))
      (define captured (box '()))
           ...
           (list (make-stream-chunk "Need info" #f #f #f)
                 (make-stream-chunk #f
                                    (hasheq 'id
                                            ...
                                            0
                                            'function
                                            (hasheq 'name "read" 'arguments "{\"path\":\"a\"}"))
                                    #f
                                    #f)
                 (make-stream-chunk #f
                                    (hasheq 'id
    ...
    (test-case "tool-call-only response: still emits assistant.message.completed"
      (define bus (make-event-bus))
      (define captured (box '()))
      ...
      (define result
        (run-agent-turn
         ...
         #:turn-id "t1"
         #:tools
         (list (hasheq 'type "function" 'function (hasheq 'name "bash" 'parameters (hasheq))))))
    ...
    (test-case "event payloads contain correct data"
      (define bus (make-event-bus))
      (define captured-events (box '()))

      (subscribe! bus
                  (lambda (evt)
                    (set-box! captured-events (append (unbox captured-events) (list evt)))))
      ...
      (define amc-event
        (for/first ([e events]
                    #:when (equal? (event-ev e) "assistant.message.completed"))
          e))
      (define tcs-event
        (for/first ([e events]
                    #:when (equal? (event-ev e) "tool.call.started"))
          e))

      ;; Check assistant.message.completed payload
      (check-not-false amc-event "assistant.message.completed event found")
      (check-equal? (hash-ref (event-payload amc-event) 'content)
                    "Answer"
