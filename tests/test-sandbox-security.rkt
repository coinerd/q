...
(define-test-suite
 ...
 (test-case "read tool rejects non-existent path traversal"
   ;; tool-read should return an error result for non-existent traversal target.
   ;; Use a path that definitely doesn't exist from any cwd.
 ...
 (test-case "read tool works on real file, rejects sibling traversal"
   ...
   (check-pred tool-result? good)
   (check-false (tool-result-is-error? good))
   ;; Traversing above it to non-existent path fails
   (define bad (tool-read (hasheq 'path (path->string (build-path tmp-dir ".." "no-such-file-xyz")))))
   (check-pred tool-result? bad)
   (check-pred tool-result-is-error? bad)
   ;; Cleanup
   (delete-directory/files tmp-dir #:must-exist? #f))
...
(define-test-suite
 ...
 (test-case "cancellation callback fires exactly once per cancel-token! call"
   (define count (box 0))
   (define tok
     (make-cancellation-token #:callback (lambda (_) (set-box! count (add1 (unbox count))))))
   (cancel-token! tok)
   (check-equal? (unbox count) 1))
 (test-case "cancellation callback receives the token"
   (define received (box #f))
   (define tok (make-cancellation-token #:callback (lambda (t) (set-box! received t))))
   (cancel-token! tok)
 ...
 (test-case "multiple independent tokens do not cross-contaminate"
   ...
   (cancel-token! tok-a)
   (check-pred cancellation-token-cancelled? tok-a)
...
(define-test-suite
 ...
 (test-case "with-resource-limits returns result for fast thunk"
   (define-values (result timed-out?) (with-resource-limits (lambda (cust) 42) #:timeout 5))
 ...
 (test-case "bash tool timeout propagates to subprocess"
   (define result (tool-bash (hasheq 'command "sleep 60" 'timeout 1)))
   (check-pred tool-result? result)
...
(define-test-suite
 sandbox-evaluator-security-tests
 (test-case "sandbox blocks filesystem write"
   ...
   (check-pred eval-result? result)
   (check-not-false (eval-result-error result) "sandbox should block file writes"))
 (test-case "sandbox blocks subprocess creation"
   (define result (eval-in-sandbox "(require racket/system) (system \"echo pwned\")"))
   (check-pred eval-result? result)
   (check-not-false (eval-result-error result) "sandbox should block system calls"))
 ...
 (test-case "sandbox allows pure computation"
   (define result
     (eval-in-sandbox "(define (fib n) (if (< n 2) n (+ (fib (- n 1)) (fib (- n 2))))) (fib 10)"))
   (check-false (eval-result-error result))
 ...
 (test-case "sandbox memory limit terminates allocation bomb"
   ...
   (define result
     (eval-in-sandbox "(define xs (make-list 10000000 (make-string 1000 #\\x))) (length xs)"
                      #:timeout 10))
   (check-pred eval-result? result)
   ;; Either error (memory exceeded) or timeout — either way, it terminates
   (check-true (or (eval-result-error result) (< (eval-result-elapsed-ms result) 15000)))))
