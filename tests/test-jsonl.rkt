  ...
  (test-suite "jsonl tests"
      ...
      (with-temp-file (lambda (path)
                        ...
                        (call-with-output-file path
                                               (lambda (out)
                                                 ...
                                                 (write-string "BROKEN\n" out))
                                               #:exists 'replace)
                        (check-exn exn:fail? (lambda () (jsonl-read-all path))))))
    ...
    (test-case "roundtrip: write and read preserves all types"
      (with-temp-file
       (lambda (path)
         (define entry
           (hasheq 'string "hello" 'number 42 'float 3.14 'bool #t 'null 'null 'list '(1 2 3)))
