...
(define-test-suite
 ...
 (test-case "sessions-list #:sort 'by-size sorts by size"
   ...
   (check-equal? (length result) 2)
   (check-true (>= (hash-ref (first result) 'size-bytes) (hash-ref (second result) 'size-bytes)))
   (delete-directory/files dir))
 ;; ============================================================
 ...
 (test-case "sessions-info counts branches (entries with parentId)"
   ...
   (define jsonl-path (build-path session-dir "session.jsonl"))
   (jsonl-append! jsonl-path (hasheq 'role "user" 'content "hi"))
   (jsonl-append! jsonl-path (hasheq 'role "assistant" 'content "hello" 'parentId "entry-1"))
 ...
 (test-case "sessions-delete with confirm?: 'y' deletes"
   ...
   (check-equal? (sessions-delete dir "confirm-del" #:confirm? #t #:in in #:out out) 'ok)
 ...
 (test-case "sessions-delete with confirm?: 'n' returns 'cancelled"
   ...
   (define out (open-output-string))
   (check-equal? (sessions-delete dir "confirm-keep" #:confirm? #t #:in in #:out out) 'cancelled)
 ...
 (test-case "sessions-delete with confirm?: empty input returns 'cancelled"
   ...
   (define out (open-output-string))
   (check-equal? (sessions-delete dir "confirm-empty" #:confirm? #t #:in in #:out out) 'cancelled)
   (delete-directory/files dir))
 ;; ============================================================
