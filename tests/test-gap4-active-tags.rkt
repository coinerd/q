...
(require rackunit
         ...
         (only-in "../util/message/message.rkt" make-message message-meta-safe))

;; Helper: create a WS message with file path in metadata
(define (make-ws-msg path-str)
  (make-message #f 'assistant 'user 'user (list) (current-seconds) (hasheq 'path path-str)))
...
(define-test-suite
 ...
 (test-case "GAP-4: rank-and-budget boosts conclusions with matching tags"
   ...
   (check equal? (task-conclusion-text (car ranked)) "Fixed bug in authentication module"))
 ...
 (test-case "GAP-4: message-meta-safe returns hash from message with meta"
   (define msg
     (make-message #f 'assistant 'user 'user (list) (current-seconds) (hasheq 'path "/src/foo.rkt")))
