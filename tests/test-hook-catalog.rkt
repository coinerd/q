...
(define-test-suite session-lifecycle-tests
                   ...
                   (test-case "session-before-switch allows pass and block"
                     (check-equal? (valid-hook-actions-for 'session-before-switch) '(pass block)))
...
(define-test-suite
 ...
 (test-case "tool.execution.started is in hook-action-schemas"
   (check-true (hash-has-key? hook-action-schemas 'tool.execution.started)))
 (test-case "tool.execution.completed is in hook-action-schemas"
   (check-true (hash-has-key? hook-action-schemas 'tool.execution.completed)))
 ...
 (test-case "tool.execution.started only allows 'pass action"
   (check-equal? (valid-hook-actions-for 'tool.execution.started) '(pass)))
 (test-case "tool.execution.completed only allows 'pass action"
   (check-equal? (valid-hook-actions-for 'tool.execution.completed) '(pass)))
...
(define-test-suite hook-validation-tests
                   (test-case "valid-hook-name? accepts known hooks"
                     (for ([name (in-list '(turn-start turn-end
                                                       tool-call
                                                       tool-result
...
(define-test-suite
 ...
 (test-case "hook-point-names returns all registered hooks"
   ...
   (for ([expected (in-list '(turn-start turn-end
                                         tool-call
                                         session-before-switch
                                         tool.execution.started
                                         tool.execution.completed))])
     (check-not-false (member expected names) (format "~a should be in hook-point-names" expected))))
 ...
 (test-case "validate-hook-result rejects invalid action for tool.execution.started"
   (define bad-result (hook-amend (hasheq 'extra 'data)))
   (check-false (validate-hook-result 'tool.execution.started bad-result))))

;; ============================================================
;; Run all tests
;; ============================================================

(run-tests (make-test-suite "Hook Catalog Expansion Tests"
