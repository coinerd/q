...
(test-case "file stats summarize successful samples and record failures"
  (define samples
    (append (for/list ([i (in-range 10)]) (hasheq 'sample i 'status "pass" 'duration_ms (* 100 (add1 i))))
  ...
  (check-equal? (hash-ref stats 'p95_ms) 955.0) ; 900 + 0.55*(1000-900); max is 1000 so 1450 was impossible
...
(define (valid-manifest #:samples [n 10])
  (hasheq
   ...
   "q_sha" "f0b8f8cf00000000000000000000000000000000"
   "scheduler" "batch"
   "mode" "subprocess"
   "jobs" 2
...
(test-case "fewer than ten successful samples is rejected"
  ...
  (check-true (for/or ([e (in-list errors)]) (string-contains? e "fewer than 10"))
...
(test-case "missing environment/scheduler/digest fields are rejected"
  (for ([field '("environment" "scheduler" "mode" "jobs" "q_sha"
                 "selected_paths_digest" "command" "families")])
    (define m (hash-remove (valid-manifest) field))
    (check-true (pair? (hb:hotspot-manifest-errors m))
                (format "missing ~a must be rejected" field))))

(test-case "failed and timed-out attempts must be recorded in samples"
  (define m (valid-manifest))
  (define truncated-samples
    (list (hasheq "file" "tests/example.rkt"
                  "samples" (for/list ([i (in-range 10)]) (hasheq "sample" i "status" "pass" "duration_ms" 100))
                  "stats" (hasheq "successful" 10 "failures" 0 "timeouts" 0
                                  "median_ms" 100 "p95_ms" 100 "min_ms" 100 "max_ms" 100))))
  ;; stats claim one failure but samples record none -> inconsistent
  (define m2 (hash-set m "families"
                       (list (hash-set (first (hash-ref m "families"))
                                       "stats"
                                       (hasheq "successful" 9 "failures" 1 "timeouts" 0
                                               "median_ms" 100 "p95_ms" 100 "min_ms" 100 "max_ms" 100)))))
  (check-true (pair? (hb:hotspot-manifest-errors m2))
              "stats/sample inconsistency must be rejected"))
...
(test-case "manifest --check passes on untouched artifact and fails on byte drift"
  ...
  (call-with-output-file manifest-path
    (lambda (out) (display (hb:hotspot-canonical-json m) out)))
  ...
  (call-with-output-file manifest-path
    (lambda (out) (display (string-append (hb:hotspot-canonical-json m) " ")) out)
    #:exists 'truncate/replace)
  (define errors (hb:hotspot-artifact-errors manifest-path sums-path m))
  (check-true (for/or ([e (in-list errors)]) (string-contains? e "drift"))
