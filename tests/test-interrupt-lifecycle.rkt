...
(define interrupt-tests
  (test-suite "functional interrupt lifecycle"
    ...
    (test-case "canonical prompt terminal publication failure still releases ownership"
      (define dir (make-temporary-file "q-prompt-terminal-failure-~a" 'directory))
      (define bus (make-event-bus))
      (define sess (make-test-session dir bus))
      (define sentinel (exn:fail "prompt terminal publication failed" (current-continuation-marks)))
      (subscribe! bus
                  (lambda (evt)
                    (when (and (equal? (event-ev evt) "turn.completed")
                               (equal? (hash-ref (event-payload evt) 'scope #f) "prompt"))
                      (check-true (agent-session-prompt-running? sess)
                                  "ownership stays held during terminal publication")
                      (raise sentinel))))
      (define raised (box #f))
      (dynamic-wind void
                    (lambda ()
                      (parameterize ([current-event-bus-error-handler
                                      (lambda (_evt _handler subscriber-exn) (raise subscriber-exn))])
                        (with-handlers ([exn:fail? (lambda (exn) (set-box! raised exn))])
                          (run-prompt! sess "terminal failure")))
                      (check-eq? (unbox raised) sentinel)
                      (check-false (agent-session-prompt-running? sess))
                      (check-false (active-session-turn-id sess)))
                    (lambda ()
                      (call-with-values (lambda () (finish-session-turn! sess))
                                        (lambda (_turn-id _request-id) (void)))
                      (release-prompt! sess)
    ...
    (test-case "wired request cancels the active prompt, acknowledges once, and next prompt succeeds"
      ...
      (define correlated-acks
        (filter (lambda (evt)
                  (and (equal? (event-ev evt) "turn.completed")
                       (equal? (hash-ref (event-payload evt) 'scope #f) "prompt")
                       (equal? (hash-ref (event-payload evt) 'reason #f) "cancelled")
      ...
      (check-equal? (hash-ref (event-payload acknowledgement) 'target-session-id)
                    (agent-session-session-id sess))
      (check-equal? (hash-ref (event-payload acknowledgement) 'target-turn-id) turn-id)
      ...
      (define lifecycle-phases
        (filter (lambda (phase)
                  (member phase '("interrupt.requested" "interrupt.accepted" "turn.completed")))
                (map event-ev (unbox observed))))
      (check-equal? lifecycle-phases '("interrupt.requested" "interrupt.accepted" "turn.completed"))
      (check-false (for/or ([evt (in-list (unbox observed))])
                     (member (event-ev evt) '("turn.cancelled" "interrupt.failed"))))
