...
(define message-helper-tests
  (test-suite "util/message-helpers"
    ...
    (test-case "has-tool-calls? returns #t for message with tool-call part"
      (define msg
        (make-message "m1"
                      ...
                      'assistant
                      'message
                      (list (make-text-part "hello") (make-tool-call-part "tc1" "bash" "{}"))
                      1000
                      (hasheq)))
    ...
    (test-case "has-tool-calls? returns #f for text-only message"
      (define msg
        (make-message "m2" #f 'assistant 'message (list (make-text-part "hello")) 1000 (hasheq)))
      (check-false (has-tool-calls? msg)))

    (test-case "has-tool-calls? returns #f for user message"
      (define msg (make-message "m3" #f 'user 'message (list (make-text-part "hi")) 1000 (hasheq)))
      (check-false (has-tool-calls? msg)))
    ...
    (test-case "tool-result-message? returns #t for tool-result kind"
      (define msg (make-message "m4" #f 'tool 'tool-result '() 1000 (hasheq)))
      (check-not-false (tool-result-message? msg)))

    (test-case "tool-result-message? returns #t for bash-execution kind"
      (define msg (make-message "m5" #f 'tool 'bash-execution '() 1000 (hasheq)))
      (check-not-false (tool-result-message? msg)))

    (test-case "tool-result-message? returns #f for regular message"
      (define msg (make-message "m6" #f 'user 'message (list (make-text-part "hi")) 1000 (hasheq)))
