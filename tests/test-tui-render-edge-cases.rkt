...
(define render-edge-tests
  (test-suite "Render Pipeline Edge Cases"
    ...
    (test-case "RE2: long text wraps at narrow terminal width"
      ...
      (define events (event-sequence "assistant.message.completed" (hash 'content long-text)))
      ...
      (check-true (> (length lines) 1) (format "expected wrapped lines, got ~a" (length lines))))
