...
(test-case "dispatch-hooks with pass handlers preserves payload"
  (define reg (make-extension-registry))
  (register-extension! reg (extension "ext" "1" "1" (hasheq 'test (λ (p) (hook-pass p)))))
...
(test-case "dispatch-hooks with amend updates payload"
  ...
  (register-extension! reg
                       (extension "amender" "1" "1" (hasheq 'test (λ (p) (hook-amend "amended")))))
  (define result (dispatch-hooks 'test "original" reg))
...
(test-case "dispatch-hooks chains amend then pass"
  (define reg (make-extension-registry))
  (register-extension! reg
                       (extension "amender" "1" "1" (hasheq 'test (λ (p) (hook-amend "changed")))))
  (register-extension! reg (extension "passer" "1" "1" (hasheq 'test (λ (p) (hook-pass p)))))
  (define result (dispatch-hooks 'test "original" reg))
  (check-eq? (hook-result-action result) 'amend)
...
(test-case "dispatch-hooks with block stops immediately"
  (define reg (make-extension-registry))
  (register-extension! reg (extension "blocker" "1" "1" (hasheq 'test (λ (p) (hook-block "nope")))))
...
(test-case "dispatch-hooks block prevents later handlers from running"
  ...
  (register-extension! reg (extension "blocker" "1" "1" (hasheq 'test (λ (p) (hook-block "stop")))))
  (register-extension! reg
                       (extension "effect"
                                  "1"
                                  "1"
...
(test-case "dispatch-hooks isolates handler exceptions"
  (define reg (make-extension-registry))
  (register-extension! reg (extension "crasher" "1" "1" (hasheq 'test (λ (p) (error "boom")))))
