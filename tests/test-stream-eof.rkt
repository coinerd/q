...
(define stream-eof-tests
  (test-suite "Silent Stream EOF Detection (BUG-SILENT-STREAM-EOF)"
    ...
    (test-case "silent EOF emits synthetic model.stream.completed with finish_reason eof"
      ...
      (define result
        (stream-from-provider mock-prov req bus "test-session" "turn-1" (test-state) #f #f))
    ...
    (test-case "normal completion does NOT emit synthetic model.stream.completed"
      ...
      (define req (make-model-request '() #f (hasheq)))
      (define result
        (stream-from-provider mock-prov req bus "test-session" "turn-2" (test-state) #f #f))
      (sync-events bus)
      (define completions (unbox captured))
    ...
    (test-case "empty stream yields #f immediately — no synthetic event"
      ...
      (define req (make-model-request '() #f (hasheq)))
      (define result
        (stream-from-provider mock-prov req bus "test-session" "turn-3" (test-state) #f #f))
      (sync-events bus)
      (define completions (unbox captured))
    ...
    (test-case "silent EOF emits message.end when message was started"
      ...
      (define chunks (list (make-stream-chunk "Partial text" #f (hasheq) #f)))
      (define mock-prov (make-eof-mock-provider chunks))
      (define req (make-model-request '() #f (hasheq)))
      (define result
        (stream-from-provider mock-prov req bus "test-session" "turn-4" (test-state) #f #f))
      (sync-events bus)
