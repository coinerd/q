...
(define (make-test-message id role text #:kind [kind 'message])
...
(define-test-suite
 ...
 (test-case "compact-history reduces message count"
   (define msgs (make-n-messages 30))
   (define result (compact-history msgs #:summarize-fn test-summarize #:token-config tiny-tc))
 ...
 (test-case "compact-history does not modify original messages"
   ...
   (compact-history msgs #:summarize-fn test-summarize #:token-config tiny-tc)
 ...
 (test-case "compact-history with too few messages returns identity"
   ...
   (define result (compact-history msgs #:summarize-fn test-summarize))
 ...
 (test-case "compact-history with empty messages returns empty result"
   (define result (compact-history '() #:summarize-fn test-summarize))
   (check-equal? (compaction-result-removed-count result) 0)
 ...
 (test-case "custom summarize function is used"
   ...
   (define result (compact-history msgs #:summarize-fn custom-summarize #:token-config tiny-tc))
   (define summary (compaction-result-summary-message result))
 ...
 (test-case "write-compaction-entry! writes to session log"
   ...
   (for ([i (in-range 10)])
     (append-entry! path (make-test-message (format "orig~a" i) 'user (format "Original ~a" i))))
   ;; Run compaction
 ...
 (test-case "write-compaction-entry! preserves original history"
   (define dir (make-temp-dir))
   (define path (session-path dir))
   (define original-msgs
     (for/list ([i (in-range 10)])
       (make-test-message (format "orig~a" i) 'user (format "Original ~a" i))))
   ...
   (check-equal? (length all-entries) 11)
   (for ([i (in-range 10)])
     (check-equal? (message-id (list-ref all-entries i)) (format "orig~a" i)))
   (delete-directory/files dir #:must-exist? #f))
 ...
 (test-case "compact-and-persist! writes summary entry to session log"
   ...
   ;; Compact and persist
   (define result
     (compact-and-persist! loaded path #:summarize-fn test-summarize #:token-config tiny-tc))
   ...
   (for ([i (in-range 30)])
     (check-equal? (message-id (list-ref all-entries i)) (format "msg~a" i)))
   (delete-directory/files dir #:must-exist? #f))
 ...
 (test-case "compact-and-persist! is no-op when nothing to compact"
   ...
   (for ([m (in-list msgs)])
     (append-entry! path m))
   (define loaded (load-session-log path))
   (define result (compact-and-persist! loaded path #:summarize-fn test-summarize))
 ...
 (test-case "compact-history contract: does not modify session log"
   ...
   (define loaded (load-session-log path))
   (compact-history loaded #:summarize-fn test-summarize #:token-config tiny-tc)
   ;; Verify log is unchanged
   (define entries-after (load-session-log path))
   (check-equal? (length entries-after) 10)
   (for ([i (in-range 10)])
     (check-equal? (message-id (list-ref entries-after i)) (format "msg~a" i)))
   (delete-directory/files dir #:must-exist? #f))
 ;; ══════════════════════════════════════════════════════════════
   ...
   (define amend-hook
     (lambda (hook-point payload)
       ...
       (define modified-payload
         (context-assembly-payload (context-assembly-payload-tier-a-messages payload)
 ...
 (test-case "build-tiered-context-with-hooks: hook returns 'block raises error"
   ...
   (check-exn (lambda (e) (and (exn:fail? e) (string-contains? (exn-message e) "blocked")))
              (lambda ()
                (build-tiered-context-with-hooks msgs
                                                 #:hook-dispatcher block-hook
                                                 #:tier-b-count 5
 ...
 (test-case "context-assembly-payload is serializable"
   ...
   (check-equal? (length (tiered-context-tier-a tc-from-payload)) (length (tiered-context-tier-a tc)))
   (check-equal? (length (tiered-context-tier-b tc-from-payload)) (length (tiered-context-tier-b tc)))
 ...
 (test-case "build-tiered-context: system-instruction always included"
   ...
   (define sys-msg (make-test-message "sys-0" 'system "System prompt" #:kind 'system-instruction))
   (define user-msg (make-test-message "user-0" 'user "Hello"))
   (define msgs
     (append (list sys-msg user-msg)
             (for/list ([i (in-range 1 15)])
               (make-test-message (format "msg~a" i) 'user (format "Message ~a" i)))))
   ...
   ;; Total should include system + user + tier-b + tier-c
   (check-true (>= (length flat) 6)))
 (test-case "build-tiered-context: system-instruction preserved with many pairs"
   ;; Simulate real scenario: system + user + many assistant/tool pairs
   (define sys-msg (make-test-message "sys" 'system "Prompt" #:kind 'system-instruction))
   (define user-msg (make-test-message "usr" 'user "Do stuff"))
   (define pairs
