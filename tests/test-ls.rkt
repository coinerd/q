...
(define (with-test-dir thunk)
  ...
  (dynamic-wind void (λ () (thunk dir)) (λ () (delete-directory/files dir))))

;; Create test files/dirs inside a given base
(define (populate-test-dir base)
  ;; Files
  (call-with-output-file (build-path base "banana.txt") (λ (p) (display "yellow" p)))
  (call-with-output-file (build-path base "apple.txt")
                         (λ (p) (display "red fruit is here" p))) ; 18 bytes
  (call-with-output-file (build-path base "cherry.dat") (λ (p) (display "x" p))) ; 1 byte
  ;; Hidden file
  (call-with-output-file (build-path base ".secret") (λ (p) (display "hidden stuff" p)))
  ;; Subdirectory
  ...
  (test-suite "tool-ls"
    ...
    (test-case "sort by name (default)"
      (with-test-dir (λ (dir)
                       ...
                       (define file-entries
                         (filter (λ (n) (member n '("apple.txt" "banana.txt" "cherry.dat"))) content))
    ...
    (test-case "sort by date"
      (with-test-dir (λ (dir)
                       ...
                       (call-with-output-file (build-path dir "old.txt") (λ (p) (display "old" p)))
    ...
    (test-case "directories listed first in name sort"
      (with-test-dir (λ (dir)
                       ...
                       (call-with-output-file (build-path dir "aaa-file.txt") (λ (p) (display "a" p)))
                       (define r (tool-ls (hasheq 'path dir)))
                       (check-false (result-is-error? r))
                       (define content (result-content r))
    ...
    (test-case "file path (not directory) returns error"
      (with-test-dir (λ (dir)
                       ...
                       (check-true (result-is-error? r))
                       (check-not-false (regexp-match? #rx"Not a directory"
                                                       (first (result-content r)))))))
    ...
    (test-case "symlinks shown with 'l' type in long format"
      (with-test-dir
       (λ (dir)
         (call-with-output-file (build-path dir "target.txt") (λ (p) (display "hello" p)))
         (make-file-or-directory-link (build-path dir "target.txt") (build-path dir "link-to-target"))
         (define r (tool-ls (hasheq 'path dir 'long? #t)))
         (check-false (result-is-error? r))
         (define content (result-content r))
         (check-not-false (ormap (λ (l) (regexp-match? #rx"^l - link-to-target$" l)) content)
