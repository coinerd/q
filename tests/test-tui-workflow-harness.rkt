...
(define workflow-harness-smoke-tests
  (test-suite "TUI Workflow Harness Smoke Tests"
    ...
    (test-case "SH1: apply-events + render-state-strings pipeline"
      ...
      (define events
        (list (make-test-event "turn.started" (hash))
              (make-test-event "assistant.message.completed" (hash 'content "Hello from assistant"))
      ...
      (check-true (>= (length lines) 1) (format "expected >=1 rendered line, got ~a" (length lines)))
      ;; The rendered output should contain the assistant message
      (check-not-false (for/or ([l (in-list lines)])
                         (string-contains? l "Hello from assistant"))
