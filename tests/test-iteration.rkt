...
(define iteration-tests
  (test-suite "Iteration Loop with Steering Queue"
      ...
      (subscribe! bus
                  (lambda (evt)
                    (set-box! collected-events
                              (append (unbox collected-events) (list (event-event evt))))))
    ...
    (test-case "without queue parameter iteration still works (backward compat)"
      ...
      (check-true (>= (length msgs) 1) "should have at least one message in result"))))
