...
(test-case "decision.rkt source contains decide-next-action call"
  (define content (call-with-input-file (build-path q-root "runtime" "iteration" "decision.rkt") port->string))
  ...
  (check-true (>= def-count 1)
              (format "expected >= 1 decide-next-action call with ctx, found ~a"
                      def-count)))

(test-case "iteration-ctx struct is used to construct decision context"
  (define content (call-with-input-file (build-path q-root "runtime" "iteration" "main-loop.rkt") port->string))
