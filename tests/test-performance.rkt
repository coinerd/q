...
(define performance-tests
  (test-suite "Performance Tests"
    ...
    (test-case "Session Store: append 100 entries in < 500ms"
      ...
      (check-true (< elapsed 500) (format "append 100 entries took ~ams (limit 500ms)" elapsed))
    ...
    (test-case "Session Index: build-index! 100 entries in < 500ms"
      ...
      (define elapsed (- (current-inexact-milliseconds) start))
      (check-true (< elapsed 500) (format "build-index! 100 entries took ~ams (limit 500ms)" elapsed))
    ...
    (test-case "Event Bus: publish 1000 events in < 500ms"
      ...
      (for ([evt (in-list events)])
        (publish! bus evt))
      (define elapsed (- (current-inexact-milliseconds) start))
      (check-true (< elapsed 500) (format "publish 1000 events took ~ams (limit 500ms)" elapsed))
    ...
    (test-case "JSONL Read/Write Roundtrip: 50 entries in < 200ms"
      ...
      (check-equal? (text-part-text (first (message-content (first loaded)))) "Message 0")
      (check-equal? (text-part-text (first (message-content (last loaded)))) "Message 49")
      (delete-directory/files dir))))
