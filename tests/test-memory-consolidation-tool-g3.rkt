...
(test-case "W4: consolidate with <2 IDs returns error"
  ...
  (parameterize ([current-memory-backend backend]
    ...
    (define result
      (handler
       (hasheq 'ids '("m1") 'merged-content "x" 'scope "session" 'session-id "s1" 'project-root ".")))
...
;; ---------------------------------------------------------------------------
;; v0.95.18 W0: Audit regression tests (expected red before W4)
;; ---------------------------------------------------------------------------

(test-case "W0 F6: invalid target type is rejected before store"
  (define backend (make-memory-hash-backend))
  (parameterize ([current-memory-backend backend]
                 [current-memory-policy default-memory-policy])
    (gen:store-memory! backend (make-test-item "bad1" "content bad one"))
    (gen:store-memory! backend (make-test-item "bad2" "content bad two"))
    (define handler (tool-execute consolidate-memory))
    (define result
      (handler (hasheq 'ids
                       '("bad1" "bad2")
...
(test-case "W0 F6: keep-originals=false supersedes rather than reporting deletion"
  (define backend (make-memory-hash-backend))
  (parameterize ([current-memory-backend backend]
                 [current-memory-policy default-memory-policy])
    (gen:store-memory! backend (make-test-item "sup1" "content sup one"))
    (gen:store-memory! backend (make-test-item "sup2" "content sup two"))
    (define handler (tool-execute consolidate-memory))
    (define result
      (handler (hasheq 'ids
                       '("sup1" "sup2")
                       ...
                       ".")))
    (check-false (tool-result-is-error? result))
    (check-false (string-contains? (tool-result-content result) "deleted"))
    (check-true (hash-has-key? (tool-result-details result) 'source-updates))))
