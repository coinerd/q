...
(define (count-events events-box event-name)
  (length (filter (lambda (e) (equal? (event-event e) event-name)) (reverse (unbox events-box)))))
...
(define session-lifecycle-guard-tests
  (test-suite "Session Lifecycle Guard Tests"

    ;; ============================================================
    ;; S1: run-prompt! on closed session should error
    ;; ============================================================
    (test-case "S1: run-prompt! on closed session raises error"
    ...
    ;; ============================================================
    (test-case "S2: close-session! called twice emits only one session.closed event"
      (define cfg (make-test-config))
      ...
      (check-equal? (count-events events "session.closed")
                    1
                    "only one session.closed event from double close")
      (check-false (session-active? sess) "session is not active after close")
      (cleanup-dir! (hash-ref cfg 'session-dir)))

    ;; ============================================================
    ;; S3: fork-session on closed session should error
    ;; ============================================================
    (test-case "S3: fork-session on closed session raises error"
      (define cfg (make-test-config))
      (define sess (make-agent-session cfg))
      (close-session! sess)
      (check-exn exn:fail?
    ...
    ;; ============================================================
    (test-case "S4: Concurrent ensure-persisted! calls are safe"
      (define cfg (make-test-config))
      (define sess (make-agent-session cfg))
    ...
    ;; ============================================================
    (test-case "S5: close-session! during active run-prompt! session deactivates"
      (define cfg (make-test-config))
      (define bus (hash-ref cfg 'event-bus))
      (define events (get-events bus))
      (define sess (make-agent-session cfg))
      ...
      (close-session! sess)
      (check-false (session-active? sess) "session deactivated after close")
