...
(define event-ordering-tests
  (test-suite "Event Ordering Invariant Regression Guard"
    ...
    (test-case "regression: assistant.message.completed before tool.call.started (invariant)"
      ...
      (for ([scenario scenarios])
        ...
        (run-agent-turn ctx prov bus #:session-id "s1" #:turn-id "t1" #:tools tools)
        ...
        (check-not-false amc-idx (format "~a: assistant.message.completed found" name))
        (check-not-false tcs-idx (format "~a: tool.call.started found" name))
    ...
    (test-case "regression: turn.started is always the first event"
      (define bus (make-event-bus))
      (define captured (box '()))
      (subscribe! bus
