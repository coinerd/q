...
(test-case "list-packages returns empty for fresh dir"
  (with-test-packages-dir (lambda () (check-equal? (list-packages) '()))))
...
(test-case "package-installed? returns #f before install"
  (with-test-packages-dir (lambda () (check-false (package-installed? "q-test-pkg")))))
...
(test-case "package-info returns #f for unknown"
  (with-test-packages-dir (lambda () (check-false (package-info "no-such-package")))))
...
(test-case "remove-package returns #f for unknown"
  (with-test-packages-dir (lambda () (check-false (remove-package "no-such-package")))))
  ...
  (dynamic-wind
   ...
   (lambda ()
     (with-test-packages-dir (lambda ()
                               ...
                               (check-equal? (qpm-manifest-name (qpm-package-manifest (car pkgs)))
                                             "q-test-pkg"))))
   (lambda () (cleanup-source-dir srcdir))))
...
(test-case "install-package-from-dir rejects path traversal in files"
  ...
  (call-with-output-file (build-path srcdir "main.rkt")
                         (lambda (out) (display "#lang racket/base\n(define x 1)\n" out))
                         #:exists 'replace)
  ;; Use raw struct to bypass safe-manifest-file-path? check in validate-manifest
  ;; (defense-in-depth: copy loop should catch it independently)
  (define m
    (qpm-manifest "traversal-test"
                  "1.0.0"
                  "1"
                  'extension
                  "Test"
  ...
  (dynamic-wind
   ...
   (lambda ()
     (with-test-packages-dir (lambda ()
                               (define result (install-package-from-dir srcdir))
                               ;; Should fail — either via validation or copy loop defense
                               (check-pred string? result)
                               (check-true (or (string-contains? result "error")
                                               (string-contains? result "escapes"))))))
   (lambda () (cleanup-source-dir srcdir))))
