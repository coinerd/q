...
(define (make-mock-ext-reg . hook-dispatcher-pairs)
  ...
  (define hooks-hash
    (let loop ([pairs hook-dispatcher-pairs]
               [h (hasheq)])
      (if (null? pairs)
          h
...
(define (tool-call-then-text-responses tool-name tool-args)
  (list (make-model-response
         (list (hash 'type "tool-call" 'id "tc-1" 'name tool-name 'arguments tool-args))
...
(define iteration-transition-tests
  (test-suite "Iteration Pipeline Transition Tests"

    ;; ============================================================
    ;; I1: iteration loop with tool-calls-pending then text response
    ;; ============================================================
    (test-case "I1: iteration loop with tool-calls-pending then text response"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
      (define prov
        (make-multi-mock-provider (tool-call-then-text-responses "read" (hasheq 'path "/tmp/test"))))
    ...
    ;; ============================================================
    (test-case "I2: tool-call hook amends tool calls"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
      (define events (box '()))
      ...
      (define prov
        (make-multi-mock-provider (tool-call-then-text-responses "bash"
                                                                 (hasheq 'command "rm -rf /"))))
      (define reg (make-tool-registry))
      (register-tool! reg (make-read-tool))
    ...
    ;; ============================================================
    (test-case "I3: tool-call hook removes all tool calls"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
      (define events (box '()))
      (subscribe! bus (lambda (e) (set-box! events (cons e (unbox events)))))
      ...
      (define prov
        (make-multi-mock-provider (tool-call-then-text-responses "read" (hasheq 'path "/a"))))
      (define reg (make-tool-registry))
      (register-tool! reg (make-read-tool))
    ...
    ;; ============================================================
    (test-case "I4: tool-result hook blocks — original messages preserved"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
      (define events (box '()))
      (subscribe! bus (lambda (e) (set-box! events (cons e (unbox events)))))
    ...
    ;; ============================================================
    (test-case "I5: max-iterations = 0 triggers max-iterations-exceeded on first tool call"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
      (define events (box '()))
      (subscribe! bus (lambda (e) (set-box! events (cons e (unbox events)))))
      ...
      (define evts (reverse (unbox events)))
      (define error-events (filter (lambda (e) (equal? (event-event e) "runtime.error")) evts))
    ...
    ;; ============================================================
    (test-case "I6: follow-up delivery mode one-at-a-time dequeues one at a time"
      (cleanup-temp-log!)
      (define bus (make-event-bus))
