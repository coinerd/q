...
(define sample-request
  (make-model-request (list (hash 'role "user" 'content "Hello")) #f (hash 'model "test-model")))

(define sample-request-with-tools
  (make-model-request (list (hash 'role "user" 'content "Read foo.rkt"))
                      (list sample-tool)
...
(define (run-provider-conformance-tests spec)
  ...
  (define test-config
    (hash 'api-key "test-key-123" 'model "test-model" 'base-url "https://example.com"))

  ;; ============================================================
  ;; 1. Provider construction
  ;; ============================================================

  (test-case (format "~a provider: make-provider returns provider?" spec-name)
    (define p (make-provider-fn test-config))
    (check-true (provider? p)))

  (test-case (format "~a provider: correct name" spec-name)
    (define p (make-provider-fn test-config))
    (check-equal? (provider-name p) spec-name))

  (test-case (format "~a provider: capabilities hash with 'streaming key" spec-name)
    (define p (make-provider-fn test-config))
  ...
  (test-case (format "~a request body: returns hash with ~a" spec-name body-messages-key)
    (define body (build-request-fn sample-request))
    (check-pred hash? body)
    (check-true (hash-has-key? body body-messages-key)))

  (test-case (format "~a request body: contains model info" spec-name)
    (define body (build-request-fn sample-request))
    ;; At least one key should reference the model — check settings-based key
    (check-true (hash? body)))

  (test-case (format "~a request body: streaming flag (#t)" spec-name)
  ...
  (test-case (format "~a response: parse-response returns model-response?" spec-name)
    (define parsed (parse-response-fn fake-response))
    (check-true (model-response? parsed)))

  (test-case (format "~a response: has content" spec-name)
    (define parsed (parse-response-fn fake-response))
    (check-true (list? (model-response-content parsed))))

  (test-case (format "~a response: has usage hash" spec-name)
    (define parsed (parse-response-fn fake-response))
    (check-true (hash? (model-response-usage parsed))))

  (test-case (format "~a response: usage has prompt_tokens" spec-name)
    (define parsed (parse-response-fn fake-response))
    (define usage (model-response-usage parsed))
    (check-true (hash-has-key? usage 'prompt_tokens)))

  (test-case (format "~a response: usage has completion_tokens" spec-name)
    (define parsed (parse-response-fn fake-response))
    (define usage (model-response-usage parsed))
    (check-true (hash-has-key? usage 'completion_tokens)))

  (test-case (format "~a response: usage has total_tokens" spec-name)
    (define parsed (parse-response-fn fake-response))
    (define usage (model-response-usage parsed))
    (check-true (hash-has-key? usage 'total_tokens)))

  (test-case (format "~a response: model string matches expected" spec-name)
    (define parsed (parse-response-fn fake-response))
    (check-equal? (model-response-model parsed) expected-model))

  (test-case (format "~a response: stop-reason is a symbol" spec-name)
    (define parsed (parse-response-fn fake-response))
    (check-true (symbol? (model-response-stop-reason parsed))))

  (test-case (format "~a response: stop-reason is ~a" spec-name expected-stop)
    (define parsed (parse-response-fn fake-response))
  ...
  (when has-translate-tool?
    (test-case (format "~a tool: translate-tool returns hash with 'name" spec-name)
  ...
  (when has-translate-stop?
    (for ([pair (in-list stop-translations)])
      ...
      (test-case (format "~a stop-reason: \"~a\" → '~a" spec-name input expected)
  ...
  (test-case (format "~a HTTP: 200 passes without error" spec-name)
    (check-not-exn (lambda () (check-status-fn #"HTTP/1.1 200 OK" #"{}"))))

  (test-case (format "~a HTTP: 401 raises error" spec-name)
    (check-exn exn:fail? (lambda () (check-status-fn #"HTTP/1.1 401 Unauthorized" #"error"))))

  (test-case (format "~a HTTP: 500 raises error" spec-name)
    (check-exn exn:fail?
               (lambda () (check-status-fn #"HTTP/1.1 500 Internal Server Error" #"internal error"))))

  ;; ============================================================
  ;; 7. Provider dispatch (mock — no real API call)
  ;; ============================================================

  (test-case (format "~a provider: provider-send is callable (won't call API)" spec-name)
    (define p (make-provider-fn test-config))
  ...
  (test-case (format "~a provider: provider-stream returns generator procedure" spec-name)
    ;; Use a mock provider to test the stream interface contract
    (define mock-resp
      (make-model-response (list (hash 'type "text" 'text "test"))
...
(run-provider-conformance-tests (hash 'name
                                      "anthropic"
                                      'make-provider
...
(run-provider-conformance-tests
 (hash
  ...
  (hash 'id
        ...
        "test-model"
        'choices
        (list (hash 'index 0 'message (hash 'role "assistant" 'content "test") 'finish_reason "stop"))
        'usage
...
(run-provider-conformance-tests
 (hash
  ...
  (hash
   'candidates
   (list (hash 'content (hash 'role "model" 'parts (list (hash 'text "test"))) 'finishReason "STOP"))
   'usageMetadata
...
(test-case "All providers: request body is a hash with messages or contents"
  ;; Anthropic and OpenAI use 'messages; Gemini uses 'contents
  (let* ([config (hash 'api-key "test" 'model "test" 'base-url "https://example.com")]
         [req (make-model-request (list (hash 'role "user" 'content "Hi")) #f (hash 'model "test"))])
    ;; Anthropic
...
(test-case "All providers: response parsing produces model-response with symbol stop-reason"
  ;; Anthropic
  (let ([parsed (anthropic-parse-response (hash 'id
                                                "msg"
                                                'type
                                                "message"
                                                'role
                                                "assistant"
                                                'content
                                                (list (hash 'type "text" 'text "x"))
                                                'model
                                                "m"
                                                'stop_reason
                                                "end_turn"
                                                'usage
                                                (hash 'input_tokens 1 'output_tokens 1)))])
  ...
  (let ([parsed
         (openai-parse-response
          (hash
           ...
           "m"
           'choices
           (list (hash 'index 0 'message (hash 'role "assistant" 'content "x") 'finish_reason "stop"))
           'usage
  ...
  (let ([parsed (gemini-parse-response
                 (hash 'candidates
                       (list (hash 'content
                                   (hash 'role "model" 'parts (list (hash 'text "x")))
                                   'finishReason
                                   "STOP"))
                       'usageMetadata
                       (hash 'promptTokenCount 1 'candidatesTokenCount 1 'totalTokenCount 2)))])
    (check-pred model-response? parsed)
    (check-true (symbol? (model-response-stop-reason parsed)))))
...
(test-case "Mock provider: stream returns generator yielding stream-chunk? then #f"
  (define resp
    (make-model-response (list (hash 'type "text" 'text "chunk1"))
                         (hash 'prompt_tokens 1 'completion_tokens 1 'total_tokens 2)
                         "test"
                         'stop))
