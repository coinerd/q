...
(test-case "make-loop-state returns a loop-state"
  (check-true (loop-state? (make-loop-state "sess-1" "turn-1"))))

(test-case "fresh loop-state has correct session and turn ids"
...
(test-case "state-add-message! adds messages in order"
  (define st (make-loop-state "sess-1" "turn-1"))
  (define msg-1 (make-message "m-1" #f 'user 'message (list (make-text-part "hi")) 1000 '#hash()))
  (define msg-2
    (make-message "m-2" "m-1" 'assistant 'message (list (make-text-part "hello")) 1001 '#hash()))
...
(test-case "state-add-event! adds events"
  (define st (make-loop-state "sess-1" "turn-1"))
...
(test-case "separate loop-state instances don't share state"
  (define st (make-loop-state "sess-1" "turn-1"))
  (define msg-1 (make-message "m-1" #f 'user 'message (list (make-text-part "hi")) 1000 '#hash()))
  (state-add-message! st msg-1)
  (state-add-message!
...
(test-case "fresh queue is empty; enqueue/dequeue round-trip"
  (define q (make-queue))
...
(test-case "subscribe and publish events on event bus"
  (define bus (make-event-bus))
  (define collected-events '())
  (subscribe! bus (lambda (evt) (set! collected-events (cons evt collected-events))))
...
(test-case "BUG-35: assistant with tool calls but NO text content"
...
(test-case "BUG-35: assistant with BOTH text and tool calls"
  (define assistant-msg-with-text
    (make-message "m-4"
                  "m-3"
                  'assistant
                  'message
                  (list (make-text-part "I'll help you") (make-tool-call-part "tc-2" "edit" "{}"))
                  1003
                  '#hash()))
...
(test-case "run-agent-turn: 3 text deltas → accumulated text"
  (define bus (make-event-bus))
  (define prov
...
(test-case "run-agent-turn: tool-call delta → tool-calls-pending"
  (define bus (make-event-bus))
  (define prov
    (make-provider
     (lambda () "mock")
     ...
     (lambda (req)
       (list (make-stream-chunk #f
                                (hasheq 'id
                                        ...
                                        0
                                        'function
                                        (hasheq 'name "bash" 'arguments "{\"command\":\"ls\"}"))
                                #f
                                #f)
             (make-stream-chunk #f #f #f #t)))))
  (define ctx
    (list (make-message "m-1" #f 'user 'message (list (make-text-part "run bash")) 1000 '#hash())))
  (define result
    (run-agent-turn
     ctx
     prov
     ...
     #:session-id "s1"
     #:turn-id "t1"
     #:tools (list (hasheq 'type "function" 'function (hasheq 'name "bash" 'parameters (hasheq))))))
...
(test-case "run-agent-turn: mixed text + tool call → both captured"
  (define bus (make-event-bus))
  (define prov
    (make-provider
     (lambda () "mock")
       ...
       (list (make-stream-chunk "Thinking..." #f #f #f)
             (make-stream-chunk #f
                                (hasheq 'id
                                        ...
                                        0
                                        'function
                                        (hasheq 'name "read" 'arguments "{\"path\":\"foo.rkt\"}"))
                                #f
                                #f)
             (make-stream-chunk #f #f #f #t)))))
  (define ctx
    (list (make-message "m-1" #f 'user 'message (list (make-text-part "do stuff")) 1000 '#hash())))
  (define result
    (run-agent-turn
     ctx
     prov
     ...
     #:session-id "s1"
     #:turn-id "t1"
     #:tools (list (hasheq 'type "function" 'function (hasheq 'name "read" 'parameters (hasheq))))))
  (check-equal? (loop-result-termination-reason result) 'tool-calls-pending)
  (define msgs (loop-result-messages result))
  (check = (length msgs) 1)
  (define assistant (car msgs))
...
(test-case "run-agent-turn: pre-cancelled token → 'cancelled termination"
  (define bus (make-event-bus))
  ...
  (define prov
    (make-provider (lambda () "mock")
                   ...
                   (lambda (req) (error 'send "not used"))
                   (lambda (req)
                     (list (make-stream-chunk "text" #f #f #f) (make-stream-chunk #f #f #f #t)))))
  (define ctx
    (list (make-message "m-1" #f 'user 'message (list (make-text-part "hi")) 1000 '#hash())))
  (define result
    (run-agent-turn ctx prov bus #:session-id "s1" #:turn-id "t1" #:cancellation-token tok))
  (check-equal? (loop-result-termination-reason result) 'cancelled))

(test-case "run-agent-turn: empty stream → completed with empty text"
  (define bus (make-event-bus))
  (define prov
    (make-provider (lambda () "mock")
                   (lambda () (hasheq 'streaming #t))
...
(test-case "run-agent-turn: hook dispatcher receives message-start/update/end"
  (define bus (make-event-bus))
  ...
  (define prov
    (make-provider (lambda () "mock")
                   ...
                   (lambda (req) (error 'send "not used"))
                   (lambda (req)
                     (list (make-stream-chunk "hi" #f #f #f) (make-stream-chunk #f #f #f #t)))))
  (define ctx
    (list (make-message "m-1" #f 'user 'message (list (make-text-part "hello")) 1000 '#hash())))
  (define result
    (run-agent-turn ctx
                    prov
                    bus
...
(test-case "BUG #633: model-request-pre hook block emits turn.completed"
  (define bus (make-event-bus))
  (define events '())
  ...
  (check-not-false (member "turn.started" event-names) "turn.started must be emitted")
  (check-not-false (member "turn.completed" event-names)
                   "turn.completed must be emitted after hook-blocked early return"))

(test-case "BUG #633: message-start hook block emits turn.completed"
  (define bus (make-event-bus))
  (define events '())
  (subscribe! bus (lambda (evt) (set! events (cons evt events))))
...
(test-case "BUG #634: stream-blocked by message-update hook emits model.stream.completed"
  (define bus (make-event-bus))
  (define events '())
  (subscribe! bus (lambda (evt) (set! events (cons evt events))))
  ...
  (define prov
    (make-provider (lambda () "mock")
                   ...
                   (lambda (req)
                     ...
                     (list (make-stream-chunk "hello" #f #f #f) (make-stream-chunk #f #f #f #t)))))
  (define ctx
    (list (make-message "m-1" #f 'user 'message (list (make-text-part "hi")) 1000 '#hash())))
  (define result
    (run-agent-turn ctx
...
(test-case "streaming emits message.start, message.delta, message.end events"
  (define bus (make-event-bus))
  (define events (box '()))
  ...
  (define msg-events
    (filter (lambda (e) (member (event-event e) '("message.start" "message.delta" "message.end")))
            evts))
