...
(define (store-some backend)
  (with-backend backend
    (lambda ()
      ...
      (call-tool tool-store-memory (hash 'content "project decision" 'scope "project" 'type "procedural"))
...
(test-case "list-memory: respects limit"
  ...
  (with-backend b
    (lambda ()
      (define r (call-tool tool-list-memory (hash 'limit 2)))
      (check-false (tool-result-is-error? r))
      (define items (hash-ref (tool-result-details r) 'items '()))
...
(test-case "list-memory: snippet is truncated"
  ...
  (with-backend b
    (lambda ()
      ...
      (call-tool tool-store-memory (hash 'content long-content))
      (define r (call-tool tool-list-memory (hash)))
      (define items (hash-ref (tool-result-details r) 'items '()))
...
(test-case "delete-memory: returns id and scope in metadata"
  ...
  (with-backend b
    (lambda ()
      ;; Store first
      (call-tool tool-store-memory (hash 'content "to delete" 'scope "session"))
      ;; Get the id
      (define lr (call-tool tool-list-memory (hash)))
      (define items (hash-ref (tool-result-details lr) 'items '()))
