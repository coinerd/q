...
(define metadata-discovery-tests
  (test-suite "run-tests metadata discovery"
    ...
    (test-case "bare @not-test metadata excludes helper files from discovery"
      ...
      ;; ── v1.00.27 W0: tier-ownership matrix (eight ownership columns) ──
      ;;
      ...
      (test-case "checksummed v1.00.27-w0 matrix is complete and drift-free against the tree"
        ...
        (check-true (file-exists? artifact) "the v1.00.27-w0 ownership matrix must exist")
