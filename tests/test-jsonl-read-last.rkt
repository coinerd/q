...
(define (with-raw-temp-file content body-fn)
  ...
  (dynamic-wind
   (lambda ()
     (call-with-output-file tmp (lambda (out) (display content out)) #:mode 'text #:exists 'truncate))
   (lambda () (body-fn tmp))
   (lambda ()
     (when (file-exists? tmp)
       (delete-file tmp)))))
...
(test-case "jsonl-read-last preserves JSON types: strings, numbers, booleans, null, arrays"
  (define entries (list (hash 's "hello" 'n 42 'b #t 'u (json-null) 'arr '(1 2 3))))
  (with-temp-jsonl entries
                   (lambda (tmp)
                     (define result (jsonl-read-last tmp 10))
                     (check-equal? (length result) 1)
...
(test-case "jsonl-read-last handles nested JSON objects"
  (define entries (list (hash 'nested (hash 'inner (hash 'deep "value")))))
  (with-temp-jsonl
   entries
   (lambda (tmp)
     (define result (jsonl-read-last tmp 10))
     (check-equal? (length result) 1)
     (check-equal? (hash-ref (hash-ref (hash-ref (first result) 'nested) 'inner) 'deep) "value"))))
