...
(define-test-suite
 ...
 (test-case "index rebuild from log matches original semantics"
   ...
   (check-equal? (hash-count (session-index-by-id idx2)) (hash-count (session-index-by-id idx1)))
   (check-equal? (sort (map message-id (leaf-nodes idx2)) string<?)
                 (sort (map message-id (leaf-nodes idx1)) string<?))
   (check-equal? (message-id (resolve-active-leaf idx2)) (message-id (resolve-active-leaf idx1)))
 ...
 (test-case "build-index! handles corrupted lines gracefully"
   ...
   (call-with-output-file sp (lambda (out) (displayln "BROKEN!!!" out)) #:mode 'text #:exists 'append)
 ...
 (test-case "add-bookmark! creates bookmark with unique id"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v23 bm-id) (add-bookmark! idx "root" "important"))
   (set! idx _v23)
 ...
 (test-case "add-bookmark! replaces bookmark with same label"
   ...
   (append-entries! sp
                    (list (make-test-message "root" #f 'system 'message)
                          (make-test-message "c1" "root" 'user 'message)))
   (define idx (build-index! sp ip))
   (define-values (_v22 bm-id1) (add-bookmark! idx "root" "my-label"))
   (set! idx _v22)
   (define-values (_v21 bm-id2) (add-bookmark! idx "c1" "my-label"))
   (set! idx _v21)
 ...
 (test-case "remove-bookmark! deletes existing bookmark"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v20 bm-id) (add-bookmark! idx "root" "to-remove"))
   (set! idx _v20)
   (check-equal? (length (list-bookmarks idx)) 1)
   (define-values (_v18 _v19) (remove-bookmark! idx bm-id))
   (set! idx _v18)
   (check-true _v19)
 ...
 (test-case "remove-bookmark! returns #f for nonexistent id"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v16 _v17) (remove-bookmark! idx "nonexistent-id"))
   (set! idx _v16)
   (check-false _v17)
   (delete-directory/files dir #:must-exist? #f))
 ;; ── list-bookmarks ──
 (test-case "list-bookmarks returns all bookmarks"
   ...
   (define idx (build-index! sp ip))
   (check-equal? (length (list-bookmarks idx)) 0)
   (define-values (_v14 _v15) (add-bookmark! idx "root" "first"))
   (set! idx _v14)
   (define-values (_v12 _v13) (add-bookmark! idx "c1" "second"))
   (set! idx _v12)
 ...
 (test-case "find-bookmark-by-label finds exact match"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v10 _v11) (add-bookmark! idx "root" "find-me"))
   (set! idx _v10)
 ...
 (test-case "find-bookmark-by-label returns #f for no match"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v8 _v9) (add-bookmark! idx "root" "some-label"))
   (set! idx _v8)
 ...
 (test-case "get-bookmark retrieves by id"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v7 bm-id) (add-bookmark! idx "root" "retrievable"))
   (set! idx _v7)
   (define bm (get-bookmark idx bm-id))
   (check-not-false bm)
 ...
 (test-case "save-bookmarks! creates sidecar file"
   ...
   (append-entry! sp (make-test-message "root" #f 'system 'message))
   (define idx (build-index! sp ip))
   (define-values (_v5 _v6) (add-bookmark! idx "root" "persistent-label"))
   (set! idx _v5)
 ...
 (test-case "save-bookmarks! and load-bookmarks roundtrip"
   ...
   (append-entries! sp
                    (list (make-test-message "root" #f 'system 'message)
                          (make-test-message "c1" "root" 'user 'message)))
   (define idx (build-index! sp ip))
   (define-values (_v4 bm-id1) (add-bookmark! idx "root" "root-bookmark"))
   (set! idx _v4)
   (define-values (_v3 bm-id2) (add-bookmark! idx "c1" "c1-bookmark"))
   (set! idx _v3)
   (save-bookmarks! sp idx)
 ...
 (test-case "bookmarks survive session-resume via sidecar"
   ...
   (define idx1 (build-index! sp ip))
   (define-values (_v2 bm-id) (add-bookmark! idx1 "c1" "resume-test"))
   (set! idx1 _v2)
   (save-bookmarks! sp idx1)
   (define idx2 (load-index-with-bookmarks sp ip))
 ...
 (test-case "build-index! infers missing parentIds from log order"
   ;; v0.15.2: Assistant messages persisted with parentId=#f must have
   ;; their parent inferred from log order during index rebuild.
   ;; Without this, the tree walk loses the conversation chain on follow-up prompts.
   (define dir (make-temporary-file "q-index-test-~a" 'directory))
   (define log-path (build-path dir "session.jsonl"))
   (define idx-path (build-path dir "session.index"))
   ;; Create a session log with missing parentIds on assistant messages
   ;; (simulating what the agent loop actually persists)
   (define sys-msg (make-message "sys" #f 'system 'session-info '() (current-seconds) (hasheq)))
   (define user1
     (make-message "user1"
                   #f
                   'user
                   ...
                   (current-seconds)
                   (hasheq)))
   ;; Assistant with parentId=#f (as persisted by loop.rkt)
   (define asst1
     (make-message "asst1"
                   #f
                   'assistant
                   ...
                   (current-seconds)
                   (hasheq)))
   (define tool1 (make-message "tool1" "asst1" 'tool 'tool-result '() (current-seconds) (hasheq)))
   ;; Second user message correctly references asst1
   (define user2
     (make-message "user2"
                   "asst1"
                   'user
                   ...
                   (current-seconds)
                   (hasheq)))
   ;; Another assistant with parentId=#f
   (define asst2
     (make-message "asst2"
                   #f
                   'assistant
                   'message
                   (list (make-text-part "Done"))
                   (current-seconds)
                   (hasheq)))
   (define tool2 (make-message "tool2" "asst2" 'tool 'tool-result '() (current-seconds) (hasheq)))
   (append-entries! log-path (list sys-msg user1 asst1 tool1 user2 asst2 tool2))
   ;; Build index — should infer parentIds
   (define idx (build-index! log-path idx-path))
   ;; asst1 should now have parentId=user1 (inferred)
   (define fixed-asst1 (lookup-entry idx "asst1"))
   (check-equal? (message-parent-id fixed-asst1) "user1")
   ;; tool1 should keep its explicit parentId=asst1
   (check-equal? (message-parent-id (lookup-entry idx "tool1")) "asst1")
   ;; user2 should keep its explicit parentId=asst1
   (check-equal? (message-parent-id (lookup-entry idx "user2")) "asst1")
   ;; asst2 should have parentId=user2 (inferred from log order)
   (define fixed-asst2 (lookup-entry idx "asst2"))
   (check-equal? (message-parent-id fixed-asst2) "user2")
   ;; Full branch walk from tool2 should traverse the chain
   (define branch (get-branch idx "tool2"))
   ;; tool2 -> asst2 -> user2 -> asst1 -> user1 -> sys = 6 entries
