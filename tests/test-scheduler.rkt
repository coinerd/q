...
;; Helper: build a permissive exec-context for tests that exercise the
;; scheduler mechanics (order, hooks, parallelism, metadata) rather than
...
(test-case "Multiple tool calls execute in order (serial)"
  ...
  (define sr (run-tool-batch tcs reg #:parallel? #f #:exec-context (make-test-exec-context)))
...
(test-case "Parallel execution preserves commit order"
  ...
  (define sr (run-tool-batch tcs reg #:parallel? #t #:exec-context (make-test-exec-context)))
...
(test-case "F6: parallel execution respects max-parallel-tools bound"
  ...
  (define result
    (parameterize ([max-parallel-tools 2])
      (run-tool-batch tool-calls reg #:parallel? #t #:exec-context (make-test-exec-context))))
