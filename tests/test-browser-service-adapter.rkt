...
(test-case "service browser-open! dispatches via adapter interface"
  ...
  (define test-adapter
    (make-browser-adapter #:open (lambda (session-id target)
                                   (set! opened-with (list session-id target))
                                   'ok)
                          #:close (lambda (sid) 'ok)
...
(test-case "service browser-observe! dispatches via adapter interface"
  ...
  (define test-adapter
    (make-browser-adapter #:open (lambda (sid target) 'ok)
                          ...
                          #:navigate (lambda (sid url) 'ok)
                          #:observe (lambda (sid selector)
                                      (set! observed-with (list sid selector))
                                      (browser-observation "https://example.com"
                                                           "Test"
                                                           "text"
                                                           "vis"
...
(test-case "service browser-navigate! dispatches via adapter interface"
  ...
  (define test-adapter
    (make-browser-adapter #:open (lambda (sid target) 'ok)
                          #:close (lambda (sid) 'ok)
                          #:navigate (lambda (sid url)
                                       (set! navigated-with url)
                                       'ok)
                          #:observe (lambda (sid selector)
                                      (browser-observation "https://example.com"
                                                           "Test"
