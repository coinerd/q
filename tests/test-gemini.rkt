...
(check-true (and (hash-has-key? tool-content 'id)
                 (> (string-length (hash-ref tool-content 'id)) 0))
...
(check-equal? (length mt-contents) 4
              "multi-turn: 4 content entries")

;; Entry 1: user
(define mt-entry1 (car mt-contents))
(check-equal? (hash-ref mt-entry1 'role) "user"
              "multi-turn: first entry role is user")

;; Entry 2: assistant → model with functionCall
(define mt-entry2 (cadr mt-contents))
(check-equal? (hash-ref mt-entry2 'role) "model"
              "multi-turn: assistant → model role")
(define mt-entry2-parts (hash-ref mt-entry2 'parts))
(check-pred list? mt-entry2-parts
             "multi-turn: model parts is a list")
...
(check-equal? (hash-ref mt-fc 'name) "bash"
              "multi-turn: functionCall name preserved")
(check-equal? (hash-ref (hash-ref mt-fc 'args) 'command) "ls"
              "multi-turn: functionCall args preserved")

;; Entry 3: tool role → user with functionResponse
(define mt-entry3 (caddr mt-contents))
(check-equal? (hash-ref mt-entry3 'role) "user"
              "multi-turn: tool → user role for Gemini")
(define mt-entry3-parts (hash-ref mt-entry3 'parts))
(check-pred list? mt-entry3-parts
             "multi-turn: tool result parts is a list")
...
(check-equal? (hash-ref mt-fr 'name) "bash"
              "multi-turn: functionResponse name matches tool name")
(check-true (hash-has-key? mt-fr 'response)
             "multi-turn: functionResponse has response field")

;; Entry 4: user again
(define mt-entry4 (cadddr mt-contents))
(check-equal? (hash-ref mt-entry4 'role) "user"
              "multi-turn: fourth entry role is user")
...
(check-equal? (length gemini-mixed-parts) 2
              "gemini mixed: text + functionCall = 2 parts")

;; First part: text
(check-equal? (hash-ref (car gemini-mixed-parts) 'text) "Sure, let me run that."
...
(check-true (and (hash-has-key? tc1 'id)
                 (> (string-length (hash-ref tc1 'id)) 0))
            "gemini tool call 1 has non-empty ID")
(check-true (and (hash-has-key? tc2 'id)
                 (> (string-length (hash-ref tc2 'id)) 0))
            "gemini tool call 2 has non-empty ID")

;; IDs must be unique
(check-not-equal? (hash-ref tc1 'id) (hash-ref tc2 'id)
                  "gemini tool calls have unique IDs")

;; IDs should be prefixed for identifiability
(check-true (string-prefix? (hash-ref tc1 'id) "gemini_")
            "gemini tool call ID has 'gemini_' prefix")
(check-true (string-prefix? (hash-ref tc2 'id) "gemini_")
            "gemini tool call ID has 'gemini_' prefix")
...
(define stream-tool-deltas
  (filter (lambda (c) (stream-chunk-delta-tool-call c)) stream-dual-chunks))

(check-equal? (length stream-tool-deltas) 2
              "gemini stream: 2 tool call deltas")
...
(check-true (> (string-length (hash-ref stc1 'id)) 0)
            "gemini stream tool call 1 has non-empty ID")
(check-true (> (string-length (hash-ref stc2 'id)) 0)
            "gemini stream tool call 2 has non-empty ID")
(check-not-equal? (hash-ref stc1 'id) (hash-ref stc2 'id)
                  "gemini stream tool calls have unique IDs")
...
(define gemini-large-events
  (for/list ([i (in-range 100)])
    (hash 'candidates
          (list (hash 'content
                      (hash 'role "model"
                            'parts (list (hash 'text (format "chunk~a " i))))
                      'finishReason #f)))))

(define gemini-large-chunks (gemini-parse-stream-chunks gemini-large-events))
(define gemini-large-texts
  (filter (lambda (c) (stream-chunk-delta-text c)) gemini-large-chunks))
(check-equal? (length gemini-large-texts) 100
              "100 text deltas preserved (cons+reverse correctness)")
(check-equal? (stream-chunk-delta-text (car gemini-large-texts)) "chunk0 "
              "first chunk is chunk0")
(check-equal? (stream-chunk-delta-text (list-ref gemini-large-texts 99)) "chunk99 "
