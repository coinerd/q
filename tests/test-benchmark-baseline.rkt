...
(define (make-test-report scores)
  (generate-report (for/list ([s (in-list scores)])
  ...
  (dynamic-wind
   ...
   (lambda ()
     ...
     (call-with-output-file
      (build-path tmp "0.18.0.json")
      (lambda (out)
        (write-json (hasheq 'version "0.18.0" 'timestamp "t" 'results '() 'summary (hasheq)) out))
      #:exists 'replace)
     (call-with-output-file
      (build-path tmp "0.19.0.json")
      (lambda (out)
        (write-json (hasheq 'version "0.19.0" 'timestamp "t" 'results '() 'summary (hasheq)) out))
      #:exists 'replace)
