...
(define event-ordering-tests
  (test-suite "Event Ordering & State Consistency Tests"
    ...
    (test-case "EO2: compaction events update state cleanly"
      ...
      (define events
        (event-sequence "turn.started"
                        (hash)
                        "assistant.message.completed"
                        (hash 'content "Before compaction")
                        "compaction.warning"
                        (hash 'tokenCount 80000)
                        "compaction.started"
                        (hash)
    ...
    (test-case "EO4: events without turn.started still produce entries"
      ...
      (define events
        ...
        (event-sequence "assistant.message.completed"
