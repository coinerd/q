...
(define fsm-suite
  (test-suite "Loop FSM tests"
    ...
    (test-case "loop-fsm: stream + stream-complete -> complete"
    ...
    (test-case "invalid transition emits error"
      (check-exn exn:fail? (lambda () (next-turn-state turn-state-emit-start turn-event-hook-pass))))
