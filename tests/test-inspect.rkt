...
(define-test-suite
 ...
 (test-case "format-inspection produces non-empty string with expected keywords"
   ...
   (append-entries! path
                    (list (msg "1"
                               ...
                               (list text)
                               1000
                               (hasheq 'model "gpt-4" 'usage (hasheq 'prompt 100 'completion 50)))
                          (msg "2"
                               "1"
                               'assistant
                               'message
                               (list tc)
                               2000
                               (hasheq 'model "gpt-4" 'usage (hasheq 'prompt 200 'completion 75)))))
   (define result (inspect-session path))
 ...
 (test-case "models list deduplicates model names from meta"
   ...
   (define path (session-path dir))
   (append-entries! path
                    (list (msg "1" #f 'user 'message (list text) 1000 (hasheq 'model "gpt-4"))
                          (msg "2" "1" 'assistant 'message (list text) 1001 (hasheq 'model "gpt-4"))
                          (msg "3" "2" 'user 'message (list text) 1002 (hasheq 'model "claude-3"))))
   (define result (inspect-session path))
 ...
 (test-case "token-usage sums prompt and completion tokens"
   ...
   (append-entries!
    path
    (list
     (msg "1" #f 'user 'message (list text) 1000 (hasheq 'usage (hasheq 'prompt 100 'completion 50)))
     (msg "2"
          "1"
          'assistant
          'message
 ...
 (test-case "inspect-session-safe handles partial/corrupted sessions"
   ...
   (call-with-output-file path
                          (lambda (out)
                            (write-json (message->jsexpr (msg "1" #f 'user 'message (list text) 1000))
                                        out)
                            (newline out)
   ...
   (delete-directory/files dir #:must-exist? #f))) ;; end test-inspect
