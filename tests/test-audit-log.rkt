...
(define (with-temp-log test-thunk)
  ...
  (dynamic-wind (lambda () (void))
                ...
                (lambda ()
                  ...
                  (define dir
                    (let-values ([(base _name _must-be-dir?) (split-path tmp-path)])
                      base))
...
(test-case "audit-log! writes to temp file"
  (with-temp-log (lambda (tmp-path)
...
(test-case "audit-log! format is correct"
  (with-temp-log
   (lambda (tmp-path)
     ...
     (check-true
      (regexp-match? #rx"[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]"
                     (first parts))
...
(test-case "audit-log! appends multiple entries"
  (with-temp-log
   (lambda (tmp-path)
     ...
     (for ([line (in-list lines)])
       ...
       (check-equal? (length parts) 4 (format "each line should have 4 fields, got: ~a" parts))))))

;; ============================================================
;; Test: with-audit-log returns thunk result
;; ============================================================

(test-case "with-audit-log returns thunk result"
  (with-temp-log
   (lambda (tmp-path)
     (define result
       (with-audit-log "sess-001" 'file-read "/etc/passwd" (lambda () 42) #:log-path tmp-path))
...
(test-case "SEC-04: rotation happens when file exceeds max size"
  (let* ([tmpdir (make-temporary-file "audit-rot-~a" 'directory)]
    ...
    (call-with-output-file tmp-path
                           (lambda (out) (display (make-string 200 #\X) out))
                           #:exists 'replace)
...
(test-case "SEC-04: small entries don't trigger rotation"
