...
(test-case "PBT: hidden-name? returns #t for all dot-prefixed strings"
  (quickcheck (property ([s arbitrary-string])
                        (define dot-name (string-append "." s))
                        (==> (not (string=? s "")) (hidden-name? dot-name)))))

(test-case "PBT: hidden-name? returns #f for strings without leading dot"
  (quickcheck (property ([s arbitrary-string])
                        (==> (and (not (string=? s "")) (not (char=? (string-ref s 0) #\.)))
                             (not (hidden-name? s))))))
...
(test-case "PBT: JSONL roundtrip preserves entry count"
  (quickcheck (property ([entries (arbitrary-list arbitrary-string)])
                        (with-temp-jsonl-file (lambda (tmp-path)
                                                (for ([e entries])
                                                  (jsonl-append! tmp-path (hasheq 'data e)))
                                                (define read-back (jsonl-read-all-valid tmp-path))
                                                (==> #t (= (length read-back) (length entries))))))))

;; ============================================================
;; Property 3: glob->regexp with no wildcards matches literally
;; ============================================================

(test-case "PBT: glob->regexp with plain name matches only that name"
  (quickcheck (property ([s arbitrary-string])
...
(test-case "PBT: glob->regexp with * matches any suffix without slashes"
  (quickcheck (property ([prefix arbitrary-string] [suffix arbitrary-string])
...
(test-case "PBT: JSONL roundtrip preserves hash keys and values"
  (quickcheck (property ([s arbitrary-string])
