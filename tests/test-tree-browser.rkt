...
(define sample-nodes
  '(("id1" #f user "Hello" 1000) ("id2" "id1" assistant "Hi there" 1001)
...
(define (make-test-state-with-overlay tbs)
  (struct-copy ui-state (initial-ui-state) [active-overlay (make-test-overlay tbs)]))
...
(define tree-browser-tests
  (test-suite "Tree Browser Tests"
    ...
    (test-case "show-overlay creates tree-browser overlay"
      ...
      (define with-ov (show-overlay state 'tree-browser '(() ()) "" #:width 80 #:height 20))
