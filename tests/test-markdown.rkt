...
(define markdown-suite
  (test-suite "markdown parsing tests"
    ...
    (test-case "inline code"
      ...
      (check-equal? result
                    (list (md-token 'text "use ") (md-token 'code "foo") (md-token 'text " here"))))
    ...
    (test-case "multiple inline code segments"
      ...
      (check-equal? result (list (md-token 'code "a") (md-token 'text " and ") (md-token 'code "b"))))
    ...
    (test-case "bold within text"
      ...
      (check-equal? result
                    (list (md-token 'text "say ") (md-token 'bold "world") (md-token 'text " now"))))

    (test-case "multiple bold segments"
      (define result (parse-inline-markdown "**a** and **b**"))
      (check-equal? result (list (md-token 'bold "a") (md-token 'text " and ") (md-token 'bold "b"))))
    ...
    (test-case "italic within text"
      ...
      (check-equal?
       result
       (list (md-token 'text "say ") (md-token 'italic "world") (md-token 'text " now"))))
    ...
    (test-case "bold and italic in same line"
      ...
      (check-equal?
       result
       (list (md-token 'bold "bold") (md-token 'text " and ") (md-token 'italic "italic"))))
    ...
    (test-case "link"
      ...
      (check-equal? result (list (md-token 'link (cons "http://example.com" "click")))))

    (test-case "link within text"
      (define result (parse-inline-markdown "see [docs](url) here"))
      (check-equal?
       result
       (list (md-token 'text "see ") (md-token 'link (cons "url" "docs")) (md-token 'text " here"))))
    ...
    (test-case "single newline produces two text tokens with newline"
      ...
      (check-equal?
       result
       (list (md-token 'text "line1") (md-token 'newline "\n") (md-token 'text "line2"))))
    ...
    (test-case "link and bold in same line"
      ...
      (check-equal?
       result
       (list (md-token 'bold "bold") (md-token 'text " ") (md-token 'link (cons "url" "link")))))
