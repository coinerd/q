...
(define test-json-mode
  (test-suite "json-mode interface tests"
    ...
    (test-case "event->json-line: single line, no embedded newlines"
      (define evt
        (make-event "model.stream.delta" 5000 "sess-4" "turn-3" (hasheq 'text "hello\nworld")))
      (define line (event->json-line evt))
    ...
    (test-case "start-json-mode!: multiple events produce multiple JSON lines"
      ...
      (stop-json-mode! bus sub-id)
      (define output (get-output-string out))
      (define lines (filter (λ (l) (not (string=? l ""))) (string-split output "\n")))
    ...
    (test-case "JSONL format: one complete JSON object per line"
      ...
      (for ([line (in-list lines)]
        ...
        (check-equal? (hash-ref (hash-ref js 'payload) 'text) (format "chunk ~a" i))))
    ...
    (test-case "error event: runtime.error serializes correctly"
      (define evt
        (make-event "runtime.error"
                    ...
                    "err-sess"
                    #f
                    (hasheq 'message "something went wrong" 'code 500)))
      (define line (event->json-line evt))
    ...
    (test-case "json-mode-event-filter: accepts session events"
      (check-true (json-mode-event-filter (make-event "session.started" 1000 "s" #f (hasheq)))))

    (test-case "json-mode-event-filter: accepts model events"
      (check-true (json-mode-event-filter (make-event "model.stream.delta" 2000 "s" "t" (hasheq)))))

    (test-case "json-mode-event-filter: accepts tool events"
      (check-true (json-mode-event-filter (make-event "tool.call.started" 3000 "s" "t" (hasheq)))))

    (test-case "json-mode-event-filter: accepts error events"
      (check-true (json-mode-event-filter (make-event "runtime.error" 4000 "s" #f (hasheq)))))

    (test-case "json-mode-event-filter: accepts compaction events"
      (check-true (json-mode-event-filter (make-event "compaction.started" 5000 "s" #f (hasheq)))))
    ...
    (test-case "intent struct: transparent equality"
      (check-equal? (intent 'prompt (hasheq 'text "hi")) (intent 'prompt (hasheq 'text "hi"))))
