...
(test-case "safe-mode? is #f by default"
  (check-false (parameterize ([current-safe-mode #f])
                 (safe-mode?))))
...
(test-case "safe-mode-active! turns on and off"
  (define was-set? #f)
...
(test-case "bash blocked in safe mode, read/ls/grep/find allowed"
...
(test-case "extension tools blocked in safe mode"
  (parameterize ([current-safe-mode #t])
...
(test-case "path outside project root blocked in safe mode"
  (parameterize ([current-safe-mode #t]
...
(test-case "path inside project root allowed in safe mode"
  (parameterize ([current-safe-mode #t]
...
(test-case "trust-level is 'full when safe mode off"
  (parameterize ([current-safe-mode #f])
    (check-equal? (trust-level) 'full)))

(test-case "trust-level is 'restricted when safe mode on"
  (parameterize ([current-safe-mode #t])
...
(test-case "safe-mode-config-info returns correct hash structure"
  (parameterize ([current-safe-mode #t])
...
(test-case "Q_SAFE_MODE=1 activates safe mode, Q_SAFE_MODE=0 does not"
  (parameterize ([current-safe-mode #f])
    (with-env-var "Q_SAFE_MODE" "1" (check-true (safe-mode?) "Q_SAFE_MODE=1 activates safe mode"))
    (with-env-var "Q_SAFE_MODE"
                  "0"
...
(test-case "all tools allowed when safe mode off"
  (parameterize ([current-safe-mode #f])
...
(test-case "custom allowed-paths: child path allowed"
  (define test-root (build-path (current-directory)))
  (parameterize ([current-safe-mode #t]
                 [current-safe-mode-config (make-safe-mode-config #:active #t
                                                                  #:allowed-paths (list test-root))])
    (check-true (allowed-path? (build-path test-root "src" "foo.rkt")))))

(test-case "custom allowed-paths: sibling prefix denied"
  (define test-root (build-path "/tmp/q-safe-test-root"))
  (define sibling (build-path "/tmp/q-safe-test-root-sibling"))
  (parameterize ([current-safe-mode #t]
                 [current-safe-mode-config (make-safe-mode-config #:active #t
                                                                  #:allowed-paths (list test-root))])
    ;; /tmp/q-safe-test-root-sibling should NOT match /tmp/q-safe-test-root
    (check-false (allowed-path? sibling))))

(test-case "custom allowed-paths: exact match allowed"
  (define test-root (build-path (current-directory)))
  (parameterize ([current-safe-mode #t]
                 [current-safe-mode-config (make-safe-mode-config #:active #t
                                                                  #:allowed-paths (list test-root))])
    (check-true (allowed-path? test-root))))
