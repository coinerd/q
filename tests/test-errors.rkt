...
(define errors-tests
  (test-suite
   "errors.rkt — domain-specific exception types"

   ;; q-error struct has message and context
   (test-case "q-error has message and context"
     (define e (with-handlers ([q-error? identity])
   ...
   ;; provider-error has provider name and status-code
   (test-case "provider-error has provider and status-code"
     (define e (with-handlers ([provider-error? identity])
                 (raise-provider-error "fail" "openai" 429)))
     (check-equal? (exn-message e) "fail")
     (check-equal? (provider-error-provider e) "openai")
     (check-equal? (provider-error-status-code e) 429))

   ;; tool-error has tool-name
   (test-case "tool-error has tool-name"
     (define e (with-handlers ([tool-error? identity])
   ...
   (test-case "session-error has session-id"
     (define e (with-handlers ([session-error? identity])
   ...
   (test-case "domain errors are exn:fail?"
     (for ([raise-fn (list (lambda () (raise-q-error "x"))
                           (lambda () (raise-provider-error "x" "p"))
                           (lambda () (raise-tool-error "x" "t"))
                           (lambda () (raise-session-error "x" "s")))])
       (define e (with-handlers ([exn:fail? identity]) (raise-fn)))
   ...
   ;; provider-error status-code defaults to #f
   (test-case "provider-error status-code defaults to #f"
     (define e (with-handlers ([provider-error? identity])
                 (raise-provider-error "msg" "test-provider")))
