...
(define streaming-tool-bug-tests
  (test-suite "Streaming Tool Bug Reproduction"
    ...
    (test-case "FIXED: streaming text cleared on turn.completed when tool.call.started follows (no assistant.message.completed)"
      ...
      ;; Model streams text
      (define s2
        (apply-event-to-state s1
                              (make-test-event "model.stream.delta"
                                               (hasheq 'delta "The answer is "))))
      (check-equal? (ui-state-streaming-text s2)
                    "The answer is "
                    "delta accumulated in streaming-text")

      (define s3
        (apply-event-to-state s2 (make-test-event "model.stream.delta" (hasheq 'delta "42."))))
      ...
      (check-false (ui-state-streaming-text s6) "B2 FIXED: streaming-text cleared on turn.completed"))
    ...
    (test-case "CORRECT: streaming text committed when assistant.message.completed fires before tool calls"
      (define s0 (initial-ui-state))

      (define s1 (apply-event-to-state s0 (make-test-event "turn.started" (hasheq))))
      (define s2
        (apply-event-to-state s1 (make-test-event "model.stream.delta" (hasheq 'delta "Hello "))))
      (define s3
        (apply-event-to-state s2 (make-test-event "model.stream.delta" (hasheq 'delta "world."))))

      ;; The FIX: emit assistant.message.completed before tool.call.started
      (define s4
        (apply-event-to-state s3
                              (make-test-event "assistant.message.completed"
                                               (hasheq 'messageId "msg-1" 'content "Hello world."))))
      ...
      (check-equal? (transcript-entry-text (car (ui-state-transcript s4)))
                    "Hello world."
      ...
      (define s5
        (apply-event-to-state
         ...
         (make-test-event "tool.call.started"
                          (hasheq 'id "tc-1" 'name "read" 'arguments (hasheq 'path "/tmp/x.txt")))))
      (check-equal? (transcript-types s5)
    ...
    (test-case "BUG: streaming text accumulates across turns (turn.completed doesn't clear it)"
      ...
      (define s1 (apply-event-to-state s0 (make-test-event "turn.started" (hasheq))))
      (define s2
        (apply-event-to-state s1 (make-test-event "model.stream.delta" (hasheq 'delta "Turn1 "))))
      (define s3
        (apply-event-to-state s2
                              (make-test-event "assistant.message.completed"
                                               (hasheq 'content "Turn1 text" 'messageId "m1"))))
      ...
      (define s5
        (apply-event-to-state s4 (make-test-event "turn.started" (hasheq) #:turn-id "turn-2")))
      (define s6
        (apply-event-to-state
         s5
         (make-test-event "model.stream.delta" (hasheq 'delta "Turn2 text") #:turn-id "turn-2")))

      ;; This SHOULD be just "Turn2 text", not "Turn1 textTurn2 text"
      (check-equal? (ui-state-streaming-text s6)
                    "Turn2 text"
                    "no cross-turn accumulation after proper completion"))

    (test-case "FIXED: streaming text does not accumulate across turns when assistant.message.completed missing"
      ...
      (define s1 (apply-event-to-state s0 (make-test-event "turn.started" (hasheq))))
      (define s2
        (apply-event-to-state s1 (make-test-event "model.stream.delta" (hasheq 'delta "Leftover "))))
      (define s3
        (apply-event-to-state
         s2
         (make-test-event "tool.call.started"
      ...
      (define s6
        (apply-event-to-state
         s5
         (make-test-event "model.stream.delta" (hasheq 'delta "New text") #:turn-id "turn-2")))
