...
(define (make-snapshot-json entries)
  (hasheq 'schema
          ...
          (for/list ([e (in-list entries)])
            (hasheq 'path (car e) 'duration_seconds (cadr e)))))
...
(define scheduler-order-suite
  (test-suite "scheduler-order: deterministic FIFO/LPT ordering and duration fallback (W3)"
    ...
    (test-case "fifo with a duration snapshot still returns input order"
      ...
      (with-snapshot-tmp `((,(list-ref files 0) 10.0) (,(list-ref files 1) 1.0)
    ...
    (test-case "lpt orders by duration descending"
      (define files (list "tests/test-slow.rkt" "tests/test-fast.rkt" "tests/test-medium.rkt"))
      (with-snapshot-tmp
       `(("tests/test-slow.rkt" 30.0) ("tests/test-fast.rkt" 2.0) ("tests/test-medium.rkt" 10.0))
       (lambda (snap)
         ...
         (check-equal? ordered
                       (list "tests/test-slow.rkt" "tests/test-medium.rkt" "tests/test-fast.rkt")))))

    (test-case "lpt retains max across multiple artifacts"
      (define files (list "tests/test-a.rkt" "tests/test-b.rkt"))
      (with-snapshot-dir-tmp `(("tests/test-a.rkt" 5.0) ("tests/test-a.rkt" 15.0)
    ...
    (test-case "lpt uses stable path tie-break for equal durations"
      (define files (list "tests/zzz.rkt" "tests/aaa.rkt" "tests/mmm.rkt"))
      (with-snapshot-tmp `(("tests/zzz.rkt" 10.0) ("tests/aaa.rkt" 10.0) ("tests/mmm.rkt" 10.0))
                         (lambda (snap)
                           (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
                           (check-eq? (ordering-record-mode rec) 'lpt)
                           (define ordered (order-files files rec))
                           ...
                           (check-equal? ordered
                                         (list "tests/aaa.rkt" "tests/mmm.rkt" "tests/zzz.rkt")))))

    (test-case "lpt keeps files without duration evidence in input-relative position"
      (define files (list "tests/test-noev.rkt" "tests/test-long.rkt" "tests/test-short.rkt"))
      (with-snapshot-tmp
       `(("tests/test-long.rkt" 20.0) ("tests/test-short.rkt" 2.0))
       (lambda (snap)
         (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
         (check-eq? (ordering-record-mode rec) 'lpt)
         (define ordered (order-files files rec))
         ...
         (check-equal? ordered
                       (list "tests/test-long.rkt" "tests/test-short.rkt" "tests/test-noev.rkt")))))
    ...
    (test-case "lpt with stale snapshot → fifo fallback"
      ...
      (with-snapshot-tmp `((,(list-ref files 0) 10.0) (,(list-ref files 1) 5.0)
    ...
    (test-case "lpt with wrong-inventory snapshot → fifo fallback"
      (define files (fake-files 3))
      (with-snapshot-tmp
       `(("tests/totally-different-a.rkt" 10.0) ("tests/totally-different-b.rkt" 5.0))
       (lambda (snap)
         (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
         (check-eq? (ordering-record-mode rec) 'fifo)
    ...
    (test-case "lpt with malformed (corrupt) snapshot → fifo fallback"
      ...
      (call-with-output-file tmp (lambda (out) (display "not valid json {{{" out)) #:exists 'replace)
      (dynamic-wind (lambda () (void))
                    (lambda ()
    ...
    (test-case "ordering never adds or removes a file (lpt with snapshot)"
      (define files
        (list "tests/test-a.rkt" "tests/test-b.rkt" "tests/test-c.rkt" "tests/test-d.rkt"))
      (with-snapshot-tmp `(("tests/test-a.rkt" 30.0) ("tests/test-c.rkt" 5.0))
                         (lambda (snap)
                           (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
                           (define ordered (order-files files rec))
                           (check-equal? (length ordered) (length files))
    ...
    (test-case "ordering never duplicates a file (lpt with snapshot)"
      (define files (list "tests/test-a.rkt" "tests/test-b.rkt" "tests/test-c.rkt"))
      (with-snapshot-tmp `(("tests/test-a.rkt" 30.0) ("tests/test-b.rkt" 20.0)
    ...
    (test-case "ordering does not change the inventory digest"
      ...
      (define files
        (list "tests/test-z.rkt" "tests/test-a.rkt" "tests/test-m.rkt" "tests/test-b.rkt"))
      (with-snapshot-tmp
       `(("tests/test-z.rkt" 20.0) ("tests/test-a.rkt" 5.0))
       (lambda (snap)
         (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
         (define ordered (order-files files rec))
         (define digest-before
           (bytes->hex-string (sha256-bytes (string->bytes/utf-8 (string-join (sort files string<?)
                                                                              "\n")))))
         (define digest-after
           (bytes->hex-string (sha256-bytes (string->bytes/utf-8 (string-join (sort ordered string<?)
                                                                              "\n")))))
    ...
    (test-case "ordering does not cross the serial/parallel partition"
      ...
      (define files
        (list "tests/test-mut-1.rkt"
              "tests/test-fast-1.rkt"
              "tests/test-mut-2.rkt"
              "tests/test-fast-2.rkt"))
      (with-snapshot-tmp `(("tests/test-fast-1.rkt" 30.0) ("tests/test-mut-1.rkt" 20.0))
                         (lambda (snap)
                           (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
                           (define ordered (order-files files rec))
    ...
    (test-case "ordering does not change final result order"
      ...
      (define files (list "tests/test-c.rkt" "tests/test-a.rkt" "tests/test-b.rkt"))
      (define file-order
        (for/hash ([f (in-list files)]
                   [i (in-naturals)])
          (values f i)))
      (with-snapshot-tmp
       `(("tests/test-a.rkt" 30.0) ("tests/test-b.rkt" 10.0) ("tests/test-c.rkt" 5.0))
       (lambda (snap)
         (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
         (define ordered (order-files files rec))
         ...
         (define fake-results
           (for/list ([f (in-list ordered)]
                      [i (in-naturals)])
             (cons f i)))
         (define re-sorted (sort fake-results < #:key (lambda (r) (hash-ref file-order (car r) 0))))
    ...
    (test-case "ordering-record->jsexpr for successful lpt"
      (define files (list "tests/test-a.rkt" "tests/test-b.rkt"))
      (with-snapshot-tmp `(("tests/test-a.rkt" 10.0) ("tests/test-b.rkt" 2.0))
                         (lambda (snap)
                           (define rec (prepare-ordering files 'lpt default-max-age-seconds snap))
                           (define json-str (ordering-record->jsexpr rec))
                           (define parsed (with-input-from-string json-str read-json))
