...
(define (make-timestamped-message id parent-id role kind ts)
  (make-message id parent-id role kind (list (make-text-part "hello")) ts (hasheq)))
...
(define-test-suite
 ...
 (test-case "append-entries! writes multiple entries atomically"
   ...
   (define path (session-path dir))
   (define msgs
     (list (make-test-message "id1" #f 'user 'message)
 ...
 (test-case "roundtrip with tool-call and tool-result content parts"
   ...
   (define m1
     (make-message "id1"
                   ...
                   'assistant
                   'message
                   (list (make-tool-call-part "tc1" "read" (hasheq 'path "/tmp/x.txt")))
                   1000
                   (hasheq)))
   (define m2
     (make-message
      ...
      'tool
      'tool-result
      (list (make-tool-result-part "tc1" (list (hasheq 'type "text" 'text "file contents")) #f))
      1001
      (hasheq)))
 ...
 (test-case "two entries with same parentId form a fork"
   ...
   (check-equal? (message-parent-id (second replayed)) (message-parent-id (third replayed)))
   (delete-directory/files dir #:must-exist? #f))
 ;; ══════════════════════════════════════════════════════════
 ...
 (test-case "write-ahead marker mechanism: marker path and creation"
   ...
   (call-with-output-file marker (lambda (out) (display 1 out)) #:mode 'text #:exists 'truncate)
 ...
 (test-case "repair-session-log! preserves entry order"
   ...
   (call-with-output-file path (lambda (out) (displayln "GARBAGE" out)) #:mode 'text #:exists 'append)
 ...
 (test-case "repair-session-log! cleans up pending marker"
   ...
   (call-with-output-file marker (lambda (out) (display 1 out)) #:mode 'text #:exists 'truncate)
   (call-with-output-file path (lambda (out) (displayln "BAD" out)) #:mode 'text #:exists 'append)
   (check-true (has-pending-marker? path))
