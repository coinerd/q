...
(test-case "find-previous-file-tracker finds tracker from summary metadata"
  (define prev-summary
    (make-message "compaction-1"
                  ...
                  (current-seconds)
                  (hasheq 'fileTracker
                          (hasheq 'readFiles '("prev.rkt") 'modifiedFiles '("mod.rkt")))))
...
(test-case "find-previous-file-tracker uses most recent summary"
  ...
  (define s2
    (make-message "c2"
                  ...
                  2000
                  (hasheq 'fileTracker
                          (hasheq 'readFiles '("new.rkt") 'modifiedFiles '("changed.rkt")))))
  ...
  (define msgs
    (list
     ...
     (make-message "m2"
                   ...
                   'assistant
                   'message
                   (list (make-tool-call-part "tc1" "read" (hasheq 'path "new.rkt")))
                   1002
                   (hasheq))
     (make-message "m3"
                   #f
                   'user
                   'message
                   (list (make-text-part "recent user msg text content"))
                   1003
                   (hasheq))))
