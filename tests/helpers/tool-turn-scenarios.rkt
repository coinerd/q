...
(struct turn-scenario (name provider-responses tools expected-events) #:transparent)

(define (turn-scenario-text text)
  "A simple text turn with no tool calls."
  (turn-scenario "text-only" (list (scenario-text text)) '() '(response)))
...
(define (turn-scenario-tool-call tool-name #:result [result "ok"])
  ...
  (turn-scenario
   ...
   (list (make-mock-tool tool-name #:handler (lambda (args ctx) (make-tool-result result (hash) #f))))
   '(tool-call tool-result response)))

(define (turn-scenario-multi-tool tool-names)
  "A multi-tool-call turn."
  (turn-scenario "multi-tool"
                 (list (scenario-multi-tool (for/list ([n (in-list tool-names)])
                                              (list n (hash))))
...
(define (turn-scenario-malformed-tool)
  ...
  (turn-scenario
   "malformed-tool"
   (list (make-model-response (list (hash 'type "tool-call" 'id "tc-bad" 'name "" 'arguments (hash)))
...
(define (check-tool-result-content result expected-content)
  ...
  (and (tool-result? result) (equal? (tool-result-content result) expected-content)))
