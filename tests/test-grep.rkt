...
(require (only-in "../tools/tool.rkt" tool-result-content tool-result-details tool-result-is-error?))
  ...
  (test-suite "grep-tool"
    ...
    (test-case "single file with matches"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-grep (hasheq 'pattern "beta" 'path (path->string f))))
    ...
    (test-case "single file with no matches"
      (with-temp-dir (λ (dir)
                       (define f (build-path dir "sample.txt"))
                       (write-string-to-file f "alpha\ngamma\ndelta\n")
                       (define r (tool-grep (hasheq 'pattern "xyz" 'path (path->string f))))
                       (check-false (result-is-error? r))
                       (define d (result-details r))
    ...
    (test-case "case-insensitive search"
      (with-temp-dir
       (λ (dir)
         ...
         (define r (tool-grep (hasheq 'pattern "hello" 'path (path->string f) 'case-insensitive? #t)))
         (check-false (result-is-error? r))
         (define d (result-details r))
    ...
    (test-case "directory search with glob filter"
      (with-temp-dir
       (λ (dir)
         ...
         (define r (tool-grep (hasheq 'pattern "findme" 'path (path->string dir) 'glob "*.txt")))
         (check-false (result-is-error? r))
         (define d (result-details r))
    ...
    (test-case "context lines around matches"
      (with-temp-dir
       (λ (dir)
         ...
         (define r (tool-grep (hasheq 'pattern "TARGET" 'path (path->string f) 'context-lines 1)))
         (check-false (result-is-error? r))
         (define c (string-join (result-content r) "\n"))
    ...
    (test-case "max-results limits output"
      (with-temp-dir
       (λ (dir)
         ...
         (define r
           (tool-grep
            (hasheq 'pattern "match" 'path (path->string f) 'max-results 3 'context-lines 0)))
         (check-false (result-is-error? r))
         (define d (result-details r))
         (check-equal? (hash-ref d 'total-matches) 7)
         (check-true (hash-ref d 'truncated?))
         (define match-lines
           (filter (lambda (l) (regexp-match? #rx"match line" l)) (result-content r)))
    ...
    (test-case "binary file is skipped silently"
      (with-temp-dir (λ (dir)
                       ...
                       (call-with-output-file f
                                              ...
                                              #:exists 'replace)
                       (define r (tool-grep (hasheq 'pattern "anything" 'path (path->string f))))
                       (check-false (result-is-error? r))
                       (define d (result-details r))
                       (check-equal? (hash-ref d 'total-matches) 0)
                       (check-equal? (hash-ref d 'files-with-matches) 0)
    ...
    (test-case "hidden files and directories are skipped"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-grep (hasheq 'pattern "findme" 'path (path->string dir))))
                       (check-false (result-is-error? r))
                       (define d (result-details r))
                       (check-equal? (hash-ref d 'total-matches) 1)
                       (define c (string-join (result-content r) "\n"))
    ...
    (test-case "regex pattern works"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-grep (hasheq 'pattern "foo[0-9]+bar" 'path (path->string f))))
                       (check-false (result-is-error? r))
                       (define d (result-details r))
                       (check-equal? (hash-ref d 'total-matches) 3))))
    ...
    (test-case "recursive directory search finds nested files"
      (with-temp-dir (λ (dir)
                       ...
                       (define r (tool-grep (hasheq 'pattern "target" 'path (path->string dir))))
                       (check-false (result-is-error? r))
                       (define d (result-details r))
                       (check-equal? (hash-ref d 'total-matches) 3)
    ...
    (test-case "context-lines 0 means no context"
      (with-temp-dir
       (λ (dir)
         ...
         (define r (tool-grep (hasheq 'pattern "MATCH" 'path (path->string f) 'context-lines 0)))
         (check-false (result-is-error? r))
         (define c (string-join (result-content r) "\n"))
