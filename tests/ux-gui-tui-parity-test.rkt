...
(define (tui-script->state script)
  (for/fold ([st (make-composer-state)])
            ([op (in-list script)])
    (match-op st op)))

(define (gui-script->draft script)
  (for/fold ([d (make-gui-draft)])
            ([op (in-list script)])
    (match-op d op)))
...
(define (match-op st op)
  (cond
    ...
    [(string? op) (if (string? st) (gui-draft-update st op) (composer-replace-buffer st op))]
    [(eq? op 'newline)
     (if (composer-state? st) (input-newline st) (gui-draft-insert-newline st))]
    [(eq? op 'clear)
     (if (composer-state? st) (make-composer-state) (make-gui-draft))]
    [else st]))
...
(test-case
 "draft-text-parity: identical edit scripts yield identical text"
 (define scripts
   `(("hello world")
...
(test-case
 "draft-update-parity: incremental edits stay in lockstep"
...
(test-case
 "artifact-identity-parity: toggle-detail intents are equal across frontends"
 ...
 (check-equal? (toggle-detail-intent "turn-7")
               (toggle-detail-intent "turn-7")))

;; ---------------------------------------------------------------------------
;; 3. Same folding state transitions (disclosure)
;; ---------------------------------------------------------------------------

(require "disclosure-state-test.rkt" racket/port)
...
(test-case
 "submit-result-parity: same input + intent -> same submitted text"
...
(test-case
 "newline-result-parity: newline policy is shared, not per-frontend"
...
(test-case
 "shortcut-intent-parity: physical key -> named intent is frontend-neutral"
 (define cases
 ...
 (for ([c (in-list cases)])
   ...
   (check-equal? (gui-key->intent k #:shift? s? #:control? ctl? #:alt? alt?)
                 (let ([r (resolve-key->intent k #:shift? s? #:control? ctl? #:alt? alt?)])
                   (and r (hash-ref (hash 'ui.composer.submit (make-composer-submit-intent "")
                                          ...
                                          'ui.transcript.toggle-detail (make-toggle-detail-intent #f))
                                    r
                                    #f))))))

(test-case
 "preference-surface-parity: same knobs exist for both frontends"
 (define prefs (default-preferences))
 (for ([key (in-list '(reasoning-visibility preview-length composer-max-rows
                                            submit-key newline-keys keybindings))])
   (check-not-false (preferences-ref prefs key #f)
                    (format "missing shared preference: ~a" key)))
...
(test-case
 "legacy-single-line-contract-preserved"
 (check-true (input-key-should-submit? #\return #f #f))
 (check-false (input-key-should-submit? #\return #t #f))
