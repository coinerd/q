...
(define suite
  (test-suite "Tool-read workflow tests"
    ...
    (test-case "wf-tool-read: agent reads file and explains content"
      ...
      (define prov
        (make-scripted-provider
         (list (tool-call-response "tc-1" "read" (hash 'path "src/hello.rkt"))
      ...
      (define wr
        (run-workflow prov
                      "Read src/hello.rkt"
                      #:tools reg
                      #:files (list (cons "src/hello.rkt" "#lang racket\n(displayln \"hello\")"))))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("read"))
                    #t
      ...
      (check-true (>= (length assistant-entries) 2) "expected at least 2 assistant messages")
    ...
    (test-case "wf-tool-read: read tool handles empty path gracefully"
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "read" (hash 'path ""))
       ...
       (make-tool
        ...
        (lambda (args ctx)
          ...
          (if (string=? path "")
              ...
              (make-success-result (list (hasheq 'type "text" 'text "file content")))))))

      (define wr (run-workflow prov "Read nothing" #:tools reg #:files '()))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("read")) #t)
