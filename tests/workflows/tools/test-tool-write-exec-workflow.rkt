...
(define suite
  (test-suite "Tool-write+exec workflow tests"
    ...
    (test-case "wf-tool-write-exec: write file then run bash command"
      ...
      (define prov
        (make-scripted-provider
         (list (tool-call-response
                "tc-1"
                "write"
                (hash 'path "tests/test-hello.rkt" 'content "#lang racket\n(check-equal? 1 1)"))
               (tool-call-response "tc-2" "bash" (hash 'command "raco test tests/test-hello.rkt"))
      ...
      (register-tool!
       ...
       (make-tool
        ...
        (lambda (args ctx)
          ...
          (make-success-result (format "Wrote ~a bytes to ~a" (string-length content) path)))))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("write" "bash"))
                    #t
      ...
      (when write-dir
        ...
        (when (file-exists? written-file)
          ...
          (check-true (string-contains? actual-content "check-equal? 1 1")
                      (format "written file should contain test content, got: ~a" actual-content))))
    ...
    (test-case "wf-tool-write-exec: write error is handled gracefully"
      (define prov
        (make-scripted-provider
         (list (tool-call-response "tc-1" "write" (hash 'path "/forbidden/path.rkt" 'content "data"))
      ...
      (define wr (run-workflow prov "Write to forbidden path" #:tools reg #:files '()))
      ...
      (define recorder (workflow-result-events wr))
      (define tc-started (events-of-type recorder "tool.call.started"))
      (check-true (>= (length tc-started) 1) "expected at least 1 tool.call.started")
