...
(define (make-edit-tool)
  (make-tool
   ...
   (lambda (args ctx)
     ...
     (cond
       [(not (file-exists? full-path)) (make-error-result (format "File not found: ~a" path))]
       [else
        (define content (file->string full-path))
        (cond
          [(not (string-contains? content old-text))
           (make-error-result (format "Old text '~a' not found in ~a" old-text path))]
          [else
           (define replaced (string-replace content old-text new-text))
           (call-with-output-file full-path (lambda (out) (display replaced out)) #:exists 'replace)
           (make-success-result
            (format "Replaced '~a' with '~a' in ~a" old-text new-text path))])]))))
...
(define suite
  (test-suite "Tool-edit workflow tests"
    ...
    (test-case "wf-tool-edit: agent edits config file content"
      (define prov
        (make-scripted-provider
         (list (tool-call-response "tc-1"
                                   "edit"
                                   (hash 'path "src/config.rkt" 'old-text "8080" 'new-text "9090"))
      ...
      (define wr
        (run-workflow prov
                      ...
                      #:tools reg
                      #:files (list (cons "src/config.rkt" "(define port 8080)"))))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("edit"))
                    #t
    ...
    (test-case "wf-tool-edit: edit on missing file returns error gracefully"
      (define prov
        (make-scripted-provider
         (list (tool-call-response "tc-1"
                                   "edit"
                                   (hash 'path "nonexistent.rkt" 'old-text "foo" 'new-text "bar"))
               (text-response "The file does not exist"))))

      (define reg (make-tool-registry))
      (register-tool! reg (make-edit-tool))

      (define wr (run-workflow prov "Edit the missing file" #:tools reg #:files '()))
      ...
      ;; SIDE-EFFECTS: tool sequence is ("edit")
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("edit")) #t)
    ...
    (test-case "wf-tool-edit: old-text not in file returns error"
      (define prov
        (make-scripted-provider
         (list (tool-call-response
                "tc-1"
                "edit"
                (hash 'path "src/data.rkt" 'old-text "nonexistent-string" 'new-text "replacement"))
      ...
      (when project-dir
        ...
        (when (file-exists? data-file)
          ...
          (check-equal? content "(define x 42)" "file should be unchanged when old-text not found")))
