...
(define (setup-bounded-project files)
  ...
  (for ([f (in-list files)])
    ...
    (call-with-output-file full-path (lambda (out) (display (cdr f) out)) #:exists 'replace))
...
(define (make-bounded-read-tool project-root)
  (make-tool
   ...
   (lambda (args ctx)
     ...
     (cond
       [(not path) (make-error-result "missing path argument")]
       [(and (string? path)
             (or (not (absolute-path? path)) (string-prefix? path (path->string project-root))))
        (make-success-result (list (hasheq 'type "text" 'text (format "Contents of ~a" path))))]
       [else
        (make-error-result (format "SAFE-MODE: path '~a' is outside project root '~a'"
                                   path
  ...
  (make-tool
   ...
   (lambda (args ctx)
     ...
     (cond
       ...
       [(and (string? path)
             (or (not (absolute-path? path)) (string-prefix? path (path->string project-root))))
        (make-success-result (list (hasheq 'type "text" 'text (format "Wrote to ~a" path))))]
       [else
        (make-error-result (format "SAFE-MODE: path '~a' is outside project root '~a'"
                                   path
                                   (path->string project-root)))]))))
...
(define (has-error-result? msg)
  (for/or ([part (in-list (message-content msg))])
    (and (tool-result-part? part) (tool-result-part-is-error? part))))
...
(define safe-mode-boundary-tests
  (test-suite "Safe-Mode Boundary Workflow"
      ...
      (with-handlers ([exn:fail? (lambda (e)
        ...
        (define provider
          (make-scripted-provider
           (list (tool-call-response "tc-1" "read" (hasheq 'path "/etc/passwd"))
        ...
        (check-eq? (check-session-jsonl-valid log-path) #t "Session log should be valid JSONL")

        ;; ── Outcome: tool results contain errors for out-of-bounds ──
        (define tool-result-entries (entries-with-role entries 'tool))
        (check-equal? (length tool-result-entries) 1 "Should have 1 tool-result entry")
        (when (= (length tool-result-entries) 1)
          (check-true (has-error-result? (car tool-result-entries)) "Tool result should be an error"))

        ;; ── Durable state: session log records tool-call + error result ──
        (check >=
               (length (entries-with-role entries 'assistant))
               1
    ...
    (test-case "out-of-bounds write tool-call returns error result"
      ...
      (with-handlers ([exn:fail? (lambda (e)
        ...
        (define provider
          (make-scripted-provider
           (list (tool-call-response "tc-1"
                                     "write"
                                     (hasheq 'path "/tmp/malicious.rkt" 'content "(rm -rf /)"))
    ...
    (test-case "in-bounds access succeeds while out-of-bounds fails"
      ...
      (with-handlers ([exn:fail? (lambda (e)
        ...
        (define provider
          (make-scripted-provider
           (list (tool-call-response "tc-1" "read" (hasheq 'path "safe.txt"))
                 (tool-call-response "tc-2" "read" (hasheq 'path "/etc/shadow"))
                 (text-response "Done"))))
        ...
        (check-eq? (check-session-jsonl-valid log-path) #t "Session log should be valid")

        ;; 2 tool-result entries: first success, second error
        (define tool-result-entries (entries-with-role entries 'tool))
        (check >= (length tool-result-entries) 2 "Should have at least 2 tool-result entries")
