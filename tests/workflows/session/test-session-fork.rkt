...
(define session-fork-tests
  (test-suite "Session Fork Workflow"
    ...
    (test-case "fork creates clean branch from source session"
      ...
      (define provider
        (make-scripted-provider (list (text-response "First response")
      ...
      (with-handlers ([exn:fail? (lambda (e)
        ...
        (check-equal? (length (entries-with-role source-entries-before 'user))
                      2
                      "Source should have 2 user entries before fork")
        (check-equal? (length (entries-with-role source-entries-before 'assistant))
                      2
                      "Source should have 2 assistant entries before fork")

        ;; 4. Fork the session
        (define forked-rt (sdk:fork-session! rt-after-2))
        (check-pred (lambda (x) (not (eq? x 'no-active-session)))
                    forked-rt
        ...
        (check-not-equal? forked-sid sid "Forked session must have a different session ID")
        ...
        (check-equal? (length forked-entries)
                      source-entry-count-before
        ...
        (check-equal? (length source-entries-after)
                      source-entry-count-before
        ...
        (check >
               (length forked-entries-after)
               source-entry-count-before
               "Forked session should have more entries after new prompt")
        (check-equal? (length (entries-with-role forked-entries-after 'user))
                      3
                      "Forked session should have 3 user entries")
        (check-equal? (length (entries-with-role forked-entries-after 'assistant))
                      3
                      "Forked session should have 3 assistant entries")

        ;; 10. Session tree structure valid in both
        (check-eq? (check-session-tree-structure source-log-path)
                   #t
                   "Source session tree structure should be valid")
        (check-eq? (check-session-tree-structure forked-log-path)
                   #t
                   "Forked session tree structure should be valid")

        ;; 11. Both logs are valid JSONL
        (check-eq? (check-session-jsonl-valid source-log-path)
                   #t
                   "Source session log should be valid JSONL")
        (check-eq? (check-session-jsonl-valid forked-log-path)
                   #t
    ...
    (test-case "source session remains usable after fork"
      ...
      (define provider
        (make-scripted-provider
         (list (text-response "First") (text-response "Second") (text-response "Third on source"))))
      ...
      (with-handlers ([exn:fail? (lambda (e)
        ...
        (check-equal? (length (entries-with-role entries 'user))
                      2
                      "Source should have 2 user entries after fork + extra prompt")
        (check-equal? (length (entries-with-role entries 'assistant))
                      2
