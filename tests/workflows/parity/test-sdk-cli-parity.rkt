...
(define suite
  (test-suite "SDK-CLI parity workflow tests"
    ...
    (test-case "wf-parity: single-shot SDK run matches CLI expectations"
      ...
      (define prov (make-scripted-provider (list (text-response "Hello from the agent"))))
    ...
    (test-case "wf-parity: tool-call run produces same trace via SDK"
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "echo" (hash 'message "test-parity"))
    ...
    (test-case "wf-parity: session IDs follow consistent format"
      (define prov (make-scripted-provider (list (text-response "ok"))))
      ...
      (check-true (string-contains? (path->string log-path) sid) "log path should contain session ID")
