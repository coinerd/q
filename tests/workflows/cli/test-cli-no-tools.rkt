...
(define suite
  (test-suite "CLI no-tools / restricted tools workflow tests"
    ...
    (test-case "wf-no-tools: tool-call with empty registry produces error"
      ...
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "bash" (hash 'command "ls")))))
    ...
    (test-case "wf-no-tools: plain text with no tools completes normally"
      (define prov (make-scripted-provider (list (text-response "No tools needed"))))
      ...
      (check-equal? (length tool-events)
                    0
    ...
    (test-case "wf-no-tools: tool-call for registered tool executes successfully"
      ...
      (define echo-tool
        (make-tool "echo"
                   "Echo back the input"
                   (hasheq 'type "object" 'properties (hasheq "message" (hasheq 'type "string")))
                   (lambda (args ctx)
                     (define msg
                       (or (hash-ref args "message" #f) (hash-ref args 'message "no message")))
                     (make-success-result (list (hasheq 'type "text" 'text msg))))))
      (register-tool! reg echo-tool)

      ;; Provider: first returns tool-call, then returns text summary
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "echo" (hash 'message "hello world"))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("echo")) #t)
    ...
    (test-case "wf-no-tools: tool-call for unknown tool produces error not crash"
      ...
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "bash" (hash 'command "rm -rf /"))
