...
(define suite
  (test-suite "CLI single-shot workflow tests"
    ...
    (test-case "wf-cli: single-shot prompt returns text and writes session log"
      ...
      (define prov (make-scripted-provider (list (text-response "The file contains hello world"))))
    ...
    (test-case "wf-cli: session-info returns correct data after single prompt"
      (define prov (make-scripted-provider (list (text-response "42"))))
    ...
    (test-case "wf-cli: event recorder captured model.stream.delta events"
      (define prov (make-scripted-provider (list (text-response "Hello from the model"))))
      ...
      (for ([evt (in-list delta-events)])
        ...
        (check-true (hash? payload) "model.stream.delta payload should be a hash")
        ;; Payload should contain either 'delta (text) or 'delta-tool-call
        (check-true (or (hash-has-key? payload 'delta) (hash-has-key? payload 'delta-tool-call))
    ...
    (test-case "wf-cli: session log has assistant message with expected text"
      ...
      (define prov (make-scripted-provider (list (text-response expected-text))))
      ...
      (check-true (>= (length text-parts) 1) "assistant message should have at least one text part")
