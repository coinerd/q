...
(define suite
  (test-suite "CLI session resume workflow tests"
    ...
    (test-case "wf-resume: multi-turn preserves history with 2 prompts"
      ...
      (define prov
        (make-scripted-provider (list (text-response "First answer")
                                      (text-response "Second answer"))))
      (define wr (run-workflow-multi-turn prov (list "First question" "Second question")))
    ...
    (test-case "wf-resume: explicit session resume preserves full history"
      ...
      (define prov1 (make-scripted-provider (list (text-response "Paris"))))
      (define reg (make-tool-registry))
      (define rt1
        (sdk:make-runtime #:provider prov1
      ...
      (define prov2 (make-scripted-provider (list (text-response "Berlin"))))
      (define rt4
        (sdk:make-runtime #:provider prov2
                          #:session-dir session-dir
                          #:tool-registry reg
                          #:event-bus bus2))
      ...
      (check-equal? (check-session-contains-turns log-path #:user-turns 2 #:assistant-turns 2) #t)
    ...
    (test-case "wf-resume: multi-turn event sequence has two turn cycles"
      (define prov (make-scripted-provider (list (text-response "alpha") (text-response "beta"))))
      (define wr (run-workflow-multi-turn prov (list "prompt 1" "prompt 2")))
      ...
      (check-equal? (length turn-completed) 2 "expected 2 turn.completed events in 2-turn workflow")
