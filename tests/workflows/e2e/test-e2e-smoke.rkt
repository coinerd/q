...
(define suite
  (test-suite "E2E smoke tests"
    ...
    (test-case "e2e-smoke: agent responds to simple prompt"
      ...
      (define prov (make-scripted-provider (list (text-response "I can help"))))
    ...
    (test-case "e2e-smoke: agent uses tool then responds"
      ...
      (define prov
        (make-scripted-provider (list (tool-call-response "tc-1" "list" (hash 'dir "."))
      ...
      ;; Run workflow
      (define wr (run-workflow prov "List files" #:tools reg))
      ...
      (check-equal? (check-session-tool-sequence (workflow-result-session-log wr) '("list"))
                    #t
    ...
    (test-case "e2e-smoke: multi-turn conversation maintains session"
      ...
      (define wr (run-workflow-multi-turn prov (list "First prompt" "Second prompt")))

      ;; DURABLE STATE: session log is valid
      (check-equal? (check-session-jsonl-valid (workflow-result-session-log wr)) #t)

      ;; DURABLE STATE: at least 2 user turns
      (check-equal? (check-session-contains-turns (workflow-result-session-log wr) #:user-turns 2)
                    #t
