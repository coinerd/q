...
(define ansi-suite
  (test-suite "ansi tests"
    ...
    (test-case "styled with all known style symbols does not error"
      (define result
        (styled "ok"
                '(bold dim italic underline inverse red green yellow blue magenta cyan white gray)))
      (if (color-enabled?)
          (begin
    ...
    (test-case "styled and ansi behave consistently with color state"
      ...
      (check-equal? (string-contains? styled-result "\x1b[") (string-contains? ansi-result "\x1b[")))

    (test-case "ansi-reset and styled agree on color state"
      (define reset-result (ansi-reset))
      (define styled-result (styled "test" '(red)))
      (check-equal? (string=? reset-result "") (string=? styled-result "test")))))
