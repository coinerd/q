...
(test-case "inject-hook-event returns pass when no handler registered"
  (with-extension-test (make-test-extension #:name "no-handler")
...
(test-case "check-hook-passed succeeds on pass result"
  (with-extension-test (make-test-extension #:name "pass-ext"
...
(test-case "check-hook-amended succeeds on amend result"
  (with-extension-test (make-test-extension #:name "amend-ext"
...
(test-case "check-hook-blocked succeeds on block result"
  (with-extension-test (make-test-extension #:name "block-ext"
...
(test-case "check-hook-payload succeeds with correct payload"
  (with-extension-test (make-test-extension #:name "payload-ext"
                                            #:hooks (hasheq 'context
                                                            (λ (p) (hook-amend (list 'modified p)))))
...
(test-case "make-test-extension with custom hooks"
  (define ext
    (make-test-extension #:name "hooked" #:hooks (hasheq 'turn-start (λ (p) (hook-pass p)))))
...
(test-case "capture-hook-results returns empty list for no payloads"
  (with-extension-test (make-test-extension #:name "empty-cap")
...
(test-case "registry isolation: first test's extensions don't leak"
  (with-extension-test (make-test-extension #:name "isolated-a")
...
(test-case "registry isolation: second test has its own registry"
  (with-extension-test (make-test-extension #:name "isolated-b")
