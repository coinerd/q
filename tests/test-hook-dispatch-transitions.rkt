...
(define (make-ext-reg-with-2 hook-point
                             handler1
                             handler2
                             #:name1 [name1 "ext-a"]
                             #:name2 [name2 "ext-b"])
...
(define hook-dispatch-tests
  (test-suite "Extension Hook Dispatch Transition Tests"

    ;; ============================================================
    ;; E1: Hook timeout on slow handler returns error-default
    ;; ============================================================
    (test-case "E1: hook timeout on slow handler returns error-default"
      ...
      (parameterize ([current-hook-timeout-ms 10])
        ...
        (check-equal? (hook-result-action result) 'pass "slow advisory hook returns pass on timeout")
    ...
    ;; ============================================================
    (test-case "E2: critical hook timeout returns block default"
      (parameterize ([current-hook-timeout-ms 10])
        (define (slow-handler payload)
          (sleep 0.1)
          (hook-amend "should not reach"))
    ...
    ;; ============================================================
    (test-case "E3: advisory hook timeout returns pass default"
      (parameterize ([current-hook-timeout-ms 10])
        (define (slow-handler payload)
          (sleep 0.1)
        ...
        (check-equal? (hook-result-action result) 'pass "slow advisory hook returns pass on timeout")
        (check-equal? (hook-result-payload result) "data" "original payload preserved")))

    ;; ============================================================
    ;; E4: Hook exception on critical → block
    ;; ============================================================
    (test-case "E4: hook exception on critical hook returns block"
      (define (failing-handler payload)
        (error "handler boom!"))
      ;; 'tool-call is critical
      (define reg (make-ext-reg-with 'tool-call failing-handler))
      (define result (dispatch-hooks 'tool-call "payload" reg))
      (check-equal? (hook-result-action result) 'block "critical hook exception returns block"))

    ;; ============================================================
    ;; E5: Hook exception on advisory → pass
    ;; ============================================================
    (test-case "E5: hook exception on advisory hook returns pass"
      (define (failing-handler payload)
        (error "handler boom!"))
      ;; 'message-end is advisory
      (define reg (make-ext-reg-with 'message-end failing-handler))
      (define result (dispatch-hooks 'message-end "payload" reg))
      (check-equal? (hook-result-action result) 'pass "advisory hook exception returns pass")
      (check-equal? (hook-result-payload result)
                    "payload"
    ...
    ;; ============================================================
    (test-case "E6: multiple handlers — first amends, second blocks → block wins"
      (define call-log (box '()))
      ...
      (define result (dispatch-hooks 'message-end "original" reg))
      (check-equal? (hook-result-action result) 'block "block from second handler wins")
      ;; Both handlers should have been called
      (check-equal? (length (unbox call-log)) 2 "both handlers were called"))

    ;; ============================================================
    ;; E7: Multiple handlers: first blocks → second never called
    ;; ============================================================
    (test-case "E7: multiple handlers — first blocks → short-circuit"
      (define call-log (box '()))
      (define (blocker payload)
        (set-box! call-log (cons 'block (unbox call-log)))
      ...
      (define result (dispatch-hooks 'tool-call "payload" reg))
      (check-equal? (hook-result-action result) 'block "first handler blocks")
      (check-equal? (unbox call-log) '(block) "second handler never called"))

    ;; ============================================================
    ;; E8: dispatch-hooks with zero handlers returns hook-pass
    ;; ============================================================
    (test-case "E8: dispatch-hooks with zero handlers returns hook-pass"
      (define reg (make-extension-registry))
      ;; No extensions registered
      (define result (dispatch-hooks 'message-end "payload" reg))
      (check-equal? (hook-result-action result) 'pass "zero handlers returns pass")
      (check-equal? (hook-result-payload result) "payload" "original payload returned with pass"))))
