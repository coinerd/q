...
(define test-tui-utf8
  (test-suite "TUI UTF-8 Support (BUG-28 accumulator-based)"
    ...
    (test-case "reassemble-utf8-chars decodes ä (2-byte)"
      (define result (reassemble-utf8-chars (list (integer->char #xC3) (integer->char #xA4))))
      (check-equal? result #\ä))

    (test-case "reassemble-utf8-chars decodes ü (2-byte)"
      (define result (reassemble-utf8-chars (list (integer->char #xC3) (integer->char #xBC))))
      (check-equal? result #\ü))

    (test-case "reassemble-utf8-chars decodes 日 (3-byte)"
      (define result
        (reassemble-utf8-chars (list (integer->char #xE6) (integer->char #x97) (integer->char #xA5))))
      (check-equal? result #\日))

    (test-case "reassemble-utf8-chars decodes emoji (4-byte)"
      (define result
        (reassemble-utf8-chars
         (list (integer->char #xF0) (integer->char #x9F) (integer->char #x98) (integer->char #x80))))
      (check-equal? result #\😀))
...
(define cursor-visibility-tests
  (test-suite "Cursor Visibility (BUG-33)"
...
(define real-stdin-utf8-tests
  (test-suite "BUG-41: real-stdin-read-msg UTF-8 Decoding"
...
(define mouse-decode-tests
  (test-suite "Mouse Event Decoding (ESC[M)"

    (test-case "mouse: ESC[M decodes as tmousemsg"
      (define mouse-bytes (bytes #x1b #x5b #x4d #x40 #x41 #x42))
      (define msg
        (parameterize ([current-input-port (open-input-bytes mouse-bytes)])
          (tui-read-key #:timeout 0.05)))
    ...
    (test-case "mouse: incomplete sequence returns escape"
      ...
      (check-true (or (tkeymsg? msg) (not msg)) "mouse: incomplete sequence returns escape or #f"))
