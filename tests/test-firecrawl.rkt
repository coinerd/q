...
(define firecrawl-tests
  (test-suite "Firecrawl Tool Tests"
    ...
    (test-case "search without query returns error"
      (with-handlers ([exn:fail? (lambda (e)
                                   (check-true (string-contains? (exn-message e) "query")
                                               (format "Expected query error, got: ~a"
                                                       (exn-message e))))])
    ...
    (test-case "scrape without url returns error"
      (with-handlers ([exn:fail? (lambda (e)
                                   (check-true (string-contains? (exn-message e) "url")
                                               (format "Expected url error, got: ~a"
                                                       (exn-message e))))])
    ...
    (test-case "valid action values accepted"
      (for ([action '("search" "scrape" "crawl" "map")])
        (check-not-false (valid-action? action) (format "~a should be a valid action" action))))
    ...
    (test-case "API request without key gives clear error"
      ...
      (with-handlers ([exn:fail? (lambda (e) (set! caught-error (exn-message e)))])
...
(define ssrf-tests
  (test-suite "SSRF Protection Tests"

    (test-case "ftp:// URL rejected (bad scheme)"
      (check-exn #rx"Blocked: URL scheme must be http or https"
    ...
    (test-case "http://localhost rejected (private host)"
      (check-exn #rx"Blocked: URL points to private/internal address"
    ...
    (test-case "https://example.com passes validation"
      (check-not-exn (lambda () (validate-url "https://example.com/page"))))

    (test-case "http://example.com passes validation"
      (check-not-exn (lambda () (validate-url "http://example.com/page"))))
    ...
    (test-case "scrape action returns error result for localhost URL"
      (define r (tool-firecrawl (hasheq 'action "scrape" 'url "http://localhost:8080/secret")))
      (check-pred tool-result? r)
      (check-pred tool-result-is-error? r)
      (check-true (string-contains? (content-text r) "Blocked")))

    (test-case "map action returns error result for private IP"
      (define r (tool-firecrawl (hasheq 'action "map" 'url "http://10.0.0.1/internal")))
      (check-pred tool-result? r)
      (check-true (tool-result-is-error? r)))

    (test-case "crawl action returns error result for loopback"
      (define r (tool-firecrawl (hasheq 'action "crawl" 'url "http://127.0.0.1/api")))
      (check-pred tool-result? r)
      (check-true (tool-result-is-error? r)))))
...
(define poll-deadline-tests
  (test-suite "poll-crawl-status deadline guard"
