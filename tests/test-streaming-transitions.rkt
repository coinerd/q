...
(define streaming-transition-tests
  (test-suite "TUI Streaming State Transitions"
    ...
    (test-case "streaming text committed when assistant.message.completed fires before tool.call.started"
      ...
      (define events
        (list (make-test-event "turn.started" (hasheq))
              ...
              (make-test-event
               "tool.call.started"
               (hasheq 'id "tc-1" 'name "read" 'arguments (hasheq 'path "/tmp/test.txt")))))
    ...
    (test-case "full text committed before tool calls in complete cycle"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
      ...
      (check-false (ui-state-streaming-text final) "streaming-text clear at end of tool-call turn")
      (check-false (ui-state-busy? final) "not busy after turn.completed"))

    ;; Test 3: Streaming text does NOT accumulate across turns
    (test-case "streaming text does NOT accumulate across turns"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              ...
              (make-test-event "turn.completed" (hasheq 'termination 'completed 'turnId "turn-1"))
              (make-test-event "turn.started" (hasheq) #:turn-id "turn-2")
              (make-test-event "model.stream.delta" (hasheq 'delta "Turn2") #:turn-id "turn-2")))
      (define states (simulate-and-record s0 events))
      (define final (state-at states 6))
      (check-equal? (ui-state-streaming-text final) "Turn2" "no cross-turn accumulation")
      (check-equal? (transcript-length final) 1 "only one assistant entry from turn 1"))

    ;; Test 4: Full tool-use loop: answer + tool + follow-up answer
    (test-case "full tool-use loop: answer + tool + follow-up answer"
      (define s0 (initial-ui-state))
      (define events
        (list
      ...
      (check-false (ui-state-streaming-text final) "streaming-text clear after full loop")
      (check-false (ui-state-busy? final) "not busy after loop completes"))

    ;; Test 5: Tool calls without prior streaming (edge case)
    (test-case "tool calls without prior streaming"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "cancellation mid-stream clears streaming text"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
      ...
      (check-false (ui-state-streaming-text final) "streaming-text cleared on cancel")
      (check-false (ui-state-busy? final) "not busy after cancel")
      (check-equal? (transcript-length final) 0 "no transcript entries for cancelled turn"))

    ;; Test 7: Multiple tool calls preserve all entries
    (test-case "multiple tool calls preserve all entries"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "failed tool calls are recorded as tool-fail"
      (define s0 (initial-ui-state))
      (define events
        (list
         (make-test-event "turn.started" (hasheq))
    ...
    (test-case "empty streaming: no deltas, only completion"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "assistant.message.completed" (hasheq 'messageId "m1" 'content ""))
    ...
    (test-case "cancelled turn clears streaming text and busy state"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "out-of-order: tool.call.completed without prior tool.call.started"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "duplicate: two assistant.message.completed events"
      (define s0 (initial-ui-state))
      (define events
        (list
         (make-test-event "turn.started" (hasheq))
         (make-test-event "model.stream.delta" (hasheq 'delta "Hello"))
         (make-test-event "assistant.message.completed" (hasheq 'messageId "m1" 'content "Hello"))
         ;; Duplicate!
         (make-test-event "assistant.message.completed" (hasheq 'messageId "m1" 'content "Hello"))
         (make-test-event "turn.completed" (hasheq 'termination 'completed 'turnId "t1"))))
      (define states (simulate-and-record s0 events))
    ...
    (test-case "duplicate: two tool.call.started for same tool"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "assistant.message.completed"
    ...
    (test-case "out-of-order: model.stream.delta after assistant.message.completed"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "model.stream.delta" (hasheq 'delta "Hello"))
    ...
    (test-case "out-of-order: compaction.started without compaction.completed"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "model.stream.delta" (hasheq 'delta "Hi"))
              (make-test-event "assistant.message.completed" (hasheq 'messageId "m1" 'content "Hi"))
              (make-test-event "compaction.started" (hasheq))
              (make-test-event "turn.completed" (hasheq 'termination 'completed 'turnId "t1"))))
      (define states (simulate-and-record s0 events))
      (define final (state-at states 5))
      ;; Status message should still say "Compacting..." since completed never fired
      (check-equal? (ui-state-status-message final)
                    "Compacting..."
    ...
    (test-case "nested turn.started ×2 then turn.completed ×1: still busy"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "compaction.started during active streaming: status overrides"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
    ...
    (test-case "session.started during active session: session-id overwrite"
      (define s0 (initial-ui-state #:session-id "old-session"))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "model.stream.delta" (hasheq 'delta "Hello"))
              ;; New session starts while old is active
              (make-test-event "session.started" (hasheq 'sessionId "new-session"))
              (make-test-event "assistant.message.completed"
                               (hasheq 'messageId "m1" 'content "Hello"))
              (make-test-event "turn.completed" (hasheq 'termination 'completed 'turnId "t1"))))
      (define states (simulate-and-record s0 events))
    ...
    (test-case "queue.status-update sets queue-counts field"
      (define s0 (initial-ui-state))
      (define events
        (list (make-test-event "turn.started" (hasheq))
              (make-test-event "model.stream.delta" (hasheq 'delta "Hi"))
              (make-test-event "assistant.message.completed" (hasheq 'messageId "m1" 'content "Hi"))
              (make-test-event "queue.status-update" (hasheq 'steering 2 'followup 1))
              (make-test-event "turn.completed" (hasheq 'termination 'completed 'turnId "t1"))))
      (define states (simulate-and-record s0 events))
      (define final (state-at states 5))
