...
[![Version](https://img.shields.io/badge/version-0.15.0-blue.svg)](https://github.com/coinerd/q)
...
```bash
racket main.rkt --version  # q version 0.15.0
...
├── tests/          Full test suite (343 files)
...
**v0.15.0** — Request-Cycle Trace Logger. Structured trace.jsonl per session with sequence numbers, ISO timestamps, finish_reason capture, iteration decision tracking. Enriched events: model name, max_tokens, settings in request events. CLI `q sessions trace` command with --summary and --json modes. Config-gated via logging.trace.enabled. 5402 tests.

**v0.14.4** — Config Validation + Iteration Budget + Provider Settings. Config.json validation with clear error messages before mock fallback. Increased iteration budgets (soft 20→50, hard=max(soft*1.6, 80)). Exploration steering hint after 8+ consecutive tool calls. Provider settings (max-tokens) wired through to API request body. 5370 tests.
