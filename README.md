...
├── tests/          Full test suite (331 files)
...
**v0.14.1** — Remove Context Reduction from Retry. Retries always use same context (no trimming). TUI transcript O(1) append. Pair-aware context reducer, prompt pinning, progressive timeout, newline bleed fix, settings contract fix. 5330+ tests.
