...
[![Version](https://img.shields.io/badge/version-0.19.7-blue.svg)](https://github.com/coinerd/q)
...
```bash
racket main.rkt --version  # q version 0.19.7
...
├── tests/          Full test suite (397 files)
...
**For production or CI**, prefer environment variables (`Q_OPENAI_API_KEY`, `Q_ANTHROPIC_API_KEY`, etc.) or the **OS keychain backend** (`secret-tool` on Linux, macOS Keychain). Both avoid writing secrets to disk.
...
**v0.19.7** — CI Hardening. Pre-commit version sync + metrics lint. Dependency parity check (info.rkt + build-deps). CI-aware test guards for out-of-repo paths. Assertion guardrails (range checks replacing exact counts). Gitignore hygiene (stray file detection). CI log preservation + failure summary artifact. 5300+ tests.
