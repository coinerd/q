...
**CI improvements:**
- Add deprecation deadline linter (`scripts/lint-deprecation-deadlines.rkt`) — catches expired version-gated TODOs
- Remove `known-large` exceptions from dependency policy (all modules now under size targets)
...
- **RA-4**: New `scripts/audit-project.rkt` — reproducible project audit
  - Module inventory (774 modules, by-layer breakdown)
  - Risky API usage scanner
  - TODO/FIXME/HACK marker scanner
  - Oversized module detector
  - CI integration via `--ci` flag
- **RA-5**: Annotated deprecation TODOs with milestone references
- CI step for project audit (continue-on-error)

### PRs
- #2726 — W0: Reproducible audit script
- #2727 — W1: Deprecation TODO tracking
- #2728 — W2: Version bump + CI wiring

## v0.25.1 — 2026-04-29

### Module Decomposition — context-assembly.rkt (RA-3)

- **RA-3a**: Extract `context-pinning.rkt` (partition-messages, pinning helpers)
- **RA-3b**: Extract `context-fit.rkt` (truncate-messages-to-budget, fit-messages-from-recent)
- **RA-3c**: Extract `context-summary.rkt` (catalog, summary cache, context-summary-prompt, etc.)
- **VER**: Architecture fitness test for line count (<600); version bump to 0.25.1
- context-assembly.rkt: 825 → 587 lines

## v0.25.1 — 2026-04-29

### Security Hardening

- **RA-1a**: Allowlist execution policy for non-interactive/CI (`current-execution-policy`: warn/block/allowlist)
- **RA-1b**: High-risk destructive pattern confirmation notices (`[SECURITY NOTICE]` prefix in tool output)
- **RA-2**: Configurable secret scrubbing with extra denylist + allowlist for environment variables
- **VER**: Version bump to 0.25.1, security docs updated
...
**Housekeeping**
- Synced version references from 0.25.1 to 0.25.1 (VER-01)
...
- **Unified event system** — Replaced local `emit-gsd-event!` in gsd-planning.rkt
  with delegation to events.rkt + session bus fallback. String event names
  converted to symbols. Bus bridge connects GSD event bus to session event bus.
- **Normalized /go pipeline** — `handle-go-command` refactored to 3-step pipeline:
  `normalize-plan` → `validate-normalized-plan` → `make-wave-executor-from-validated`.
  Emits `gsd.plan.parsed`, `gsd.plan.normalized`, `gsd.plan.validated` events.
- **Archive result migration** — `archive-completed-plan!` returns `gsd-command-result`
  (gsd-ok/gsd-err) instead of hasheq. `cmd-done` simplified to direct delegation.
- **Transaction coverage** — `cmd-done` wrapped with `with-gsd-transaction` for
  atomic archive with rollback on failure. Emits `gsd.archive.failed` on rollback.
- **Write guard policy migration** — `gsd-write-guard` returns `policy-decision` directly
  instead of hasheq/#t. Consumers use `policy-allowed?`/`policy-blocked?` predicates.
- **Event taxonomy expansion** — Added `gsd.mode.changed`, `gsd.plan.archived`,
  `gsd.archive.failed`, and pipeline events to stable taxonomy.
- 3 new tests (archive result types, write guard policy-decision). 221 GSD tests pass.
...
- **F3: Shim Purification** — Removed behavioral logic from `gsd-planning-state.rkt`
  ...
  (deprecated, removal planned for v0.25.1).
...
**Goal**: Make CI green whenever local pre-commit passes. Addresses v0.25.1
...
**W4** — arch-report.rkt script (CI gate), version bump 0.25.1 → 0.25.1.
...
### Audit Remediation (v0.25.1 Post-Merge Fixes)
...
- **AUDIT-10**: Verified shell-quote direct import fix (already done in v0.25.1)
...
- **DOC-02**: Updated releasing.md verified-against to v0.25.1
...
- **Updated**: All verified-against markers to v0.25.1
...
**Wave 3 — Version bump 0.25.1 → 0.25.1** (PR #1663)
...
**Wave 3 — Version bump 0.25.1 → 0.25.1** (PR #1660)
- Version bump and CHANGELOG update
...
Milestone #92 — Post-v0.25.1 review follow-ups and planning prompt augmentation.
...
- Synced all version surfaces: info.rkt, README.md, docs/*.md, wiki-src/ to 0.25.1.
...
registration fixes from v0.25.1 review.
...
Security and robustness fixes from comprehensive audit of v0.25.1–v0.25.1
...
### Critical Fixes (from PROJECT_REVIEW_v0.25.1)
...
- Bump version references across all docs to 0.25.1
...
`.planning/REVIEW-v0.25.1.md` (73 findings: 6 CRITICAL, 23 MAJOR, 32 MINOR, 12 NIT).
...
- **Wave 11**: Fixed README v0.25.1 status block — replaced v0.13.x description with accurate v0.25.1 features (D1). Synced metrics (D2)
