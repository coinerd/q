...
(define (pending-marker-path session-log-path)
  ...
  (define base (if (path? session-log-path)
...
(define (verify-session-integrity session-log-path)
  ...
  (cond
    ...
    [else
       ...
       (for/fold ([valid '()]
         ...
         (cond
           [(not (jsonl-line-valid? line-text))
            (values valid
                    (append invalid
                            (list (hasheq 'line-number line-num
                                          'reason "invalid JSON")))
                    seen)]
           [else
            (define parsed
            ...
            (define missing
              (if parsed
                  (filter (lambda (f) (not (hash-has-key? parsed f)))
                          required-fields)
                  required-fields))
            (cond
              [(not parsed)
              ...
              [(not (null? missing))
               (values valid
                       (append invalid
                               (list (hasheq 'line-number line-num
                                             'reason
                                             (format "missing required fields: ~a"
                                                     missing))))
                       seen)]
     ...
     (define timestamps
       (map (lambda (p) (hash-ref (cdr p) 'timestamp)) valid-entries))
     ...
     (hasheq 'total-entries total-entries
             'valid-entries (length valid-entries)
             'invalid-entries invalid-entries
             'truncated-at-end? truncated-at-end?
             'entry-order-valid? entry-order-valid?)]))
...
(define (repair-session-log! session-log-path)
  ...
  (cond
    [(not (file-exists? session-log-path))
     (hasheq 'entries-kept 0 'entries-removed 0)]
    [else
     (define lines
       (call-with-input-file session-log-path
         (lambda (in) (sequence->list (in-lines in)))
       ...
       (for/fold ([valid '()]
         ...
         (cond
           [(<= (string-length (string-trim line)) 0)
            ...
            (values valid rmvd seen)]
           [(not (jsonl-line-valid? line))
            (values valid (+ rmvd 1) seen)]
           [else
            (define parsed
              (with-handlers ([exn:fail? (lambda (e) #f)])
            ...
            (cond
              ...
              [(not (null? missing))
               (values valid (+ rmvd 1) seen)]
              [(and entry-id (hash-has-key? seen entry-id))
               (values valid (+ rmvd 1) seen)]
              [else
               (values (append valid (list parsed))
                       rmvd
     ...
     (hasheq 'entries-kept (length valid-entries)
