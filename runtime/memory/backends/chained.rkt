...
(define (make-chained-backend l1 l2 #:write-through? [write-through? #t] #:l2-read? [l2-read? #t])
  (memory-backend (format "chained(~a+~a)" (memory-backend-name l1) (memory-backend-name l2))
                  ...
                  (lambda (id scope)
                    ...
                    r1)
                  ;; list: merge from both, dedup
                  (lambda (query)
                    (define items-l1 (memory-result-value (gen:list-memory l1 query)))
                    (cond
                      [(and l2-read? (gen:memory-available? l2))
                       (define items-l2 (memory-result-value (gen:list-memory l2 query)))
