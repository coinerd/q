...
the report-only inventory (`racket scripts/run-tests/classify-metadata.rkt
--lint-metadata`) returned exit 0 with
...
`docs/TEST_CONVENTIONS.md`. Re-verified at v1.00.11 close: the same
...
`@boundary`); Phase 0 rolling *trend* updates beyond the retained v1.00.11
snapshot; and any test-suite consolidation, which stays blocked pending
adequacy evidence per the consolidation policy.

**Delivered (v1.00.11):** a deterministic, retained feedback and reliability
baseline now exists — `docs/reports/test-feedback-baseline-v1.00.11.md` plus
its machine-readable companion, generated by
`scripts/run-tests/baseline-report.rkt --runs <run-ids>` from twelve
maintainer-named runs (ten successful main/PR L3 runs and the two v1.00.10 L4
...
**Status of the time budgets (v1.00.11, from the retained baseline):** the
...
in `docs/reports/test-feedback-baseline-v1.00.11.md`. No target in this table
...
This measurement obligation is satisfied as of v1.00.11 by
`docs/reports/test-feedback-baseline-v1.00.11.md` (twelve runs: ten successful
...
| L3 | Measured — adopt report p50/p95 as the baseline of record; target *maintain or improve* | Ten successful main/PR runs in the v1.00.11 report | Test infrastructure | Next retained refresh |
| L4 | Measured — adopt cold/warm runs 32522576690/32526868295 as the baseline of record | Two full-regression runs in the v1.00.11 report | Test infrastructure | Next retained refresh |
