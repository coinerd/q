...
and workflow gates, and since v1.00.11 the restored Racket 8.11
`test-cross-version` gate on main push (its 8.11 entry comes from the
version-indexed `ci/racket-package-lock.rktd`, selected by exact runtime match —
no cross-runtime fallback). The non-CI boundary is enforced by grepping
...
scheduled for v1.00.15; that milestone slipped one release, and at v1.00.15
...
`docs/TEST_CONVENTIONS.md`. Re-verified at v1.00.15 close: the same
repository invocation from a clean checkout returns exit 0 with
`missing-required=0` across all 1,336 files (invalid=0, deprecated-alias=0,
explicit=1336, heuristic-only=0, inventory schema 1) against the enforced
canonical file-list digest
`ab265d57edba32d7499390578bee67c776ce3e1afe1fc9eac3e88096bf29f7e4`,
recorded in `docs/reports/test-regression-log.md`.
...
### v1.00.16 — fast-gate halving objective (baseline of record 2026-08-24)

| Element | Status (v1.00.16) |
|---|---|
| Baseline of record | `docs/reports/test-feedback-baseline-v1.00.16.md` + machine-readable companion + `fast-gate-budget-v1.00.16.md`/`.json`, generated by `scripts/run-tests/baseline-report.rkt --runs 32745843124,32748197712 --fast-budget`; `--check` is byte-identical (`md=identical json=identical`). |
| Fast-gate halving objective | fast-gate p50 (setup + max shard) ≤ 244.0 s — ≤ 50% of the W0-of-record p50 488.0 s. Retained 2-run sample p50 = 627.0 s (619.0 / 635.0) → achieved ratio **1.2848×** of the baseline; verdict **MISSED** — recorded with attribution, no target massaging (shortfall and next lever in the fast-gate budget and the regression log). |
| Grouped-execution share | Not measured in the retained CI sample: the fast gate runs the `fast` suite in subprocess isolation on three shards; grouped in-process execution remains a local `unit-fast` capability (see "Local execution modes" row) — a scoped unknown, never estimated. |
| Remediation report | `docs/reports/fast-timesink-remediation-v1.00.16.md` (W2): the W0 slowest fast files — OAuth2 callback one-shot-server tests and peers — fixed via the `#:on-complete` production seam and shared fixtures; per-file before/after medians over 2 runs each; no test deleted, weakened, or merged. |
| Prepared-env activation | W3 cutover (GO, observation pending): ubuntu fast-gate lanes opt into `prepared-environment: auto` behind the `RACKET_PREPARED_ARTIFACT=off` rollback switch; producer `fast-env` + guarded fallback. The v1.00.16 sample runs predate the cutover merge and used the legacy setup path (setup 343/348 s) — the warm-restore observation run is the named next lever. |
| Shard-count decision | `FAST_SHARD_COUNT` study (W3, report-only): **KEEP 3**; activation requires predicted max-shard improvement at N=4/6/8, inventory preservation, an accepted runner-minute cost delta, and an observed post-activation check run recorded in the regression log. |
...
**Delivered (v1.00.16):** a regenerable halving-objective baseline of record —
`docs/reports/test-feedback-baseline-v1.00.16.md` (+ JSON companion) and
`docs/reports/fast-gate-budget-v1.00.16.md` (+ JSON), produced by
`scripts/run-tests/baseline-report.rkt --runs 32745843124,32748197712
--fast-budget` and verified byte-identical with `--check`. The W2 fast-timesink
remediation report, the W3 prepared-env cutover, and the `FAST_SHARD_COUNT`
KEEP-3 study are recorded in the regression log and the adoption-status section
...
remeasurement is deferred until a warm prepared-env hosted run exists (W3
observation gate); the current 1.2848× shortfall and its cost attribution stay
recorded until then.

**Delivered (v1.00.11):** a deterministic, retained feedback and reliability
baseline now exists — `docs/reports/test-feedback-baseline-v1.00.15.md` plus
its machine-readable companion, generated by
`scripts/run-tests/baseline-report.rkt --runs <run-ids>` from twelve
maintainer-named runs (ten successful main/PR L3 runs and the two v1.00.15 L4
full-regression runs 32522576690 cold / 32526868295 warm). Identical inputs
reproduce byte-identical outputs (`--check`). All figures below are read from
that report; nothing in this plan restates or re-derives them.
...
This measurement obligation is satisfied as of v1.00.15 by
`docs/reports/test-feedback-baseline-v1.00.15.md` (twelve runs: ten successful
main/PR L3 runs plus L4 runs 32522576690 and 32526868295; per-suite and
per-shard p50/p95 computed by the method declared in that report — linear
interpolation between closest ranks over the sorted per-(suite,shard) job
step wall-clock sample; no p90 is computed or implied). Parallel-only
instability is tracked there as a measured rate over the retained runs, and no
known-failure ledger entry counts as an exemption from that rate.

### Target decisions ledger

Every decision recorded from a baseline must carry evidence. No target has
been revised from the original 5s/30s/120s aspiration.
...
A revision to any of these numbers is valid only if it states the sample
(run IDs), the reason, the owner, and a remeasurement date, and only if the
underlying report is regenerable byte-for-byte from named inputs.
