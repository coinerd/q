...
(reproduced from `raw/red-first-evidence.txt`, SHA256SUMS adjacent).
...
- The preflight's live-path verification (branch published, tip ancestry,
  policy presence) is exercised at the classifier/preflight-function level and
  through coordinator seam tests with typed stops; full live-path preflight
  against github.com requires credentials and is covered by the protected CI
  lane on the wave PR rather than offline fixtures.
