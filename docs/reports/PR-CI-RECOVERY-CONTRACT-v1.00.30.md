...
- **Quantiles:** sorted observations, linear interpolation at (n-1)*p, milliseconds retained until display; W1 must implement and pin the exact helper with `pr-latency-guard` (an outstanding W1 obligation), not by `tests/test-ci-exit-truth.rkt` — an earlier draft of this contract asserted that pin and the W0 review-fix corrected it. The legacy rounded/midpoint helper is not reused. W1 must add boundary tests covering <= versus > and min(10%, 60 s).
