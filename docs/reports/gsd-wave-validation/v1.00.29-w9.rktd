...
(validation
 ...
 (results
  ...
  ((criterion . "PROTECTED-CI INCIDENT (first production run of the W9 producer) — fail-closed aggregate refusal exposed two pre-existing defects")
   (result
    . "CI run 34714192043 (PR #9680): all three test shards green, but the W9 producer REFUSED the aggregate: 'refusing to write a proof bundle over a non-clean aggregate (files=1190 fail=1 timeout=0 skip=0)'. Forensics over the retained artifacts: (1) test-results-fast-0 records tests/test-prepared-env-report.rkt exit 1 failed 1 (check-equal? actual 'verified' expected 'unknown' at :266) — the W6 'no telemetry' isolation case inherited ambient Q_PREPARED_ENV_* (GITHUB_ENV exports from the W6 identity wiring) and classified from real CI telemetry; the identical masked failure is present in PR #9674's run 34692582693 (test-results-fast-0, job green) — proving it predates W9. (2) The reason the failing shard job stayed green: the ci.yml shard steps pipe the runner through tee WITHOUT pipefail, so the runner's nonzero exit is masked — a repo-wide CI-integrity defect (~25 sites, 7 workflows), filed as BUG-0073 (standalone fix; the campaign's own producer caught what the PR gate had been swallowing since at least W6). IN-WAVE REMEDIATION: run-script in tests/test-prepared-env-report.rkt now scrubs the four Q_PREPARED_ENV_* variables before applying per-case assignments (verified locally under forced ambient vars: 32/32 green); the pipefail sweep is deliberately NOT smuggled into this wave — it is BUG-0073's standalone fix. The producer's behavior on this incident is exactly the contracted §9 semantics: refuse + named cause + loud, never a green skip"))
