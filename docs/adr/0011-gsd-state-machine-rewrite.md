...
### State Machine Core (v0.24.6 → v0.24.6)
...
### v0.24.6 — State Unification & Command Contracts (F1, F2, F5)
...
- **Shim purification**: `gsd-planning-state.rkt` is now pure delegation (DEPRECATED, removal planned for v0.24.6).
- **Invariant checker**: `gsd-invariants-hold?` validates mode, wave bounds, completed set integrity.

### v0.24.6 — Policy Engine & Transaction Wrappers (F3, F4)
...
### v0.24.6 — Pipeline Refactor & Observability (F7, F6)
...
### v0.24.6 — Documentation & Fitness Gates (F8)
...
### v0.24.6 — Production Integration Wiring
