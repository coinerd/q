...
(define (browser-target->jsexpr t)
  (hasheq 'url (browser-target-url t)
          'tab-selector (browser-target-tab-selector t)
          'viewport-width (browser-target-viewport-width t)
          'viewport-height (browser-target-viewport-height t)))
...
(define (browser-action->jsexpr a)
  (cond
    [(browser-action-navigate? a)
     (hasheq 'type "navigate" 'url (browser-action-navigate-url a)
             'wait-until (browser-action-navigate-wait-until? a))]
    [(browser-action-click? a)
     (hasheq 'type "click" 'selector (browser-action-click-selector a)
             'button (browser-action-click-button a))]
    [(browser-action-type? a)
     (hasheq 'type "type" 'selector (browser-action-type-selector a)
             'text (browser-action-type-text a)
             'clear-first (browser-action-type-clear-first? a))]
    [(browser-action-press? a)
     (hasheq 'type "press" 'key (browser-action-press-key a)
             'modifiers (browser-action-press-modifiers a))]
    [(browser-action-extract? a)
     (hasheq 'type "extract" 'selector (browser-action-extract-selector a)
             'extract-type (browser-action-extract-extract-type a))]
    [(browser-action-screenshot? a)
     (hasheq 'type "screenshot" 'selector (browser-action-screenshot-selector a)
             'full-page (browser-action-screenshot-full-page? a))]
    [(browser-action-scroll? a)
     (hasheq 'type "scroll" 'direction (browser-action-scroll-direction a)
             'amount (browser-action-scroll-amount a))]
    [(browser-action-wait? a)
     (hasheq 'type "wait" 'selector (browser-action-wait-selector a)
             'timeout-ms (browser-action-wait-timeout-ms a))]
    [else (error 'browser-action->jsexpr "unknown action: ~a" a)]))

(define (jsexpr->browser-action j)
  (define t (hash-ref j 'type))
  (case t
    [("navigate") (browser-action-navigate (hash-ref j 'url)
                                           (hash-ref j 'wait-until "load"))]
    [("click") (browser-action-click (hash-ref j 'selector)
                                     (hash-ref j 'button "left"))]
    [("type") (browser-action-type (hash-ref j 'selector)
                                   (hash-ref j 'text)
                                   (hash-ref j 'clear-first #f))]
    [("press") (browser-action-press (hash-ref j 'key)
                                     (hash-ref j 'modifiers '()))]
    [("extract") (browser-action-extract (hash-ref j 'selector)
                                         (hash-ref j 'extract-type "text"))]
    [("screenshot") (browser-action-screenshot (hash-ref j 'selector #f)
                                               (hash-ref j 'full-page #f))]
    [("scroll") (browser-action-scroll (hash-ref j 'direction "down")
                                       (hash-ref j 'amount 300))]
    [("wait") (browser-action-wait (hash-ref j 'selector #f)
                                   (hash-ref j 'timeout-ms 5000))]
...
(struct browser-observation (url
                             title
                             text-content
                             visible-text
                             dom-summary
...
(define (browser-observation->jsexpr o)
  (hasheq 'url (browser-observation-url o)
          'title (browser-observation-title o)
          'text-content (browser-observation-text-content o)
          'visible-text (browser-observation-visible-text o)
          'dom-summary (browser-observation-dom-summary o)
          'accessibility-tree (browser-observation-accessibility-tree o)
          'screenshot-mime (browser-observation-screenshot-mime o)
          'screenshot-bytes (and (browser-observation-screenshot-bytes o)
                                 (bytes->string/utf-8
                                  (base64-encode (browser-observation-screenshot-bytes o))))
          'console-errors (browser-observation-console-errors o)
          'network-requests (browser-observation-network-requests o)
          'viewport-size (browser-observation-viewport-size o)
          'metadata (browser-observation-metadata o)))
...
(struct browser-session-info (id
                              status
                              created-at
                              last-activity
                              profile-kind
...
(define (browser-session-info->jsexpr s)
  (hasheq 'id (browser-session-info-id s)
          'status (browser-session-info-status s)
          'created-at (browser-session-info-created-at s)
          'last-activity (browser-session-info-last-activity s)
          'profile-kind (browser-session-info-profile-kind s)
          'artifact-dir (browser-session-info-artifact-dir s)))
