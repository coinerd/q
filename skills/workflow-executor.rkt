...
(define (execute-step-group group idx variables prev-result exec-ctx)
  ;; Spawn APIs own the security decision.  Keeping approval here would make
  ;; singleton workflows prompt twice and would authorize parallel work using
  ;; a coarse group preview instead of the batch's immutable execution plan.
  (cond
    [(= (length group) 1)
