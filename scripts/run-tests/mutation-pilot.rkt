...
(struct site (module offset line col operator description) #:transparent)
...
(define (offset->line/col text offset)
  ...
  (define col (if m (- (string-length prefix) (cdar m)) (string-length prefix)))
  (values line col))
...
(define (enumerate-sites module text)
  (sort
   (append*
    (for/list ([op (in-list (mutation-operators))])
...
(define (apply-single-mutation text s)
  (define op
    (findf (lambda (o) (eq? (car o) (site-operator s))) (mutation-operators)))
  (cond
    [(not op) text]
    [else
     ...
     (if (and m (= (caar m) (site-offset s)))
         (string-append (substring text 0 (caar m))
                        replacement
                        (substring text (cdar m)))
         text)]))
...
(define (parse-plan-mutants module-texts #:max-mutants [max-mutants 25])
  (define all-sites
    (append*
     (for/list ([mt (in-list module-texts)])
  ...
  (for ([entry (in-list all-sites)])
    ...
    (cond
      [(not (module-text-parses? mutated))
       (set! planned
             (cons (list (struct-copy site s
                          [operator
                           (string->symbol
                            (string-append (symbol->string (site-operator s))
                                           ":invalid"))])
                        text)
                   planned))]
      [else
...
(define (manifest->killing-map root modules)
  ...
  (cond
    ...
    [else
     ...
     (define wanted (for/hash ([m (in-list modules)]) (values m #t)))
     ...
     (for* ([entry (in-list (hash-ref manifest 'entries '()))]
       ...
       (hash-update! acc covered
                     (lambda (old)
                       (sort (cons (hash-ref entry 'test) old) string<?))
                     '()))
     acc]))
...
(define (run-one-test test-path root timeout-sec)
  ...
  (define-values (sp stdout _stdin stderr)
    (parameterize ([current-directory root])
      (subprocess #f #f #f
                  (find-executable-path "raco") "test" "--quiet" test-path)))
  (define out-drain
    (thread (lambda () (copy-port stdout (open-output-nowhere)))))
  (define err-drain
    (thread (lambda () (copy-port stderr (open-output-nowhere)))))
  ...
  (define status
    (cond
      [finished?
       (if (eq? (subprocess-status sp) 0) 'pass 'fail)]
      [else
...
(define (restore-module! root module-texts module)
  ...
  (when mt (write-mutated! root (car mt) (cdr mt))))
  ...
  (dynamic-wind
    ...
    (lambda ()
      ...
      (for ([entry (in-list planned)])
        ...
        (set! results
              (cons (hasheq 'module module
                           'operator (symbol->string (site-operator s))
                           'line (site-line s)
                           'col (site-col s)
                           'description (site-description s)
                           'status (symbol->string status)
                           'killing_test (or killing-test (json-null))
                           'tests tests
                           'test_outcomes
                           (for/list ([o (in-list outcome-details)])
                             (list (list-ref o 0)
                                   (symbol->string (list-ref o 1))
                                   (list-ref o 2)))
                           'duration_seconds duration)
                    results))
        (write-pilot-json! out-dir
                           (pilot->jsexpr (reverse results) opts
                                          #:aborted aborted-early
                                          #:start-ms start-ms))))
    (lambda ()
      (restore-all! out-dir root modules)))
  (pilot->jsexpr (reverse results) opts
                 #:aborted aborted-early
                 #:start-ms start-ms))
...
(define (pilot->jsexpr results opts #:aborted [aborted #f] #:start-ms [start-ms #f])
  ...
  (hasheq
   'schema_version 1
   'generated_by "racket scripts/run-tests/mutation-pilot.rkt"
   'tool_decision "repo-owned reviewed micro-mutations (no external tool met criteria)"
   'pilot_version pilot-version
   'racket_version (version)
   'modules (hash-ref opts 'modules)
   'budget_seconds (hash-ref opts 'budget-seconds 300)
   'max_mutants (hash-ref opts 'max-mutants 25)
   'test_timeout_seconds (hash-ref opts 'test-timeout 120)
   'dry_run (hash-ref opts 'dry-run #f)
   'mutants results
   'totals (hasheq
            'planned (length results)
            'killed killed
            'survived survived
             'timeout (count-status "timeout")
             'invalid (count-status "invalid")
             'not_run (count-status "not-run")
             'not_covered (count-status "not-covered"))
    'detection_score (if (> (+ killed survived) 0)
                         (exact->inexact (/ killed (+ killed survived)))
                         (json-null))
   'aborted_early aborted
   'wall_clock_seconds (if start-ms
...
(define (write-findings! out-dir jsexpr)
  ...
  (define (lines-for status heading)
    (define ms
      (for/list ([m (in-list (hash-ref jsexpr 'mutants))]
        ...
        (format "- ~a:~a:~a ~a"
                (hash-ref m 'module) (hash-ref m 'line) (hash-ref m 'col)
                (hash-ref m 'description))))
    (if (pair? ms) (cons "" (cons heading ms)) '()))
  (define detection (hash-ref jsexpr 'detection_score))
  (define lines
    (list
     "# Mutation pilot findings (informational — never a gate)"
     ""
  ...
  (with-output-to-file (build-path (string->path out-dir) "findings.md")
    (lambda ()
      (display (string-append (string-join all-lines "\n") "\n")))
    #:exists 'replace))
...
(define (parse-tests-for specs)
  ...
  (for/list ([spec (in-list specs)])
    (match (string-split spec "=")
      [(list module tests)
       (cons module (filter non-empty-string? (string-split tests ",")))]
...
(define (changed-modules-from-diff root base)
  ...
  (unless git (raise-user-error 'mutation-pilot "git not found for --from-diff"))
  (define-values (sp out _in _err)
    (subprocess #f #f #f git "-C" root "diff" "--name-only" base))
...
(define (main [argv (current-command-line-arguments)])
  ...
  (command-line
   ...
   #:once-each
   [("--budget") sec "wall-clock budget in seconds (default 300)"
    (set! budget (or (string->number sec)
                     (raise-user-error 'mutation-pilot "bad --budget: ~a" sec)))]
   [("--max-mutants") n "total mutant cap (default 25)"
    (set! max-mutants (or (string->number n)
                          (raise-user-error 'mutation-pilot "bad --max-mutants: ~a" n)))]
   [("--test-timeout") sec "per-test timeout in seconds (default 120)"
    (set! test-timeout (or (string->number sec)
                           (raise-user-error 'mutation-pilot "bad --test-timeout: ~a" sec)))]
   [("--out") dir "artifact directory (default reports/mutation-pilot)"
    (set! out-dir dir)]
   [("--root") dir "repo root (default cwd)"
    (set! root dir)]
   [("--dry-run") "plan only: enumerate sites, write plan, execute nothing"
    (set! dry-run #t)]
   [("--from-diff") base "mutate changed .rkt modules from git diff BASE (no repo-wide path)"
    (set! from-diff base)]
   [("--modules") list "comma-separated repo-relative modules (required unless --from-diff)"
    (set! modules-spec list)]
   [("--tests") list "global fallback killing tests (comma-separated)"
    (set! global-tests (filter non-empty-string? (string-split list ",")))]
   #:multi
   [("--tests-for") spec "module=test1,test2 — explicit killing tests (repeatable)"
  ...
  (define opts
    (hasheq 'root root
            ...
            'max-mutants max-mutants
            'test-timeout test-timeout
            'killing-map killing-alist))
  (unless dry-run
    (displayln (format "mutation-pilot: ~a module(s), budget ~as, cap ~a mutant(s)"
                       (length modules) budget max-mutants)))
