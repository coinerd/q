...
(define (make-in-process-exit-handler exit-box)
  (lambda (code)
    (raise (in-process-test-exit
            (format "test file requested (exit ~a) during in-process execution" code)
...
(define (run-single-file/in-process test-path #:timeout [timeout #f])
  ...
  (cond
    ...
    [else
       ...
       (parameterize ([current-custodian cust])
          (thread (lambda ()
                    (with-handlers ([in-process-test-exit?
                                     (lambda (e)
                                       (set-box! exit-code (in-process-test-exit-code e)))]
                                    [exn:fail? (lambda (e)
     ...
     (define stderr-bytes (string->bytes/utf-8 (get-output-string stderr-out)))
      (if completed?
          (let ([result (parse-result-bytes test-path stdout-bytes stderr-bytes
                                            (or (unbox exit-code) 0) (elapsed))])
...
(define (main args)
   ...
   (when changed-base
      ...
      (when explain?
        ...
        (when json-out
          (call-with-output-file json-out #:exists 'truncate/replace
            (lambda (out)
              (write-json (hasheq 'explain #t
                                  'selection (selection->jsexpr sel)) out)
              (newline out))))
        (exit 0))
     ...
     (unless (pair? merged)
       ...
       (when json-out
         (call-with-output-file json-out #:exists 'truncate/replace
           (lambda (out)
             (write-json (hasheq 'error 'empty-selection
                                 'selection (selection->jsexpr sel)) out)
             (newline out))))
       (exit 3))
     ...
     (when (equal? prioritize "impact")
       ...
       (define ctx (make-prioritize-ctx explicit
                                        (hash-ref sel 'selected '())
                                        hist-weights
                                        boundaries))
     ...
     (when impact-dry-run?
       ...
       (define plan (hasheq 'plan merged
                            'selection (selection->jsexpr sel)
                            'prioritization (or prioritize-payload
                                                (hasheq 'prioritized #f))))
       (if json-out
           (call-with-output-file json-out #:exists 'truncate/replace
             (lambda (out) (write-json plan out) (newline out)))
           (begin (write-json plan) (newline)))
       (exit 0))
  ...
  (define plan-mode
    (cond [(equal? shard-plan "report") 'report]
  ...
  (when (and plan-mode (or (eq? plan-mode 'report) (> shard-total 1)))
    ...
    (define plan
      (build-shard-plan/safe all-suite-files
                             shard-total
                             #:durations dur
                             #:profile-skips? (lambda (f)
                                                (profile-skips-test?
                                                 profile
                                                 (hash-ref (get-file-metadata f) 'requires '())))
    ...
    (printf ";; run-tests: duration-aware plan (~a) replaces round-robin for shard ~a/~a~n"
            (shard-plan-mode plan) shard-index shard-total)
  ...
  (unless (pair? suite-files)
   ...
   (define suite-label (if changed-base "impact" (symbol->string suite)))
