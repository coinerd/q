# Test Ownership Map (generated — do not edit)

Generated by `scripts/run-tests/inventory.rkt --ownership-map` (W3).
Every production area must have an accountable test destination.

| Area | Production source | Suite | Tests | Boundary tags | Owning path |
|---|---|---|---|---|---|
| runtime | runtime/ | runtime | 155 | e2e, integration, unit | tests |
| provider | llm/ (provider adapters) | provider | 8 | integration, unit | tests |
| session | agent/ (agent session) | runtime | 138 | e2e, integration, unit | tests |
| tools | tools/ + agent/roles/tool-gateway | tools | 3 | unit | tests |
| extensions | extensions/ | extensions | 144 | e2e, integration, pure, unit | tests |
| tui | tui/ | tui | 89 | integration, unit | tests |
| workflows | scripts/run-tests/workflows/ + GSD | workflows | 31 | integration | tests/workflows |

No gaps: every production area has a test destination.
